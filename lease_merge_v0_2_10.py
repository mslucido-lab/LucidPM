...
def get_lease_merge_context(tenant_id: int, lease_id: int, db: str = TEST_DB_NAME) -> dict[str, str]:
    ...
    context = {
        ...
        # Amendment tokens - resolve from ParentLeaseID and ExecutionDate.
        # Base leases return empty strings.
        "AmendmentEffectiveDate": _short_date_no_leading_zero(execution_date) if parent_lease_id else "",
        "AmendmentEffectiveDateLong": _long_date(execution_date) if parent_lease_id else "",
        "AmendmentNumber": amendment_number_word,
        "OriginalLeaseExecutionDate": _long_date(parent_execution) if parent_lease_id else "",
        "OriginalLeaseEffectiveDate": _long_date(parent_start) if parent_lease_id else "",
