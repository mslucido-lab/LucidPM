...
class LeaseDocumentState(AppState):
    ...
    def _validate_piece_range(self, start: int, end: int, ignore_piece_id: int = 0) -> bool:
        """Validate page range and exclude the current piece while editing."""
        ...
        if ignore_id > 0:
            rows = run_query(
                ...
                "WHERE LeaseSourceDocumentID = ? AND LeaseDocumentPieceID <> ?",
                (self.selected_source_document_id, ignore_id), db=self.db,
            )
        else:
            rows = run_query(
                "SELECT LeaseDocumentPieceID, StartPage, EndPage, PieceName "
                "FROM LeaseDocumentPieces WHERE LeaseSourceDocumentID = ?",
                (self.selected_source_document_id,), db=self.db,
            )
    ...
    def create_piece(self):
        ...
        if not self._validate_piece_range(start, end, current_edit_id):
            return
