...
class LeaseDocumentState(AppState):
    ...
    def _load_lease_templates(self):
        ...
        self.lease_template_ids = [int(t.template_id) for t in self.lease_templates]
    ...
    def _lease_template_label(self, t: LeaseTemplateRow) -> str:
        ...
        return f"{name} - {prop} [ID={int(t.template_id or 0)}]" if prop else f"{name} [ID={int(t.template_id or 0)}]"

    def _template_label_for_id(self, template_id: int) -> str:
        tid = int(template_id or 0)
        for t in self.lease_templates:
            if int(t.template_id or 0) == tid:
    ...
    def set_selected_template_label(self, label: str):
        ...
        try:
            ...
            template_id = int(self.lease_template_ids[idx])
        except Exception:
            template_id = 0
        if template_id > 0:
    ...
    def select_lease_template(self, template_id: int):
        ...
        self.slot_template_id = int(template_id or 0)
        self.selected_template_label = self._template_label_for_id(self.selected_template_id)
        self.form_error = ""
        self.form_success = ""
        rows = run_query(
    ...
    def save_lease_template(self):
        ...
        try:
            if self.lt_template_mode == "edit" and int(self.selected_template_id or 0) > 0:
                ...
                self.slot_template_id = int(self.selected_template_id or 0)
                self.selected_template_label = self._template_label_for_id(self.selected_template_id)
                self.lt_template_mode = "edit"
    ...
    def reset_template_section_form(self):
        if int(self.slot_template_id or 0) <= 0 and int(self.selected_template_id or 0) > 0:
    ...
    def _ensure_selected_template_before_slot_save(self) -> bool:
        ...
        def apply_template_id(template_id: int) -> bool:
            ...
            self.slot_template_id = tid
            self.selected_template_label = self._template_label_for_id(tid)
            self.lt_template_mode = "edit"
            return True

        if int(self.slot_template_id or 0) > 0:
        ...
        if label:
            if label in self.lease_template_labels:
                try:
                    idx = self.lease_template_labels.index(label)
                    if apply_template_id(int(self.lease_template_ids[idx])):
                        return True
                except Exception:
                    pass
        ...
        if template_name:
            try:
                ...
                if rows and apply_template_id(int(rows[0].get("LeaseTemplateID") or 0)):
                    return True
            except Exception:
                pass
        ...
        if template_name:
            for t in self.lease_templates:
                if str(t.template_name or "").strip() == template_name:
                    if apply_template_id(int(t.template_id or 0)):
                        return True

        if len(self.lease_templates) == 1:
            return apply_template_id(int(self.lease_templates[0].template_id or 0))
    ...
    def save_template_section(self):
        ...
        try:
            ...
            self.selected_template_id = active_template_id
            self.slot_template_id = active_template_id
            self.selected_template_label = self._template_label_for_id(active_template_id)
            self.reset_template_section_form()
        except Exception as ex:
        ...
        rx.table.cell(
            rx.hstack(
                ...
                rx.button(
                    ...
                    variant="soft",
                    color_scheme="green",
                    on_click=lambda: LeaseDocumentState.start_new_template_section_for_template(row.template_id),
                ),
                spacing="2",
            )
        ),
