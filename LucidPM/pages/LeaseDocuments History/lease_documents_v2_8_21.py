...
class LeaseDocumentState(AppState):
    ...
    def _lease_template_label(self, t: LeaseTemplateRow) -> str:
        ...
        return f"{name} - {prop} [ID={_safe__safe_int(t.template_id)}]" if prop else f"{name} [ID={_safe__safe_int(t.template_id)}]"

    def _template_label_for_id(self, template_id: int) -> str:
        tid = _safe_int(template_id)
        for t in self.lease_templates:
            if _safe__safe_int(t.template_id) == tid:
    ...
    def _ensure_template_label_for_id(self, template_id: int) -> str:
        ...
        if not label:
            ...
            if rows:
                ...
                if not any(_safe__safe_int(t.template_id) == tid for t in self.lease_templates):
        ...
        if template_name:
            for t in self.lease_templates:
                if str(t.template_name or "").strip() == template_name:
                    if apply_template_id(_safe__safe_int(t.template_id)):
                        return True
