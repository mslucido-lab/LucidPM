...
class LeaseDocumentState(AppState):
    ...
    def edit_section(self, section_id: int):
        ...
        rows = run_query(
            ...
            "StartPage, EndPage, SortOrder, IsReusable, IsActive, Content FROM LeaseDocumentSections WHERE LeaseDocumentSectionID = ?",
            (int(section_id),), db=self.db,
        )
        if not rows:
    ...
    def parse_pasted_clauses(self):
        ...
        import re
        lines = raw.replace("\r\n", "\n").replace("\r", "\n").split("\n")
        heading_re = re.compile(r"^\s*(?:[0-9]+(?:\.[0-9]+)*[\.)]?|[A-Z][\.)]|[a-z]\)|[IVXLCDM]+[\.)])\s+\S+", re.IGNORECASE)
        ...
        for idx, chunk in enumerate(chunks, start=1):
            ...
            if not content:
                continue
            first_line = next((ln.strip() for ln in chunk if ln.strip()), "")
            article, label = self._parse_clause_header(first_line)
            label = label or f"Clause {idx}"
            tag = self._slug_from_label(label)
            preview = content.replace("\n", " ").strip()
            if len(preview) > 160:
                preview = preview[:157].rstrip() + "..."
            drafts.append(DraftClauseRow(draft_id=idx, article_number=article, display_label=label, clause_tag=tag, preview=preview, content=content))
    ...
    def load_draft_clause(self, draft_id: int):
        ...
        for draft in self.draft_clauses:
            if int(draft.draft_id or 0) == did:
                ...
                self.form_success = "Draft clause loaded into the editor. Review it, then click Save Draft as New Section."
                return
        self.form_error = "Draft clause not found."

    def _save_text_clause_section(self, draft: DraftClauseRow) -> int:
        if not self.selected_source_document_id:
            raise ValueError("Select a source document on the Load tab before saving pasted clauses.")
        source_path = str(self.selected_source_path or "TEXT_ONLY").strip() or "TEXT_ONLY"
        root = self.storage_root.strip() or DEFAULT_DOCUMENT_ROOT
        ...
        run_exec(
            ...
            "(LeaseSourceDocumentID, LeaseID, SectionType, SectionName, ExhibitCode, StartPage, EndPage, "
            "StoredFilePath, StorageRoot, RelativePath, SortOrder, IsReusable, IsActive, ClauseTag, ArticleNumber, DisplayLabel, Content, UpdatedOn) "
            "VALUES (?, NULL, ?, ?, NULL, 0, 0, ?, ?, NULL, ?, 1, 1, ?, ?, ?, ?, SYSDATETIME())",
            (
                int(self.selected_source_document_id),
        ...
        rows = run_query("SELECT TOP 1 LeaseDocumentSectionID FROM LeaseDocumentSections ORDER BY LeaseDocumentSectionID DESC", db=self.db)
    ...
    def save_loaded_draft_as_section(self):
        ...
        if not str(self.p_content or "").strip():
            ...
            return
        draft = DraftClauseRow(draft_id=0, article_number=self.p_article_number, display_label=self.p_display_label or self.p_section_name or "Clause", clause_tag=self.p_clause_tag, preview="", content=self.p_content)
        try:
            new_id = self._save_text_clause_section(draft)
    ...
    def save_all_draft_clauses(self):
        ...
        if not self.draft_clauses:
            self.form_error = "Split pasted text before saving draft clauses."
            return
        try:
            saved = 0
            for draft in self.draft_clauses:
                ...
                saved += 1
            self.form_success = f"Saved {saved:,} draft clause section(s)."
            self.draft_clauses = []
            self.draft_clause_count = "0 draft clause(s)"
            self._load_sections()
            self._load_all_sections()
            ...
            self.form_error = f"Could not save draft clauses: {ex}"
    ...
    return rx.vstack(
        ...
        rx.card(
            rx.vstack(
                ...
                rx.text("Paste lease article text here to create draft clause sections. This creates text-backed sections and does not change the PDF page-split workflow.", size="2", color="#666"),
                rx.cond(
                    LeaseDocumentState.selected_source_document_id > 0,
                    rx.callout.root(rx.callout.text("Draft clauses will be saved under the currently selected source document."), color_scheme="blue", width="100%"),
