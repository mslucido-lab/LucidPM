...
class LeaseDocumentState(AppState):
    ...
    def parse_mode_help_text(self) -> str:
        if self.p_creation_mode == "Text Clause":
            return "Create a text-backed clause from this source document. Page numbers are references only, so multiple clauses may share the same page."
    ...
    def edit_section(self, section_id: int):
        ...
        rows = run_query(
            ...
            "StartPage, EndPage, SortOrder, IsReusable, IsActive, StoredFilePath, Content FROM LeaseDocumentSections WHERE LeaseDocumentSectionID = ?",
            (int(section_id),), db=self.db,
        )
        if not rows:
        ...
        self.p_creation_mode = "Text Clause" if str(r.get("Content") or "").strip() and (not stored_path or stored_path == str(self.selected_source_path or "")) else "PDF Page Split"
    ...
    def create_section(self):
        ...
        try:
            ...
            root = self.storage_root.strip() or DEFAULT_DOCUMENT_ROOT
            if is_text_clause:
                section_path = str(self.selected_source_path or "TEXT_ONLY").strip() or "TEXT_ONLY"
                rel = relative_to_root(section_path, root) if section_path != "TEXT_ONLY" else None
            else:
                output_name = (
    ...
    def parse_pasted_clauses(self):
        ...
        heading_re = re.compile(
            r"^\s*(?:[0-9]+(?:\.[0-9]+)*[\.)]?|[A-Z][\.)]|[a-z]\)|[IVXLCDM]+[\.)])\s+\S+",
            re.IGNORECASE,
        )
    ...
    def _save_text_clause_section(self, draft: DraftClauseRow) -> int:
        ...
        if self.selected_source_page_count and end > self.selected_source_page_count:
    ...
    def set_p_creation_mode(self, v: str): self.p_creation_mode = v
