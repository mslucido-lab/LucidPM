...
v2.7.11 — Adds LeaseStatus, suppresses valid prorated-start RentDueDay warning, and shows lease status badges.

v2.7.12 — Adds LeaseTerminationDate UI/save support for terminated leases.

v2.7.11 — Adds LeaseStatus support and suppresses valid prorated-start RentDueDay warnings.

v2.9.0 — Adds lease amendment form fields: ExecutionDate and ParentLeaseID.

v2.9.1 — Adds tenant header edit re-entry button, row-click lease editing, and Amends Lease alignment fix.

v2.8.3 — Clean rebuild from v2.7.15 adding tenant Addresses tab with CRUD and application-note import.

v2.7.13 — Stable tenant split-panel resizer and responsive tenant header.

v2.7.15 — List row click opens tenant edit-ready and removes intermediate Edit/View buttons.

v2.7.14 — Makes tenant header card responsive within dynamic split panel.

v2.7.10 — Adds email double-send guard and preserves original attachment filenames.

v2.7.9 — Full-page dynamic width tracks the resizable sidebar.

v2.7.5 — Adds native Windows file dialog attachment picker to tenant outbound email compose modal.

v2.7.4 — Adds contact picker to tenant outbound email compose modal.

v2.7.3 — Adds tenant outbound email compose modal, SMTP send, and
          Communications logging.

v2.7.2 — State management patch: clear screening form, si banners, and rent
          schedule banners on tenant switch in _load_tenant_detail().

v2.7.1 — Fixes References tab tenant-switch state reset; active reference edits are canceled when changing tenants.

v2.7.0 — Adds Tenant References tab and Emergency Contact flag.

v2.6.0 — Adds Screening UX polish: summary callout and factor breakdown UI.

v2.5.9b — Fixes TenantScreeningFactors FK column compatibility for Phase 1.5.
...
ADDRESS_LABEL_BASE = ["Personal", "Business", "Mailing", "Other"]
ADDRESS_LABEL_OPTIONS_DEFAULT = [ADDRESS_LABEL_NONE] + ADDRESS_LABEL_BASE
LEASE_PARENT_NONE_LABEL = "Base lease (no parent)"
...
def _fmt_short_date_no_pad(value) -> str:
    """Return M/D/YYYY without relying on platform-specific strftime flags."""
    if value is None:
        return ""
    ...
    try:
        parsed = datetime.datetime.fromisoformat(str(value)[:10]).date()
...
class TenantAddress(rx.Base):
    addr_id: int = 0
    tenant_id: int = 0
    label: str = ""
    display_label: str = ADDRESS_LABEL_NONE
    line1: str = ""
    line2: str = ""
    city: str = ""
    state: str = ""
...
class TenantReference(rx.Base):
    reference_id: int = 0
    reference_name: str = ""
    reference_phone: str = ""
    reference_email: str = ""
    reference_address: str = ""
    relationship_years: str = ""
    relationship: str = ""
    notes: str = ""
    date_contacted: str = ""
    outcome: str = "Not Yet Contacted"
    imported_from_tally: bool = False
    source_label: str = ""
...
class ScreeningRecord(rx.Base):
    ...
    calculated_score: str = ""
    suggested_decision: str = ""
    suggested_tier: str = ""
    notes: str = ""


class ScreeningFactor(rx.Base):
    factor_code: str = ""
    factor_label: str = ""
    points_earned: int = 0
    points_max: int = 0
...
class TenantState(AppState):
    ...
    f_is_emergency: bool = False
    form_error: str = ""
    form_success: str = ""

    # Addresses
    addresses: list[TenantAddress] = []
    addr_mode: str = "new"
    selected_addr_id: int = 0
    address_label_options: list[str] = ADDRESS_LABEL_OPTIONS_DEFAULT
    a_label: str = ADDRESS_LABEL_NONE
    a_line1: str = ""
    a_line2: str = ""
    a_city: str = ""
    a_state: str = ""
    a_zip: str = ""
    a_country: str = ""
    a_is_primary: bool = False
    a_notes: str = ""

    # References
    references: list[TenantReference] = []
    selected_reference_id: int = 0
    reference_mode: str = "new"
    show_reference_form: bool = False
    reference_form_error: str = ""
    reference_form_success: str = ""
    ref_name: str = ""
    ref_phone: str = ""
    ref_email: str = ""
    ref_address: str = ""
    ref_relationship: str = ""
    ref_relationship_years: str = ""
    ref_notes: str = ""
    ref_date_contacted: str = ""
    ref_outcome: str = "Not Yet Contacted"
    ...
    # Contact picker for compose email modal
    ...
    l_status: str = "Active"
    l_termination_date: str = ""
    l_start: str = ""
    l_end: str = ""
    lease_execution_date: str = ""
    lease_parent_lease_id: str = ""
    lease_parent_options: list[dict] = []
    l_rent: str = ""
    l_deposit: str = ""
    l_due_day: str = "1"
    l_next_due: str = ""
    ...
    screening_factor_records: list[ScreeningFactor] = []
    screening_latest_result: str = ""
    screening_latest_tier: str = ""
    screening_latest_deposit: str = ""
    screening_latest_score: str = ""
    screening_latest_decision: str = ""
    screening_latest_completed_date: str = ""
    ...
    @rx.var
    def lease_parent_label(self) -> str:
        for opt in self.lease_parent_options:
            if str(opt.get("value") or "") == str(self.lease_parent_lease_id or ""):
                return str(opt.get("label") or LEASE_PARENT_NONE_LABEL)
        return LEASE_PARENT_NONE_LABEL

    @rx.var
    def lease_parent_labels(self) -> list[str]:
        labels = [LEASE_PARENT_NONE_LABEL]
        current_id = str(int(self.selected_lease_id or 0)) if int(self.selected_lease_id or 0) > 0 else ""
        for opt in self.lease_parent_options:
            if str(opt.get("value") or "") and str(opt.get("value") or "") != current_id:
                labels.append(str(opt.get("label") or ""))
        return labels

    def set_lease_parent_from_label(self, label: str):
        if str(label or "") == LEASE_PARENT_NONE_LABEL:
            self.lease_parent_lease_id = ""
            return
        for opt in self.lease_parent_options:
            if str(opt.get("label") or "") == str(label or ""):
                self.lease_parent_lease_id = str(opt.get("value") or "")
                return
        self.lease_parent_lease_id = ""
    ...
    @rx.var
    def references_available_to_import(self) -> bool:
        notes = str(self.tenant_notes or "").lower()
        return "reference 1 name:" in notes or "trade reference 1:" in notes

    @rx.var
    def emergency_contact_available_to_import(self) -> bool:
        return "emergency contact name:" in str(self.tenant_notes or "").lower()

    @rx.var
    def address_available_to_import(self) -> bool:
        n = str(self.tenant_notes or "").lower()
        return ("address:" in n) or ("present address:" in n)

    @rx.var
    def address_editing_banner(self) -> str:
        return f"Editing address: {self.a_line1}" if self.a_line1 else "Editing address"

    @rx.var
    def reference_editing_banner(self) -> str:
        return f"Editing reference: {self.ref_name}" if self.ref_name else "Editing reference"
    ...
    def rent_due_day_warning(self) -> str:
        try:
            ...
            # Valid pattern: mid-month start with 1st-of-month due day.
            # This represents a prorated first month followed by standard monthly payments.
            if due_day == 1:
                return ""
    ...
    def _ensure_schema(self):
        """Create references table and emergency contact flag if missing."""
        try:
            run_exec(
                "IF OBJECT_ID('dbo.TenantReferences', 'U') IS NULL "
                "BEGIN "
                ...
                "ImportedFromTally BIT NOT NULL CONSTRAINT DF_TenantReferences_ImportedFromTally DEFAULT 0, "
                "CreatedDate DATETIME NOT NULL CONSTRAINT DF_TenantReferences_CreatedDate DEFAULT GETDATE(), "
                "CONSTRAINT FK_TenantReferences_Tenants FOREIGN KEY (TenantID) REFERENCES dbo.Tenants(TenantID)"
                "); "
                "IF OBJECT_ID('dbo.SchemaChangeLog', 'U') IS NOT NULL "
                "INSERT INTO dbo.SchemaChangeLog (ScriptName, AppliedOn, AppliedBy, Notes) "
                "VALUES ('tenant_references_create.sql', GETDATE(), SUSER_SNAME(), "
                "'Created TenantReferences table for tenant reference tracking'); "
                "END",
                db=self.db,
            )
            run_exec(
                ...
                "IF OBJECT_ID('dbo.SchemaChangeLog', 'U') IS NOT NULL "
                "INSERT INTO dbo.SchemaChangeLog (ScriptName, AppliedOn, AppliedBy, Notes) "
                "VALUES ('contacts_emergency_contact_flag.sql', GETDATE(), SUSER_SNAME(), "
                "'Added Contacts.IsEmergencyContact flag'); "
                "END",
                db=self.db,
            )
        except Exception as ex:
            print(f"[tenants] _ensure_schema failed: {ex}")
    ...
    def reload_on_db_change(self):
        ...
        self.screening_factor_records = []
        self.screening_latest_result = ""
        self.screening_latest_tier = ""
        self.screening_latest_deposit = ""
        self.screening_latest_score = ""
        self.screening_latest_decision = ""
        self.screening_latest_completed_date = ""
        self.show_screening_form = False
        self._reset_address_state(clear_rows=True)
        self._reset_reference_state(clear_rows=True)
        self._ensure_schema()
        self._load_property_options()
        self.load_tenant_list()
        self._load_lease_lookups()
        ...
        try:
            ...
            self.screening_records = [
                ScreeningRecord(
                    ...
                    calculated_score=str(r.get("CalculatedScore") or ""),
                    suggested_decision=str(r.get("SuggestedDecision") or ""),
                    suggested_tier=str(r.get("SuggestedTier") or ""),
                    notes=str(r.get("Notes") or ""),
                )
                for r in rows
            ]
            if self.screening_records:
                latest = self.screening_records[0]
                self.screening_latest_result = latest.overall_result
                self.screening_latest_tier = latest.risk_tier or latest.suggested_tier
                self.screening_latest_deposit = latest.deposit_recommended
                self.screening_latest_score = latest.calculated_score
                self.screening_latest_decision = latest.suggested_decision
                self.screening_latest_completed_date = latest.completed_date
                self.load_screening_factors(latest.screening_id)
            else:
                self.screening_latest_result = ""
                self.screening_latest_tier = ""
                self.screening_latest_deposit = ""
    ...
    def _screening_factor_fk_column(self) -> str:
        """Return the FK column name used by TenantScreeningFactors.

        Phase 1.5 specs use TenantScreeningID, but some test DBs may have
        been created with ScreeningID. Support both without requiring a
        destructive table rebuild.
        """
        try:
            rows = run_query(
                "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS "
                "WHERE TABLE_SCHEMA = 'dbo' AND TABLE_NAME = 'TenantScreeningFactors' "
                "AND COLUMN_NAME IN ('TenantScreeningID', 'ScreeningID')",
                (),
                db=self.db,
            )
            cols = {str(r.get('COLUMN_NAME') or r.get('column_name') or '') for r in rows}
            if 'TenantScreeningID' in cols:
                return 'TenantScreeningID'
            if 'ScreeningID' in cols:
                return 'ScreeningID'
        except Exception:
            pass
        return 'TenantScreeningID'
    ...
    def _screening_factor_label(self, code: str) -> str:
        labels = {
            "EVICTIONS": "Evictions",
    ...
    def _factor_model_from_dict(self, factor: dict) -> ScreeningFactor:
        code = str(factor.get("FactorCode") or "")
        earned = int(factor.get("PointsEarned") or 0)
        max_pts = int(factor.get("PointsMax") or 0)
        hard = bool(factor.get("HardFlag"))
        return ScreeningFactor(
            factor_code=code,
            factor_label=self._screening_factor_label(code),
            points_earned=earned,
            points_max=max_pts,
            score_text=f"{earned}/{max_pts}",
            hard_flag=hard,
            hard_flag_text="Yes" if hard else "No",
            notes=str(factor.get("Notes") or ""),
        )

    def load_screening_factors(self, screening_id: int):
        sid = int(screening_id or 0)
        if sid <= 0:
            self.screening_factor_records = []
            return
        try:
            factor_fk_col = self._screening_factor_fk_column()
            rows = run_query(
                "SELECT FactorCode, PointsEarned, PointsMax, HardFlag, Notes "
                f"FROM TenantScreeningFactors WHERE {factor_fk_col} = ? "
                "ORDER BY FactorID",
                (sid,),
                db=self.db,
            )
            self.screening_factor_records = [self._factor_model_from_dict(r) for r in rows]
        except Exception as ex:
            self.screening_factor_records = []
            self.screening_form_error = f"Could not load assessment factors: {ex}"
    ...
    def run_screening_assessment(self):
        ...
        self.screening_factor_records = [self._factor_model_from_dict(f) for f in factor_details]
    ...
    def select_screening_record(self, screening_id: int):
        ...
        try:
            ...
            self._pending_factor_details = []
            self.load_screening_factors(sid)
            self.show_screening_form = True
        except Exception as ex:
    ...
    def save_screening_record(self):
        ...
        try:
            ...
            if saved_screening_id > 0 and self.s_assessment_run and self._pending_factor_details:
                factor_fk_col = self._screening_factor_fk_column()
                run_exec(
                    f"DELETE FROM TenantScreeningFactors WHERE {factor_fk_col} = ?",
                    (saved_screening_id,),
                    db=self.db,
                )
                for factor in self._pending_factor_details:
                    run_exec(
                        ...
                        f"({factor_fk_col}, FactorCode, PointsEarned, PointsMax, HardFlag, Notes) "
                        "VALUES (?, ?, ?, ?, ?, ?)",
                            (
                                saved_screening_id,
    ...
    def load_contacts(self):
        rows = run_query(
            ...
            "WorkPhone, HomePhone, Email1, Email2, IsPrimary, "
            "ISNULL(IsEmergencyContact, 0) AS IsEmergencyContact, Salutation "
        ...
        self.contacts = [
            Contact(
                ...
                is_emergency=bool(r.get("IsEmergencyContact")),
    ...
    def select_contact(self, contact_id: int):
        ...
        if matches:
            ...
            self.f_is_emergency = c.is_emergency
    ...
    def save_contact(self):
        ...
        if not (self.f_first.strip() or self.f_last.strip()):
            ...
            return
        if self.f_is_emergency:
            params = [self.tenant_id]
            query = "SELECT TOP 1 ContactID FROM Contacts WHERE TenantID = ? AND IsEmergencyContact = 1"
            if self.contact_mode == "edit" and int(self.selected_contact_id or 0) > 0:
                query += " AND ContactID <> ?"
                params.append(int(self.selected_contact_id))
            existing = run_query(query, tuple(params), db=self.db)
            if existing:
                self.form_error = "Another contact is already marked as emergency contact. Only one emergency contact is allowed per tenant."
                return
        ...
        if self.contact_mode == "edit":
            run_exec(
                ...
                "ContactRole=?, WorkPhone=?, HomePhone=?, Email1=?, Email2=?, IsPrimary=?, IsEmergencyContact=? "
                "WHERE ContactID=?",
                (self.f_salutation.strip(), self.f_first.strip(), self.f_last.strip(),
                 ...
                 self.f_is_primary, self.f_is_emergency, self.selected_contact_id),
                db=self.db,
            )
            ...
            run_exec(
                ...
                (self.tenant_id, self.f_salutation.strip(), self.f_first.strip(),
                 ...
                 self.f_email1.strip(), self.f_email2.strip(), self.f_is_primary, self.f_is_emergency),
                db=self.db,
            )
    ...
    def set_f_is_emergency(self, v): self.f_is_emergency = v

    # ── Addresses ────────────────────────────────────────────────────────────

    def _reset_address_state(self, clear_rows: bool = False):
        if clear_rows:
            self.addresses = []
        self.addr_mode = "new"
        self.selected_addr_id = 0
        self.address_label_options = list(ADDRESS_LABEL_OPTIONS_DEFAULT)
        self.a_label = ADDRESS_LABEL_NONE
        self.a_line1 = ""
        self.a_line2 = ""
        self.a_city = ""
        self.a_state = ""
        self.a_zip = ""
        self.a_country = ""
        self.a_is_primary = False
        self.a_notes = ""
        self.form_error = ""
        self.form_success = ""

    def _format_address_city_state_zip(self, city: str, state: str, zip_code: str) -> str:
        city = str(city or "").strip()
        state = str(state or "").strip()
        zip_code = str(zip_code or "").strip()
        state_zip = " ".join(x for x in [state, zip_code] if x).strip()
        parts = []
        if city:
            parts.append(city)
        if state_zip:
    ...
    def _load_addresses(self):
        ...
        try:
            rows = run_query(
                "SELECT TenantAddressID, TenantID, AddressLabel, AddressLine1, AddressLine2, "
                "City, State, ZipCode, CountryOrRegion, IsPrimary, Notes "
                "FROM TenantAddresses WHERE TenantID = ? "
                "ORDER BY IsPrimary DESC, TenantAddressID ASC",
                (self.tenant_id,),
                db=self.db,
            )
            self.addresses = []
            for r in rows:
                raw_label = str(r.get("AddressLabel") or "").strip()
                self.addresses.append(
                    TenantAddress(
                        addr_id=int(r.get("TenantAddressID") or 0),
                        tenant_id=int(r.get("TenantID") or 0),
                        label=raw_label,
                        display_label=raw_label or ADDRESS_LABEL_NONE,
                        line1=str(r.get("AddressLine1") or ""),
                        line2=str(r.get("AddressLine2") or ""),
                        city=str(r.get("City") or ""),
                        state=str(r.get("State") or ""),
                        zip_code=str(r.get("ZipCode") or ""),
                        city_state_zip=self._format_address_city_state_zip(
                            r.get("City"), r.get("State"), r.get("ZipCode")
                        ),
                        country=str(r.get("CountryOrRegion") or ""),
                        is_primary=bool(r.get("IsPrimary")),
                        notes=str(r.get("Notes") or ""),
                    )
                )
            ...
            self.addresses = []
            self.form_error = f"Could not load addresses: {ex}"

    def _ensure_address_label_option(self, label: str):
        label = str(label or "").strip()
        opts = list(ADDRESS_LABEL_OPTIONS_DEFAULT)
        if label and label not in opts:
            opts.append(label)
        self.address_label_options = opts

    def new_address(self):
        self._reset_address_state(clear_rows=False)

    def select_address(self, addr_id: int):
        self.addr_mode = "edit"
        self.selected_addr_id = int(addr_id or 0)
        self.form_error = ""
        self.form_success = ""
        match = next((a for a in self.addresses if a.addr_id == self.selected_addr_id), None)
        if not match:
            self.form_error = "Address not found."
            return
        lbl = str(match.label or "").strip()
        self._ensure_address_label_option(lbl)
        self.a_label = lbl if lbl else ADDRESS_LABEL_NONE
        self.a_line1 = match.line1
        self.a_line2 = match.line2
        self.a_city = match.city
        self.a_state = match.state
        self.a_zip = match.zip_code
        self.a_country = match.country
        self.a_is_primary = match.is_primary
        self.a_notes = match.notes

    def save_address(self):
        self.form_error = ""
        self.form_success = ""
        if self.tenant_id <= 0:
            self.form_error = "Select a tenant first."
            return
        if not self.a_line1.strip():
            self.form_error = "Address Line 1 is required."
            return
        if not self.a_city.strip():
            self.form_error = "City is required."
            return

        label_to_save = None if self.a_label == ADDRESS_LABEL_NONE else self.a_label.strip()
        values = (
            label_to_save,
            self.a_line1.strip(),
            self.a_line2.strip() or None,
            self.a_city.strip(),
            self.a_state.strip() or None,
            self.a_zip.strip() or None,
            self.a_country.strip() or None,
            int(bool(self.a_is_primary)),
            self.a_notes.strip() or None,
        )
        try:
            if self.addr_mode == "edit" and int(self.selected_addr_id or 0) > 0:
                run_exec(
                    "UPDATE TenantAddresses SET "
                    "AddressLabel=?, AddressLine1=?, AddressLine2=?, City=?, State=?, "
                    "ZipCode=?, CountryOrRegion=?, IsPrimary=?, Notes=? "
                    "WHERE TenantAddressID=? AND TenantID=?",
                    values + (int(self.selected_addr_id), self.tenant_id),
                    db=self.db,
                )
                message = "Address updated."
            else:
                run_exec(
                    "INSERT INTO TenantAddresses "
                    "(TenantID, AddressLabel, AddressLine1, AddressLine2, City, State, ZipCode, CountryOrRegion, IsPrimary, Notes) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (self.tenant_id,) + values,
                    db=self.db,
                )
            ...
            self._load_addresses()
            self.new_address()
            self.form_success = message
        except Exception as ex:
            self.form_error = f"Could not save address: {ex}"

    def delete_address(self):
        if int(self.selected_addr_id or 0) <= 0:
            return
        try:
            run_exec(
                "DELETE FROM TenantAddresses WHERE TenantAddressID=? AND TenantID=?",
                (int(self.selected_addr_id), self.tenant_id),
                db=self.db,
            )
            self._load_addresses()
            self.new_address()
            self.form_success = "Address deleted."
        except Exception as ex:
            self.form_error = f"Could not delete address: {ex}"

    def _parse_city_state_zip_from_address_line(self, value: str) -> tuple[str, str, str]:
        value = str(value or "").strip()
        city = state = zip_code = ""
        if not value:
            return city, state, zip_code
        if "," in value:
            city_part, _, rest = value.partition(",")
    ...
    def import_address_from_notes(self):
        self.form_error = ""
        self.form_success = ""
        if self.tenant_id <= 0:
            self.form_error = "Select a tenant first."
        ...
        notes = self._notes_map()
        candidates = []

        personal_raw = self._note_value(notes, "address")
        if personal_raw:
            parts = [p.strip() for p in personal_raw.split("\n") if p.strip()]
            line1 = parts[0] if parts else ""
            city = state = zip_code = ""
            if len(parts) > 1:
                city, state, zip_code = self._parse_city_state_zip_from_address_line(parts[1])
            if line1:
                candidates.append(("Personal", line1, city, state, zip_code))

        business_line1 = self._note_value(notes, "present address")
        if business_line1:
            candidates.append((
                "Business",
                business_line1,
                self._note_value(notes, "city"),
                self._note_value(notes, "state"),
                self._note_value(notes, "zip", "zip code"),
            ))

        if not candidates:
            self.form_error = "No address found in application notes."
            return

        existing = {a.line1.strip().lower() for a in self.addresses if a.line1.strip()}
        assigned_primary = len(self.addresses) > 0
        imported = 0
        skipped = 0

        try:
            for label, line1, city, state, zip_code in candidates:
                key = line1.strip().lower()
                if key in existing:
                    skipped += 1
                    continue
                make_primary = 0
            ...
            if imported and skipped:
                self.form_success = f"Imported {imported}; {skipped} already on file."
            elif imported:
                self.form_success = f"Imported {imported} address(es) from application."
            else:
                self.form_error = "All application addresses are already on file."
        except Exception as ex:
            self.form_error = f"Could not import address: {ex}"

    def set_a_label(self, v):
        self.a_label = v
        self._ensure_address_label_option(v)

    def set_a_line1(self, v): self.a_line1 = v
    def set_a_line2(self, v): self.a_line2 = v
    def set_a_city(self, v): self.a_city = v
    def set_a_state(self, v): self.a_state = v
    def set_a_zip(self, v): self.a_zip = v
    def set_a_country(self, v): self.a_country = v
    def set_a_is_primary(self, v): self.a_is_primary = v
    def set_a_notes(self, v): self.a_notes = v

    # ── References ───────────────────────────────────────────────────────────

    def _fmt_ref_date(self, v) -> str:
        if v is None:
            return ""
        if hasattr(v, "strftime"):
            return v.strftime("%m/%d/%Y")
    ...
    def _date_input_or_none(self, value: str):
        value = str(value or "").strip()
        if not value:
            return None
        try:
    ...
    def _notes_map(self) -> dict:
        return _parse_notes_dict(self.tenant_notes)

    def _note_value(self, notes: dict, *labels: str) -> str:
        for label in labels:
            val = str(notes.get(str(label).lower(), "") or "").strip()
            if val:
                return val
        return ""
    ...
    def _reset_reference_state(self, clear_rows: bool = True):
        """Reset all References tab tenant-scoped state before switching tenants."""
        if clear_rows:
            self.references = []
        self.selected_reference_id = 0
        self.reference_mode = "new"
        self.show_reference_form = False
        self.reference_form_error = ""
        self.reference_form_success = ""
        self.ref_name = ""
        self.ref_phone = ""
        self.ref_email = ""
        self.ref_address = ""
        self.ref_relationship = ""
        self.ref_relationship_years = ""
        self.ref_notes = ""
        self.ref_date_contacted = ""
        self.ref_outcome = "Not Yet Contacted"
    ...
    def load_references(self):
        ...
        try:
            rows = run_query(
                "SELECT ReferenceID, ReferenceName, ReferencePhone, ReferenceEmail, ReferenceAddress, "
                "RelationshipYears, Relationship, Notes, DateContacted, Outcome, ImportedFromTally "
                "FROM TenantReferences WHERE TenantID = ? ORDER BY CreatedDate DESC, ReferenceID DESC",
                (self.tenant_id,), db=self.db,
            )
            self.references = [
                TenantReference(
                    reference_id=int(r.get("ReferenceID") or 0),
                    reference_name=str(r.get("ReferenceName") or ""),
                    reference_phone=str(r.get("ReferencePhone") or ""),
                    reference_email=str(r.get("ReferenceEmail") or ""),
                    reference_address=str(r.get("ReferenceAddress") or ""),
                    relationship_years=str(r.get("RelationshipYears") or ""),
                    relationship=str(r.get("Relationship") or ""),
                    notes=str(r.get("Notes") or ""),
                    date_contacted=self._fmt_ref_date(r.get("DateContacted")),
                    outcome=str(r.get("Outcome") or "Not Yet Contacted"),
                    imported_from_tally=bool(r.get("ImportedFromTally")),
                    source_label="Tally" if bool(r.get("ImportedFromTally")) else "",
                )
                for r in rows
            ]
        except Exception as ex:
            self.references = []
            self.reference_form_error = f"Could not load references: {ex}"

    def new_reference(self):
        self.selected_reference_id = 0
        self.reference_mode = "new"
        self.show_reference_form = True
        self.reference_form_error = ""
        self.reference_form_success = ""
        self.ref_name = ""
        self.ref_phone = ""
    ...
    def cancel_reference_form(self):
        self._reset_reference_state(clear_rows=False)

    def select_reference(self, reference_id: int):
        self.reference_form_error = ""
        self.reference_form_success = ""
        rid = int(reference_id or 0)
        matches = [r for r in self.references if r.reference_id == rid]
        if not matches:
            self.reference_form_error = "Reference not found."
            return
        r = matches[0]
        self.selected_reference_id = rid
        self.reference_mode = "edit"
        self.show_reference_form = True
        self.ref_name = r.reference_name
        self.ref_phone = r.reference_phone
        self.ref_email = r.reference_email
        self.ref_address = r.reference_address
        self.ref_relationship = r.relationship
        self.ref_relationship_years = r.relationship_years
        self.ref_notes = r.notes
        self.ref_outcome = r.outcome or "Not Yet Contacted"
        if r.date_contacted:
            try:
                self.ref_date_contacted = datetime.datetime.strptime(r.date_contacted, "%m/%d/%Y").date().strftime("%Y-%m-%d")
            except Exception:
                self.ref_date_contacted = ""
        else:
            self.ref_date_contacted = ""
    ...
    def save_reference(self):
        ...
        self.reference_form_success = ""
        if self.tenant_id <= 0:
            self.reference_form_error = "Select a tenant first."
            return
        if not str(self.ref_name or "").strip():
            self.reference_form_error = "Reference name is required."
            return
        date_contacted = self._date_input_or_none(self.ref_date_contacted)
        values = (
            str(self.ref_name or "").strip() or None,
            str(self.ref_phone or "").strip() or None,
            str(self.ref_email or "").strip() or None,
            str(self.ref_address or "").strip() or None,
            str(self.ref_relationship_years or "").strip() or None,
            str(self.ref_relationship or "").strip() or None,
            str(self.ref_notes or "").strip() or None,
            date_contacted,
            str(self.ref_outcome or "Not Yet Contacted").strip() or "Not Yet Contacted",
        )
        try:
            if self.reference_mode == "edit" and int(self.selected_reference_id or 0) > 0:
                run_exec(
                    "UPDATE TenantReferences SET ReferenceName=?, ReferencePhone=?, ReferenceEmail=?, "
                    "ReferenceAddress=?, RelationshipYears=?, Relationship=?, Notes=?, DateContacted=?, Outcome=? "
                    "WHERE ReferenceID=? AND TenantID=?",
                    values + (int(self.selected_reference_id), self.tenant_id),
                    db=self.db,
                )
                self.reference_form_success = "Reference saved."
            else:
                run_exec(
                    "INSERT INTO TenantReferences (TenantID, ReferenceName, ReferencePhone, ReferenceEmail, "
                    "ReferenceAddress, RelationshipYears, Relationship, Notes, DateContacted, Outcome, ImportedFromTally) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)",
                    (self.tenant_id,) + values,
                    db=self.db,
                )
                self.reference_form_success = "Reference created."
            self.show_reference_form = False
            self.load_references()
        except Exception as ex:
            self.reference_form_error = f"Could not save reference: {ex}"

    def delete_reference(self):
        rid = int(self.selected_reference_id or 0)
        if rid <= 0:
            self.reference_form_error = "Select a reference to delete."
            return
        try:
            run_exec("DELETE FROM TenantReferences WHERE ReferenceID=? AND TenantID=?", (rid, self.tenant_id), db=self.db)
            self.reference_form_success = "Reference deleted."
            self.show_reference_form = False
            self.selected_reference_id = 0
            self.load_references()
        except Exception as ex:
            self.reference_form_error = f"Could not delete reference: {ex}"

    def import_references_from_notes(self):
        self.reference_form_error = ""
        self.reference_form_success = ""
        if self.tenant_id <= 0:
            self.reference_form_error = "Select a tenant first."
        ...
        try:
            ...
            imported = 0
            for i in [1, 2, 3]:
                name = self._note_value(notes, f"Reference {i} name", f"Trade reference {i}")
                if not name:
                    continue
                existing = run_query(
                    "SELECT TOP 1 ReferenceID FROM TenantReferences "
                    "WHERE TenantID=? AND LOWER(LTRIM(RTRIM(ISNULL(ReferenceName,'')))) = LOWER(?)",
                    (self.tenant_id, name.strip()), db=self.db,
                )
                if existing:
                    continue
                years = self._note_value(notes, f"Reference {i} years doing business")
                if i == 1 and not years:
                    years = self._note_value(notes, "Tally - Years Doing Business")
                run_exec(
                    "INSERT INTO TenantReferences (TenantID, ReferenceName, ReferencePhone, ReferenceEmail, "
                    "ReferenceAddress, RelationshipYears, Relationship, Notes, DateContacted, Outcome, ImportedFromTally) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, NULL, NULL, ?, 1)",
                    (
                        self.tenant_id,
                        name.strip(),
                        self._note_value(notes, f"Reference {i} phone") or None,
                        self._note_value(notes, f"Reference {i} email") or None,
                        self._note_value(notes, f"Tally - Reference Address {i}") or None,
                        years or None,
                        self._note_value(notes, f"Reference {i} relationship") or None,
                        "Not Yet Contacted",
                    ),
                    db=self.db,
                )
                imported += 1
            self.load_references()
            self.reference_form_success = f"{imported} reference(s) imported." if imported else "No new references found."
        except Exception as ex:
            self.reference_form_error = f"Could not import references: {ex}"

    def import_emergency_contact_from_notes(self):
        self.form_error = ""
        self.form_success = ""
        if self.tenant_id <= 0:
            self.form_error = "Select a tenant first."
        ...
        try:
            notes = self._notes_map()
            name = self._note_value(notes, "Emergency contact name")
            phone = self._note_value(notes, "Emergency contact phone")
            email = self._note_value(notes, "Emergency contact email")
            if not (name or phone or email):
                self.form_error = "No emergency contact found in tenant notes."
                return

            existing_emergency = run_query(
                "SELECT TOP 1 ContactID FROM Contacts WHERE TenantID=? AND IsEmergencyContact=1",
                (self.tenant_id,), db=self.db,
            )
            ...
            rows = run_query(
                "SELECT ContactID, WorkPhone, HomePhone, Email1, Email2 FROM Contacts WHERE TenantID=?",
                (self.tenant_id,), db=self.db,
            )
            phone_digits = self._normalize_digits(phone)
            email_norm = str(email or "").strip().lower()
            match_id = 0
            for r in rows:
                if phone_digits and (
                    self._normalize_digits(r.get("WorkPhone")) == phone_digits
                    or self._normalize_digits(r.get("HomePhone")) == phone_digits
                ):
                    match_id = int(r.get("ContactID") or 0)
                    break
                if email_norm and (
                    str(r.get("Email1") or "").strip().lower() == email_norm
                    or str(r.get("Email2") or "").strip().lower() == email_norm
                ):
                    match_id = int(r.get("ContactID") or 0)
                    break
            ...
            if match_id > 0:
                run_exec(
                    ...
                    (match_id, self.tenant_id), db=self.db,
                )
                self.form_success = "Emergency contact updated."
            else:
                parts = str(name or "").strip().split(None, 1)
                first = parts[0] if parts else ""
                last = parts[1] if len(parts) > 1 else ""
                run_exec(
                    "INSERT INTO Contacts (TenantID, FirstName, LastName, ContactRole, WorkPhone, Email1, IsPrimary, IsEmergencyContact) "
                    "VALUES (?, ?, ?, ?, ?, ?, 0, 1)",
                    (self.tenant_id, first, last, "", phone or None, email or None),
                    db=self.db,
                )
                self.form_success = "Emergency contact created."
            self.load_contacts()
            self.new_contact()
        except Exception as ex:
            self.form_error = f"Could not import emergency contact: {ex}"

    def set_ref_name(self, v): self.ref_name = v
    def set_ref_phone(self, v): self.ref_phone = v
    def set_ref_email(self, v): self.ref_email = v
    def set_ref_address(self, v): self.ref_address = v
    def set_ref_relationship(self, v): self.ref_relationship = v
    def set_ref_relationship_years(self, v): self.ref_relationship_years = v
    def set_ref_notes(self, v): self.ref_notes = v
    def set_ref_date_contacted(self, v): self.ref_date_contacted = v
    def set_ref_outcome(self, v): self.ref_outcome = v
    ...
    def open_compose_email(self):
        """Load contacts with emails, pre-populate To from primary, open modal."""
        self.compose_error = ""
        self.compose_success = ""
        ...
        if not self.tenant_id or int(self.tenant_id) <= 0:
    ...
    def set_compose_contact(self, label: str):
        """Update To field when contact picker selection changes."""
    ...
    def send_compose_email(self):
        ...
        if not self.compose_body.strip():
            ...
            return
        if not self.tenant_id or int(self.tenant_id) <= 0:
            self.compose_error = "No tenant selected."
            return
        ...
        try:
            ...
            prop_id = (
                int(prop_rows[0]["PropertyID"])
                if prop_rows and prop_rows[0].get("PropertyID")
                else None
            )
            ...
            run_exec(
                ...
                INSERT INTO dbo.Communications
                    (TenantID, CommDate, Method, Subject, Body, FromTo, Outcome, PropertyID)
                VALUES (?, SYSDATETIME(), 'Email', ?, ?, ?, 'Sent', ?)
                """,
                (
                    int(self.tenant_id),
                    self.compose_subject.strip(),
                    self.compose_body.strip(),
                    self.compose_to.strip(),
                    prop_id,
                ),
                db=self.db,
            )
        except Exception as ex:
            self.compose_success = f"Email sent. Note: comm log entry failed: {ex}"
            self.compose_sending = False
            self.compose_email_open = False
            return

        self.compose_success = "Email sent and logged."
        self.compose_sending = False
        self.compose_email_open = False
        self.load_comms()
    ...
    def _load_lease_parent_options(self):
        if self.tenant_id <= 0:
            self.lease_parent_options = []
            return
        rows = run_query(
            "SELECT l.LeaseID, l.LeaseStart, l.LeaseEnd, p.PropertyName "
            "FROM Leases l "
        ...
        self.lease_parent_options = [
            {
                "label": (
                    f"{str(r.get('PropertyName') or '')} · "
                    f"{_fmt_short_date_no_pad(r.get('LeaseStart')) or '?'} - "
                    f"{_fmt_short_date_no_pad(r.get('LeaseEnd')) or 'ongoing'}"
                ),
                "value": str(r.get("LeaseID") or ""),
            }
            for r in rows
        ]
    ...
    def load_leases(self):
        rows = run_query(
            ...
            "l.LeaseStart, l.LeaseEnd, l.ExecutionDate, l.ParentLeaseID, "
            "ISNULL(l.LeaseStatus, 'Active') AS LeaseStatus, l.LeaseTerminationDate, l.RentAmount "
            "FROM Leases l "
        ...
        def fmt_dt(v) -> str:
            ...
            if hasattr(v, "strftime"):
                return v.strftime("%m/%d/%Y")
            return str(v)
        self._load_lease_parent_options()
        self.leases = [
            LeaseSummary(
                ...
                l_status=str(r.get("LeaseStatus") or "Active"),
                lease_termination_date=fmt_dt(r.get("LeaseTerminationDate")),
    ...
    def select_lease(self, lease_id: int):
        ...
        rows = run_query(
            ...
            "l.LeaseStart, l.LeaseEnd, l.ExecutionDate, l.ParentLeaseID, "
            "ISNULL(l.LeaseStatus, 'Active') AS LeaseStatus, l.LeaseTerminationDate, l.RentAmount, l.DepositAmount, "
        ...
        self.l_status = str(r.get("LeaseStatus") or "Active")
        raw_start = r.get("LeaseStart")
        raw_end   = r.get("LeaseEnd")
        raw_exec  = r.get("ExecutionDate")
        raw_term  = r.get("LeaseTerminationDate")
        ...
        self.lease_execution_date = raw_exec.strftime("%Y-%m-%d") if raw_exec else ""
        self.lease_parent_lease_id = str(int(r.get("ParentLeaseID"))) if r.get("ParentLeaseID") else ""
        self.l_termination_date = raw_term.strftime("%Y-%m-%d") if raw_term else ""
    ...
    def new_lease(self):
        ...
        self.l_status = "Active"
        self.l_termination_date = ""
        self.l_start = self.l_end = self.l_next_due = ""
        self.lease_execution_date = ""
        self.lease_parent_lease_id = ""
    ...
    def save_lease(self):
        ...
        try:
            ...
            lease_end = None
        try:
            execution_date = datetime.datetime.strptime(self.lease_execution_date, "%Y-%m-%d").date() if self.lease_execution_date else None
        except ValueError:
            execution_date = None
        try:
            parent_lease_id = int(self.lease_parent_lease_id) if str(self.lease_parent_lease_id or "").strip() else None
        except ValueError:
            parent_lease_id = None
        try:
            termination_date = datetime.datetime.strptime(self.l_termination_date, "%Y-%m-%d").date() if self.l_termination_date else None
        except ValueError:
        ...
        if self.lease_mode == "edit":
            run_exec(
                ...
                "LeaseStart=?, LeaseEnd=?, ExecutionDate=?, ParentLeaseID=?, "
                "LeaseStatus=?, LeaseTerminationDate=?, RentAmount=?, DepositAmount=?, RentDueDay=?, "
                "NextDueDate=?, ShowAnniversaries=? WHERE LeaseID=?",
                (prop_id, suite_id, lease_type_id, lease_term_type_id,
                 lease_start, lease_end, execution_date, parent_lease_id,
                 self.l_status, termination_date, rent, deposit, due_day,
            ...
            run_exec(
                ...
                "LeaseStart, LeaseEnd, ExecutionDate, ParentLeaseID, LeaseStatus, LeaseTerminationDate, "
                "RentAmount, DepositAmount, RentDueDay, NextDueDate, ShowAnniversaries) "
                "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (self.tenant_id, prop_id, suite_id, lease_type_id, lease_term_type_id,
                 lease_start, lease_end, execution_date, parent_lease_id, self.l_status, termination_date,
                 rent, deposit, due_day, next_due, self.l_show_anniversaries),
                db=self.db,
            )
    ...
    def set_l_status(self, v): self.l_status = v
    def set_l_termination_date(self, v): self.l_termination_date = v
    def set_l_start(self, v): self.l_start = v
    def set_l_end(self, v): self.l_end = v
    def set_lease_execution_date(self, v): self.lease_execution_date = v
...
def tenant_list_row(t: TenantSummary) -> rx.Component:
    return rx.table.row(
        ...
        style=rx.cond(
            ...
            rx.cond(
                t.status == "Applicant",
                {"background": "#FFF8E1", "border_left": "3px solid #F9A825", "cursor": "pointer"},
...
def screening_summary_callout() -> rx.Component:
    summary_box = rx.box(
        rx.hstack(
                rx.text("Screening", size="1", weight="bold", color="#5D4037"),
                rx.badge(
                    TenantState.screening_latest_result,
                    color_scheme=rx.cond(
                        TenantState.screening_latest_result == "Fail",
                        "red",
                        rx.cond(TenantState.screening_latest_result == "Conditional", "orange", "green"),
                    ),
                    variant="soft",
                    size="1",
                ),
                rx.cond(
                    TenantState.screening_latest_tier != "",
                    rx.badge(TenantState.screening_latest_tier, color_scheme="blue", variant="soft", size="1"),
                    rx.fragment(),
                ),
                rx.cond(
                    TenantState.screening_latest_score != "",
                    rx.text("Score: " + TenantState.screening_latest_score, size="1", color="#5D4037"),
                    rx.fragment(),
                ),
                rx.cond(
                    TenantState.screening_latest_deposit != "",
                    rx.text("Deposit: " + TenantState.screening_latest_deposit, size="1", color="#5D4037"),
                    rx.fragment(),
                ),
                spacing="2",
                align="center",
    ...
    return rx.cond(
        ...
        rx.cond(TenantState.screening_latest_result != "", summary_box, rx.fragment()),
        rx.fragment(),
    )


def tenant_summary_header_card() -> rx.Component:
    return rx.box(
        rx.hstack(
            rx.box(
            ...
            rx.vstack(
                ...
                screening_summary_callout(),
                spacing="1", align_items="start", width="100%",
                style={"min_width": "240px", "flex": "1 1 320px"},
            ),
            rx.spacer(),
            rx.hstack(
                rx.cond(
                ...
                rx.cond(
                    TenantState.tenant_edit_mode | (TenantState.tenant_id == 0),
                    rx.fragment(),
                    rx.button(
                        "✏ Edit",
                ...
                rx.cond(
                    ...
                    rx.button(
                        "Send Email",
                        on_click=TenantState.open_compose_email,
                ...
                justify="end",
                wrap="wrap",
                style={"flex": "0 1 auto", "min_width": "160px"},
            ),
            align="start", spacing="4", width="100%", wrap="wrap",
            style={"min_width": "0"},
        ),
        style={
            "background": "white", "border": "1px solid #dde3f0",
...
def tenant_header_card() -> rx.Component:
    return rx.cond(
        TenantState.tenant_is_new,
        tenant_edit_form(),
        rx.vstack(
            tenant_summary_header_card(),
            rx.cond(
    ...
    return rx.table.row(
        rx.table.cell(
            rx.hstack(
                ...
                rx.cond(c.is_emergency, pill("Emergency", "#FFF3E0", "#E65100"), rx.fragment()),
...
def contacts_tab() -> rx.Component:
    return rx.vstack(
        ...
        rx.hstack(
            ...
            rx.button("Import emergency contact", on_click=TenantState.import_emergency_contact_from_notes,
                      variant="soft", color_scheme="orange", size="2",
                      disabled=TenantState.emergency_contact_available_to_import == False),
            spacing="2",
        ),
        rx.divider(),
        rx.cond(
        ...
        rx.hstack(
            rx.switch(checked=TenantState.f_is_emergency, on_change=TenantState.set_f_is_emergency),
            rx.vstack(
                rx.text("Emergency contact", size="2", weight="bold"),
                rx.text("Only one emergency contact is allowed per tenant", size="1", color="#666"),
                spacing="0",
            ),
            align="center", spacing="3",
        ),
...
def address_row(a: TenantAddress) -> rx.Component:
    return rx.table.row(
        rx.table.cell(
            rx.hstack(
                rx.text(a.display_label, size="2", weight="bold"),
                rx.cond(
                    a.is_primary,
                    rx.badge("Primary", color_scheme="blue", variant="soft", size="1"),
                    rx.fragment(),
                ),
                spacing="2", align="center",
            )
        ),
        rx.table.cell(rx.text(a.line1, size="2")),
        rx.table.cell(rx.text(a.city_state_zip, size="2", color="#555")),
        rx.table.cell(
            rx.button("Edit", size="1", variant="soft", color_scheme="blue",
                      on_click=TenantState.select_address(a.addr_id))
        ),
        style=rx.cond(
            TenantState.selected_addr_id == a.addr_id,
            {"background": "#f0f4ff"}, {"background": "white"},
        ),
    )


def addresses_tab() -> rx.Component:
    return rx.vstack(
        rx.cond(
            TenantState.addresses.length() > 0,
            rx.table.root(
                rx.table.header(
                    rx.table.row(
                        rx.table.column_header_cell("Label"),
                        rx.table.column_header_cell("Line 1"),
                        rx.table.column_header_cell("City / State / Zip"),
                        rx.table.column_header_cell(""),
                    )
                ),
                rx.table.body(rx.foreach(TenantState.addresses, address_row)),
                width="100%", variant="surface",
            ),
            rx.text("No addresses yet.", color="#888", size="2"),
        ),
        rx.hstack(
            rx.button("+ New address", on_click=TenantState.new_address,
                      variant="outline", color_scheme="blue", size="2"),
            rx.button("Import address from application", on_click=TenantState.import_address_from_notes,
                      variant="soft", color_scheme="orange", size="2",
                      disabled=TenantState.address_available_to_import == False),
            spacing="2",
        ),
        rx.divider(),
        rx.cond(
            TenantState.addr_mode == "edit",
            edit_banner(TenantState.address_editing_banner),
            rx.text("New address", size="3", weight="bold", color=BRAND_DARK),
        ),
        rx.grid(
            _form_field("Label", rx.select(TenantState.address_label_options,
                value=TenantState.a_label, on_change=TenantState.set_a_label, size="2", width="100%")),
            _form_field("Address Line 1 *", rx.input(value=TenantState.a_line1,
                on_change=TenantState.set_a_line1, placeholder="Street address", size="2", width="100%")),
            _form_field("Address Line 2", rx.input(value=TenantState.a_line2,
                on_change=TenantState.set_a_line2, placeholder="Suite, unit, etc.", size="2", width="100%")),
            _form_field("City *", rx.input(value=TenantState.a_city,
                on_change=TenantState.set_a_city, placeholder="City", size="2", width="100%")),
            _form_field("State", rx.input(value=TenantState.a_state,
                on_change=TenantState.set_a_state, placeholder="TX", size="2", width="100%")),
            _form_field("Zip Code", rx.input(value=TenantState.a_zip,
                on_change=TenantState.set_a_zip, placeholder="75006", size="2", width="100%")),
            _form_field("Country / Region", rx.input(value=TenantState.a_country,
                on_change=TenantState.set_a_country, placeholder="USA", size="2", width="100%")),
            columns="3", spacing="4", width="100%",
        ),
        rx.hstack(
            rx.switch(checked=TenantState.a_is_primary, on_change=TenantState.set_a_is_primary),
            rx.vstack(
                rx.text("Primary address", size="2", weight="bold"),
                rx.text("Used as default mailing address", size="1", color="#666"),
                spacing="0",
            ),
            align="center", spacing="3",
        ),
        _form_field("Notes", rx.text_area(value=TenantState.a_notes,
            on_change=TenantState.set_a_notes, placeholder="Optional notes", width="100%", rows="3")),
        rx.cond(TenantState.form_error != "",
                rx.callout(TenantState.form_error, color="red", variant="soft"), rx.fragment()),
        rx.cond(TenantState.form_success != "",
                rx.callout(TenantState.form_success, color="green", variant="soft"), rx.fragment()),
        rx.hstack(
            rx.button(
                rx.cond(TenantState.addr_mode == "edit", "Save address", "Create address"),
                on_click=TenantState.save_address, color_scheme="blue", size="2",
            ),
            rx.cond(
                TenantState.addr_mode == "edit",
                rx.button("Delete address", on_click=TenantState.delete_address,
                          color_scheme="red", variant="outline", size="2"),
                rx.fragment(),
            ),
            rx.button("Cancel", on_click=TenantState.new_address, variant="ghost", size="2"),
            spacing="3",
        ),
        spacing="4", width="100%", align_items="start",
    )
...
def lease_row(l: LeaseSummary) -> rx.Component:
    return rx.table.row(
        ...
        rx.table.cell(
            rx.badge(
                rx.cond(l.l_status != "", l.l_status, "Active"),
                color_scheme=rx.cond(
                    l.l_status == "Active",
                    "green",
                    rx.cond(
                        l.l_status == "Terminated",
                        "red",
                        rx.cond(l.l_status == "Expired", "gray", "blue"),
                    ),
                ),
                variant="soft",
                size="1",
        ...
        rx.table.cell(rx.text(l.lease_termination_date, size="2", color="#555")),
...
def leases_tab() -> rx.Component:
    return rx.vstack(
            ...
            rx.table.root(
                rx.table.header(
                    rx.table.row(
                        ...
                        rx.table.column_header_cell("Terminated"),
        ...
        # Row 1b: lease status and termination date
        rx.grid(
            rx.vstack(
                rx.text("Lease Status", size="2", color="#555"),
                rx.select(
                    ["Active", "Terminated", "Expired", "Month-to-Month"],
                    value=TenantState.l_status,
                    on_change=TenantState.set_l_status,
                    size="2",
                    width="100%",
                ),
                spacing="1", width="100%",
            ...
            rx.vstack(
                rx.text("Termination date", size="2", color="#555"),
                rx.input(
                    value=TenantState.l_termination_date,
                    on_change=TenantState.set_l_termination_date,
                    type="date",
                    size="2",
                    width="100%",
                ),
                rx.text("Used by Proforma when Lease Status is Terminated.", size="1", color="#888"),
                spacing="1", width="100%",
            ),
            columns="4", spacing="4", width="100%",
        ),

        # Row 2: start, end, execution date, parent lease
        rx.grid(
            rx.vstack(
            ...
            rx.vstack(
                rx.text("Execution Date", size="1", color="#666"),
                rx.input(value=TenantState.lease_execution_date, on_change=TenantState.set_lease_execution_date,
                         type="date", size="2", width="100%"),
                spacing="1", width="100%",
            ),
            rx.vstack(
                rx.text("Amends Lease", size="1", color="#666"),
                rx.select(
                    TenantState.lease_parent_labels,
                    value=TenantState.lease_parent_label,
                    on_change=TenantState.set_lease_parent_from_label,
                    size="2",
                    width="100%",
                    placeholder="Base lease (no parent)",
                ),
...
def reference_outcome_badge(outcome: rx.Var) -> rx.Component:
    return rx.badge(
        outcome,
        color_scheme=rx.cond(
            outcome == "Positive",
            ...
            rx.cond(
                ...
                rx.cond(
                    outcome == "No Response",
                    "amber",
                    rx.cond(outcome == "Neutral", "gray", "gray"),
                ),
            ),
        ),
        variant="soft",
...
def reference_row(r: TenantReference) -> rx.Component:
    return rx.table.row(
        rx.table.cell(rx.text(r.reference_name, size="2", weight="bold")),
        rx.table.cell(rx.text(r.reference_phone, size="2", color="#555")),
        rx.table.cell(rx.text(r.reference_address, size="2", color="#555")),
        rx.table.cell(
            rx.vstack(
                rx.text(r.relationship, size="2", color="#555"),
                rx.text(r.relationship_years, size="1", color="#777"),
                spacing="0",
            )
        ),
        rx.table.cell(rx.text(r.date_contacted, size="2", color="#555")),
        rx.table.cell(reference_outcome_badge(r.outcome)),
        rx.table.cell(
            rx.cond(
                r.imported_from_tally,
                rx.badge("Tally", color_scheme="blue", variant="soft", size="1"),
                rx.fragment(),
            )
        ),
        rx.table.cell(
            rx.button("Edit", size="1", variant="soft", color_scheme="blue",
                      on_click=TenantState.select_reference(r.reference_id))
        ),
        style=rx.cond(
            TenantState.selected_reference_id == r.reference_id,
            {"background": "#f0f4ff"},
            {"background": "white"},
        ),
    )
...
def references_form() -> rx.Component:
    return rx.vstack(
        rx.cond(
            TenantState.reference_mode == "edit",
            edit_banner(TenantState.reference_editing_banner),
            rx.text("Add reference", size="3", weight="bold", color=BRAND_DARK),
        ),
        rx.grid(
            _form_field("Reference name *", rx.input(value=TenantState.ref_name, on_change=TenantState.set_ref_name, size="2", width="100%")),
            _form_field("Phone", rx.input(value=TenantState.ref_phone, on_change=TenantState.set_ref_phone, size="2", width="100%")),
            _form_field("Email", rx.input(value=TenantState.ref_email, on_change=TenantState.set_ref_email, size="2", width="100%")),
            columns="3", spacing="4", width="100%",
        ),
        _form_field("Address", rx.input(value=TenantState.ref_address, on_change=TenantState.set_ref_address, size="2", width="100%")),
        rx.grid(
            _form_field("Relationship", rx.input(value=TenantState.ref_relationship, on_change=TenantState.set_ref_relationship, placeholder="Business associate", size="2", width="100%")),
            _form_field("Relationship years", rx.input(value=TenantState.ref_relationship_years, on_change=TenantState.set_ref_relationship_years, placeholder="1-3 years", size="2", width="100%")),
            _form_field("Date contacted", rx.input(value=TenantState.ref_date_contacted, on_change=TenantState.set_ref_date_contacted, type="date", size="2", width="100%")),
            _form_field("Outcome", rx.select(["Positive", "Neutral", "Negative", "No Response", "Not Yet Contacted"], value=TenantState.ref_outcome, on_change=TenantState.set_ref_outcome, size="2", width="100%")),
            columns="4", spacing="4", width="100%",
        ),
        _form_field("Notes", rx.text_area(value=TenantState.ref_notes, on_change=TenantState.set_ref_notes, rows="3", width="100%")),
        rx.cond(TenantState.reference_form_error != "", rx.callout(TenantState.reference_form_error, color_scheme="red", variant="soft"), rx.fragment()),
        rx.cond(TenantState.reference_form_success != "", rx.callout(TenantState.reference_form_success, color_scheme="green", variant="soft"), rx.fragment()),
        rx.hstack(
            rx.button(rx.cond(TenantState.reference_mode == "edit", "Save reference", "Create reference"), on_click=TenantState.save_reference, color_scheme="blue", size="2"),
            rx.cond(
                TenantState.reference_mode == "edit",
                rx.button("Delete reference", on_click=TenantState.delete_reference, color_scheme="red", variant="outline", size="2"),
                rx.fragment(),
            ),
            rx.button("Cancel", on_click=TenantState.cancel_reference_form, variant="ghost", color_scheme="gray", size="2"),
            spacing="3",
        ),
        spacing="4", width="100%", align_items="start",
    )
...
def references_tab() -> rx.Component:
    return rx.vstack(
        rx.cond(TenantState.reference_form_success != "", rx.callout(TenantState.reference_form_success, color_scheme="green", variant="soft"), rx.fragment()),
        rx.cond(TenantState.reference_form_error != "", rx.callout(TenantState.reference_form_error, color_scheme="red", variant="soft"), rx.fragment()),
        rx.hstack(
            ...
            rx.button(
                ...
                on_click=TenantState.import_references_from_notes,
                variant="soft", color_scheme="blue", size="2",
                disabled=TenantState.references_available_to_import == False,
            ),
            rx.button("Add Reference", on_click=TenantState.new_reference, variant="outline", color_scheme="blue", size="2"),
            width="100%", align="center", spacing="2",
        ),
        rx.cond(
            TenantState.show_reference_form,
            references_form(),
            rx.fragment(),
        ),
        rx.cond(
            TenantState.references.length() > 0,
            rx.table.root(
                rx.table.header(
                    rx.table.row(
                        rx.table.column_header_cell("Name"),
                        ...
                        rx.table.column_header_cell("Relationship / Years"),
                        rx.table.column_header_cell("Date Contacted"),
                        rx.table.column_header_cell("Outcome"),
                        rx.table.column_header_cell("Source"),
                        rx.table.column_header_cell("Actions"),
                    )
                ),
                rx.table.body(rx.foreach(TenantState.references, reference_row)),
                width="100%", variant="surface",
            ),
            rx.text("No references on file.", color="#888", size="2"),
        ),
        spacing="4", width="100%", align_items="start",
    )
...
def screening_factor_row(f: ScreeningFactor) -> rx.Component:
    return rx.table.row(
        rx.table.cell(rx.text(f.factor_label, size="2", weight="bold")),
        rx.table.cell(rx.text(f.score_text, size="2")),
        rx.table.cell(
            rx.badge(
                f.hard_flag_text,
                color_scheme=rx.cond(f.hard_flag, "red", "green"),
                variant="soft",
                size="1",
            )
        ),
        rx.table.cell(rx.text(f.notes, size="2", color="#555")),
        style=rx.cond(f.hard_flag, {"background": "#FFEBEE"}, {"background": "white"}),
    )


def screening_factor_breakdown_panel() -> rx.Component:
    return rx.cond(
        TenantState.screening_factor_records.length() > 0,
        rx.box(
            rx.vstack(
                rx.hstack(
                    rx.text("Assessment Breakdown", size="2", weight="bold", color=BRAND_DARK),
                    rx.badge("audit", color_scheme="purple", variant="soft", size="1"),
                    spacing="2",
                    align="center",
                ),
                rx.table.root(
                    rx.table.header(
                        rx.table.row(
                            ...
                            rx.table.column_header_cell("Hard Flag"),
                            rx.table.column_header_cell("Notes"),
                        )
                    ),
                    rx.table.body(rx.foreach(TenantState.screening_factor_records, screening_factor_row)),
                    width="100%",
                    variant="surface",
                ),
                spacing="3",
...
def screening_form() -> rx.Component:
    return rx.vstack(
        ...
        screening_factor_breakdown_panel(),
        rx.hstack(
            rx.button(
...
def attach_file_row_tenant(filename: str) -> rx.Component:
    return rx.text(
        "📎 " + filename,
        size="2",
        color=BRAND_DARK,
...
def attachment_picker_panel_tenant() -> rx.Component:
    return rx.vstack(
        rx.hstack(
            ...
            rx.cond(
                ...
                rx.button(
                    "Clear",
                    on_click=TenantState.clear_attachments,
                    variant="ghost",
                    size="1",
                    color_scheme="red",
                ),
        ...
        rx.cond(
            ...
            rx.vstack(
                rx.foreach(
                    TenantState.attach_filenames,
                    attach_file_row_tenant,
                ),
                spacing="1",
                padding="8px",
                background="#F9FAFB",
...
def compose_email_modal() -> rx.Component:
    return rx.dialog.root(
        rx.dialog.content(
            rx.vstack(
                rx.dialog.title("Send Email"),
                rx.cond(
                    TenantState.compose_error != "",
                    rx.callout.root(
                        rx.callout.text(TenantState.compose_error),
                        color_scheme="red",
                        width="100%",
                    ),
                    rx.fragment(),
                ...
                rx.cond(
                    TenantState.compose_success != "",
                    rx.callout.root(
                        rx.callout.text(TenantState.compose_success),
                        color_scheme="green",
                        width="100%",
                    ),
                    rx.fragment(),
                ...
                rx.cond(
                    TenantState.compose_contact_options.length() > 0,
                    rx.vstack(
                        rx.text("Contact", size="2", color="#555"),
                        rx.select(
                            TenantState.compose_contact_options,
                            value=TenantState.compose_selected_contact,
                            on_change=TenantState.set_compose_contact,
                            width="100%",
                        ),
                        spacing="1",
                        width="100%",
                ...
                rx.cond(
                    ...
                    rx.callout.root(
                        rx.callout.text(
                            "No contacts with an email address found. Enter an address manually."
                        ),
                        color_scheme="amber",
                        width="100%",
                    ),
                ...
                rx.vstack(
                    ...
                    rx.input(
                        value=TenantState.compose_to,
                        on_change=TenantState.set_compose_to,
                ...
                rx.vstack(
                    ...
                    rx.input(
                        value=TenantState.compose_subject,
                        on_change=TenantState.set_compose_subject,
                        placeholder="Subject",
                        width="100%",
                    ),
                    spacing="1",
                ...
                rx.vstack(
                    ...
                    rx.text_area(
                        value=TenantState.compose_body,
                        on_change=TenantState.set_compose_body,
                ...
                attachment_picker_panel_tenant(),
                rx.hstack(
                    rx.button(
                        "Send",
                        on_click=TenantState.send_compose_email,
                        loading=TenantState.compose_sending,
                        color_scheme="blue",
                    ),
                    rx.dialog.close(
                        rx.button(
                            "Cancel",
                            on_click=TenantState.close_compose_email,
                            variant="outline",
                        ),
                    ),
                    spacing="3",
        ...
        open=TenantState.compose_email_open,
    )
    ...
    return rx.vstack(
        ...
        rx.tabs.root(
            rx.tabs.list(
                ...
                rx.tabs.trigger("References", value="references"),
                rx.tabs.trigger("Addresses", value="addresses"),
            ...
            rx.tabs.content(references_tab(), value="references", padding_top="16px"),
            rx.tabs.content(addresses_tab(), value="addresses", padding_top="16px"),
...
RESIZER_SCRIPT = """
(function() {
    function installTenantResizer() {
        if (window.__lucidTenantResizerCleanup) {
            try { window.__lucidTenantResizerCleanup(); } catch (e) {}
        }
        ...
        function applySavedWidth() {
            var panel = document.getElementById('tenant-list-panel');
            if (!panel) { return; }
            try {
                var savedW = parseInt(window.localStorage.getItem('lucidTenantListWidth') || '', 10);
        ...
        function getResizerFromEvent(e) {
            ...
            for (var i = 0; i < path.length; i++) {
                if (path[i] && path[i].id === 'panel-resizer') {
                    return path[i];
                }
            }
            var target = e.target;
            return target && target.closest ? target.closest('#panel-resizer') : null;
        }

        function startResize(e) {
            var resizer = getResizerFromEvent(e);
            if (!resizer) { return; }
            leftPanel = document.getElementById('tenant-list-panel');
            if (!leftPanel) { return; }
            try {
                var savedW = parseInt(window.localStorage.getItem('lucidTenantListWidth') || '', 10);
                if (savedW && savedW >= 300 && savedW <= 760) {
        ...
        function moveResize(e) {
            ...
            try { window.localStorage.setItem('lucidTenantListWidth', String(newWidth)); } catch (err) {}
            e.preventDefault();
        }
        ...
        window.__lucidTenantResizerCleanup = function() {
    ...
    installTenantResizer();

    var tenantObserver = new MutationObserver(function() {
        if (document.getElementById('tenant-list-panel') && document.getElementById('panel-resizer')) {
            installTenantResizer();
            if (tenantObserver) {
    ...
    if (document.body) {
        tenantObserver.observe(document.body, { childList: true, subtree: true });
    }

    setTimeout(installTenantResizer, 250);
    setTimeout(installTenantResizer, 750);
    setTimeout(installTenantResizer, 1500);
    setTimeout(installTenantResizer, 3000);
})();
"""
