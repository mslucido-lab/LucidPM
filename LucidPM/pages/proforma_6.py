...
v6 — Excludes Terminated/Expired LeaseStatus rows and prorates first month for mid-month starts with RentDueDay=1.
...
class ProformaState(AppState):
        ...
        for seg in valid:
            if month_start <= seg["start"] <= month_end:
                ...
                if rent_due_day == 1 and ls.day > 1:
                    ...
                    days_occupied = days_in_month - ls.day + 1
                    prorated = round(rent / days_in_month * days_occupied, 2)
                    return prorated
    ...
    def _do_compute(self):
        ...
        lease_rows = run_query(
            ...
            "l.RentDueDay, l.LeaseStatus, t.Suite AS TenantSuite "
            "FROM Leases l "
            "INNER JOIN Tenants t ON l.TenantID = t.TenantID "
            "WHERE l.PropertyID = ? "
            "AND (l.LeaseStatus IS NULL OR l.LeaseStatus NOT IN ('Terminated', 'Expired')) "
