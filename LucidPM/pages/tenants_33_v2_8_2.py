...
v2.8.1 — Stabilizes Addresses tab rendering to avoid Reflex Var string-concat errors.

v2.8.0 — Adds tenant Addresses tab with CRUD and application-note import.
...
class TenantState(AppState):
    ...
    # Static base-var list for the Label dropdown (same pattern as status_names,
    # method_choices, etc.). A computed @rx.var list fed to rx.select fails to
    # compile in the frontend; a plain list[str] field is the supported pattern.
    address_label_choices: list[str] = [ADDRESS_LABEL_NONE] + ADDRESS_LABEL_BASE
    a_line1: str = ""
    a_line2: str = ""
    a_city: str = ""
    a_state: str = ""
        ...
        try:
            ...
            self.addresses = [
                TenantAddress(
                    ...
                    label=str(r.get("AddressLabel") or ""),
    ...
    def select_address(self, addr_id: int):
        ...
        if not match:
            ...
            return
        lbl = (match.label or "").strip()
        self.a_label = lbl if lbl else ADDRESS_LABEL_NONE
        # Ensure the dropdown can display a legacy/unknown label without blanking
        if self.a_label not in self.address_label_choices:
            self.address_label_choices = self.address_label_choices + [self.a_label]
    ...
    def import_address_from_notes(self):
        ...
        if personal_raw:
            ...
            if len(parts) > 1 and "," in parts[1]:
                city_part, _, rest = parts[1].partition(",")
    ...
    def set_a_label(self, v): self.a_label = v
...
def address_row(a: TenantAddress) -> rx.Component:
    return rx.table.row(
        rx.table.cell(
            rx.hstack(
                rx.text(a.label, size="2", weight="bold"),
                rx.cond(
                    a.is_primary,
...
def addresses_tab() -> rx.Component:
    return rx.vstack(
        ...
        rx.grid(
            _form_field("Label", rx.select(TenantState.address_label_choices,
        ...
        _form_field("Notes", rx.text_area(value=TenantState.a_notes,
            on_change=TenantState.set_a_notes, placeholder="Optional notes", size="2", width="100%")),
        rx.cond(TenantState.form_error != "",
                rx.callout(TenantState.form_error, color="red", variant="soft"), rx.fragment()),
        rx.cond(TenantState.form_success != "",
                rx.callout(TenantState.form_success, color="green", variant="soft"), rx.fragment()),
