        ...
        try:
            ...
            for idx, p in enumerate(selected, start=1):
                ...
                if self._has_renderable_text(p):
                    rendered_text, unresolved = render_text_template(content, context) if content else ("", [])
                    if unresolved:
                        label = p.template_section_label or p.section_name
        ...
        try:
            ...
            for idx, row in enumerate(rows, start=1):
                ...
                if text_to_render or self._has_renderable_text(temp_section_meta):
                    ...
                    rendered_text, unresolved_after_render = render_text_template(text_to_render, context) if text_to_render else ("", [])
