...
# VERSION: 3.0.18k
# - v3.0.18k: adds SectionNumber and ExtensionNoticePeriod to available token picker UI.
# - v3.0.18h: adds up/down reorder buttons for package template section slots.
# - v3.0.18g: allows metadata-only Text Clause sections for header-only reusable sections.
# - v3.0.18f: expands the clause editor token picker to include the current lease_merge.py token set.
# - v3.0.18d: paste-and-split helper text and save actions are state-aware for PDF vs Text Clause mode.
# - v3.0.18c: paste-and-split supports standalone Text Clause mode without source-document warning.
...
class LeaseDocumentState(AppState):
    ...
    def set_tab(self, tab: str):
        # Leaving the Section Library drops its selection so shared section
        # edit state (editing_section_id / p_*) does not leak into another tab.
    ...
    async def handle_upload(self, files: list[rx.UploadFile]):
        ...
        try:
            ...
            self.form_success = f"Uploaded {self.selected_upload_file_name} with {pc} pages. Use Split / Add Section to split it."
            self._load_source_documents()
            self._load_sections()
            self._load_all_sections()
            self._load_reusable_section_options()
    ...
    def import_local_pdf_for_testing(self):
        ...
        try:
            ...
            self.p_start_page = "1"
            self.p_end_page = str(pc)
            self.form_success = f"Imported source PDF with {pc} pages. Use Split / Add Section to split it."
            self._load_source_documents()
            self._load_sections()
            self._load_all_sections()
            self._load_reusable_section_options()
    ...
    def open_section_in_library(self, section_id: int):
        """Open a section from another tab in the Section Library."""
        self.select_library_section(section_id)
        self.admin_lease_tab = "library"
    ...
    def create_section(self):
        ...
        if is_text_clause:
            if has_source_document and (start < 1 or end < start or end > self.selected_source_page_count):
                ...
                return
            has_heading_metadata = bool(str(self.p_article_number or "").strip() or str(self.p_display_label or "").strip())
        ...
        try:
            ...
            if is_text_clause:
                ...
                page_count = max(int(self.selected_source_page_count or 0), 1)
                next_start = min(int(end) + 1, page_count)
                self.library_create_mode = "pdf"
                self.p_section_name = ""
                self.p_exhibit_code = ""
                self.p_article_number = ""
                ...
                self.p_clause_tag = ""
                self.p_content = ""
                self.p_start_page = str(next_start)
                self.p_end_page = str(page_count)
                self.p_sort_order = str(self._next_section_sort_order())
                self.form_success = f"Section {new_id} created — next range ready."
        except Exception as ex:
    ...
    def load_draft_clause(self, draft_id: int):
        ...
        for draft in self.draft_clauses:
            if int(draft.draft_id or 0) == did:
                ...
                self.p_is_standalone_clause = int(self.selected_source_document_id or 0) <= 0
    ...
    def save_draft_clause(self, draft_id: int):
        ...
        if self.p_creation_mode != "Text Clause":
            self.form_error = "Switch Creation mode to Text Clause before saving pasted draft clauses."
            return
        did = int(draft_id or 0)
    ...
    def _load_template_sections(self):
        ...
        for idx, r in enumerate(rows):
            section_rows.append(LeaseTemplateSectionRow(
                section_id=int(r.get("LeaseTemplateSectionID") or 0),
                sort_order=int(r.get("SortOrder") or 0),
                can_move_up=idx > 0,
                can_move_down=idx < total_rows - 1,
        ...
        self.lease_template_sections = section_rows
    ...
    def _move_template_section(self, section_id: int, direction: str):
        self.form_error = ""
        self.form_success = ""
        sid = _safe_int(section_id)
        tid = _safe_int(self.selected_template_id)
        if sid <= 0 or tid <= 0:
            self.form_error = "Select a package template section first."
            return
        try:
            rows = run_query(
                "SELECT LeaseTemplateSectionID, SortOrder FROM LeaseTemplateSections "
                "WHERE LeaseTemplateID = ? AND ISNULL(IsActive, 1) = 1 "
                "ORDER BY SortOrder, LeaseTemplateSectionID",
                (tid,), db=self.db,
            )
            ordered = [
                {"id": _safe_int(r.get("LeaseTemplateSectionID")), "sort": _safe_int(r.get("SortOrder"))}
                for r in rows
            ]
            idx = next((i for i, row in enumerate(ordered) if row["id"] == sid), -1)
            if idx < 0:
                self.form_error = "Template section not found."
                return
            ...
            run_exec(
                "UPDATE LeaseTemplateSections SET SortOrder = ? WHERE LeaseTemplateSectionID = ?",
                (other["sort"], current["id"]), db=self.db,
            )
            run_exec(
                "UPDATE LeaseTemplateSections SET SortOrder = ? WHERE LeaseTemplateSectionID = ?",
                (current["sort"], other["id"]), db=self.db,
            )
            self.form_success = "Template section order updated."
            self._load_template_sections()
            self._load_lease_templates()
        except Exception as ex:
            self.form_error = f"Could not reorder template section: {ex}"

    def move_section_up(self, section_id: int):
        self._move_template_section(section_id, "up")

    def move_section_down(self, section_id: int):
        self._move_template_section(section_id, "down")
    ...
    def set_p_creation_mode(self, v: str):
        ...
        if v == "Text Clause" and int(self.selected_source_document_id or 0) <= 0:
            self.p_is_standalone_clause = True
        elif v != "Text Clause":
            self.p_is_standalone_clause = False
    ...
    return rx.table.row(
        ...
        rx.table.cell(
            rx.button(
                ...
                variant="soft",
                color_scheme="blue",
                on_click=LeaseDocumentState.start_new_section_from_source(row.source_document_id),
                title="Open this source document in the Section Library PDF splitter",
            )
        ),
        style=rx.cond(
    ...
    return rx.box(
        rx.vstack(
            ...
            rx.hstack(
                ...
                rx.button("Split ->", size="1", variant="soft", color_scheme="blue", on_click=LeaseDocumentState.start_new_section_from_source(row.source_document_id)),
                spacing="2",
            ),
            spacing="2",
            align_items="start",
...
def section_row(row: SectionRow) -> rx.Component:
    return rx.table.row(
        ...
        rx.table.cell(
            rx.hstack(
                rx.button("Edit", size="1", variant="soft", color_scheme="blue", on_click=LeaseDocumentState.open_section_in_library(row.section_id)),
        ...
        rx.table.cell(
            rx.hstack(
                rx.button(
                    ...
                    variant="outline",
                    color_scheme="gray",
                    on_click=LeaseDocumentState.move_section_up(row.section_id),
                    is_disabled=rx.cond(row.can_move_up, False, True),
                ...
                rx.button(
                    ...
                    variant="outline",
                    color_scheme="gray",
                    on_click=LeaseDocumentState.move_section_down(row.section_id),
                    is_disabled=rx.cond(row.can_move_down, False, True),
                    style=arrow_button_style,
                ),
                spacing="2",
                align="center",
            ),
            style={"min_width": "70px"},
        ),
        rx.table.cell(rx.text(row.sort_order, size="2")),
    ...
    return rx.box(
        rx.vstack(
            ...
            _token_group("Property / Suite", [
                ...
                "{{SuiteLabel}}",
                "{{SuiteFullAddress}}",
                "{{SuitePremisesDescription}}",
                "{{SuiteLegalDescription}}",
                "{{UseType}}",
                "{{County}}",
                "{{State}}",
                "{{PropertyAddress}}",
        ...
        rx.hstack(
            rx.box(
                rx.vstack(
                    ...
                    rx.text("Select a source to edit metadata or open it in the Section Library splitter.", size="1", color="#666"),
                    rx.cond(
                    ...
                    rx.card(
                        rx.vstack(
                            rx.hstack(
                                ...
                                rx.spacer(),
                                rx.cond(LeaseDocumentState.selected_source_document_id > 0, rx.button("Split / Add Section", size="1", variant="soft", color_scheme="blue", on_click=LeaseDocumentState.start_new_section_from_source(LeaseDocumentState.selected_source_document_id)), rx.fragment()),
