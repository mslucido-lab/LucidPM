...
# v2.8.0 - Fix #4: dynamic Content sections render to PDF with ReportLab before package merge.
...
def render_text_section_to_pdf_file(section_text: str, output_path: str) -> str:
    """Render a token-resolved lease section to a simple PDF file.

    This is the near-term hybrid rendering path:
      - text Content sections become ReportLab PDFs
      - static PDF sections pass through unchanged
    """
    ...
    story = []
    lines = str(section_text or "").replace("\r\n", "\n").replace("\r", "\n").split("\n")
...
class LeasePackageBuilderState(AppState):
    ...
    def generate_package(self):
        ...
        if missing:
            self.form_error = "Missing PDF files for PDF-only sections: " + ", ".join(missing[:3])
            return
