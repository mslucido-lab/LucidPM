"""
Manage Vendors page — list + inline form.
"""
...
class VendorState(AppState):
    ...
    # Category lookup (id → name, name → id)
    ...
    def save_vendor(self):
        ...
        if self.vendor_mode == "edit":
            ...
            # Keep denormalized VendorName in sync on WorkItems
...
def vendor_row(v: VendorSummary) -> rx.Component:
    return rx.table.row(
        ...
        rx.table.cell(rx.text(v.email, size="2", color="#555")),
        rx.table.cell(
            rx.cond(
                v.active == "Yes",
    ...
    return rx.vstack(
        ...
        rx.grid(
            ...
            rx.vstack(
                ...
                rx.select(VendorState.category_options[1:],  # exclude "All" from form
