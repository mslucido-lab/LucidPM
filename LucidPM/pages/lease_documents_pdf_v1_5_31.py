...
def _outer_inline_tags(text: str) -> tuple[list[str], str]:
    """Return outer inline tags that wrap the whole text, plus the inner text.

    This protects common authored patterns like:
      <b>paragraph one<br/>paragraph two</b>

    If <br/> is later converted to paragraph breaks, each resulting paragraph
    needs its own balanced <b>...</b> tags. Otherwise ReportLab sees an
    unclosed <b> tag and fails the whole PDF generation.
    """
...
def _rewrap_inline_tags(block: str, tags: list[str]) -> str:
    """Wrap a paragraph block with balanced inline tags stripped by _outer_inline_tags."""
...
def _split_blocks(text: str) -> list[str]:
    ...
    # Preserve balanced outer inline tags, such as one <b> wrapper around a
    # multi-paragraph clause, by reapplying them to each split paragraph.
    outer_tags, inner_text = _outer_inline_tags(text)
    split_source = re.sub(r"<br\s*/?>", "\n\n", inner_text, flags=re.IGNORECASE)

    blocks = [b.strip() for b in re.split(r"\n\s*\n+", split_source) if b.strip()]
    if outer_tags:
        blocks = [_rewrap_inline_tags(b, outer_tags) for b in blocks]
