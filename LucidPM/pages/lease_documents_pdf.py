...
Patched 5/19/2026 (Mark):
- v1.5.55: Adds >> indent marker support for explicitly indented lease PDF paragraphs.
- v1.5.56: Adds bulletText, leftIndent, bulletIndent, fontSize attribute support on <para> tags for hanging-indent amendment paragraphs.

Patched 5/5/2026 (Mark):
- v1.5.50: Line 839, change from: next_style in ("IndentedBody", "IndentedChildBody"): TO next_style in ("IndentedBody", "IndentedChildBody", "ClauseBody"):
- renamed the function from _keep_special_conditions_with_first_child to _keep_header_with_first_child to reflect that it now applies to all article headers
- renamed return _keep_special_conditions_with_first_child(story) TO return _keep_header_with_first_child(story)

Patched 5/4/2026:
- v1.5.49: style-based Special Conditions header widow fix; keeps article header with first indented child only.
- v1.5.47: tightens grouped 3+ year payment schedule width for better visual centering.
...
def _para_tag_flowable(para_block: str, base_style) -> "Paragraph":
    """Parse <para> tag attributes and return a styled Paragraph flowable.

    Supports attributes: align, bulletText, bulletIndent, leftIndent, fontSize.
    Falls back to _safe_paragraph if attribute parsing fails.
    """
    from copy import copy

    # Extract the opening <para ...> tag and inner content
    tag_match = re.match(r"(<para\b[^>]*>)(.*?)</para\s*>", para_block, re.IGNORECASE | re.DOTALL)
    if not tag_match:
        return _safe_paragraph(_sanitize_reportlab_markup(para_block), base_style)

    para_tag = tag_match.group(1)
    inner = tag_match.group(2).strip()

    # Parse attributes
    def _attr(name: str):
        m = re.search(rf'{name}=["\']([^"\']*)["\']', para_tag, re.IGNORECASE)
        return m.group(1) if m else None

    align_val     = _attr("align")
    bullet_text   = _attr("bulletText")      # e.g. "A." or "1."
    bullet_indent = _attr("bulletIndent")    # points — where bullet label sits
    left_indent   = _attr("leftIndent")      # points — where body text starts
    font_size     = _attr("fontSize")        # points — override body font size

    # Clone style if any layout overrides are present
    needs_override = any(v is not None for v in [bullet_indent, left_indent, font_size, align_val])

    if needs_override:
        style = copy(base_style)
        if left_indent is not None:
            try:
                style.leftIndent = float(left_indent)
            except ValueError:
                pass
        if bullet_indent is not None:
            try:
                style.bulletIndent = float(bullet_indent)
            except ValueError:
                pass
        ...
        if align_val:
            ...
            style.alignment = _align_map.get(align_val.lower(), style.alignment)
    else:
        style = base_style

    # Sanitize inner content
    sanitized = _sanitize_reportlab_markup(inner) if _looks_like_reportlab_markup(inner) else _xml(inner)

    try:
        if bullet_text is not None:
            return Paragraph(sanitized, style, bulletText=bullet_text)
        return Paragraph(sanitized, style)
    except Exception:
        # Fall back to safe paragraph on any ReportLab parse error
        return _safe_paragraph(sanitized, style)
...
def _markup_flowables(text: str, style) -> list:
    ...
    for part in parts:
        ...
        if "<para" in cleaned.lower():
            flowables.append(_para_tag_flowable(cleaned, style))
        else:
...
def _schedule_table_flowable(lines: list[str]):
    ...
    if has_two_column_rows:
        ...
        table_width = min(4.15 * 72, clause_body_width)
...
def _paragraph_for_block(block: str):
    ...
    # Detect inline bold section number prefix authored as:
    # <b>{{SectionNumber}}.</b> Body text  OR  <b>4.</b> Body text
    # Split into ARTICLE_HEADER + CLAUSE_BODY flowables so the
    # widow fix can keep them together across page boundaries.
    _bold_num_match = re.match(
        r"^<b>(\d+|\{\{SectionNumber\}\})[\.)]?<\/b>\s*(.+)$",
        clean,
        re.DOTALL | re.IGNORECASE,
    )
    if _bold_num_match:
        marker = _bold_num_match.group(1).rstrip(".") + "."
        body = _bold_num_match.group(2).strip()
        return (
            _markup_flowables(f"<b>{marker}</b>", LEASE_STYLES["ARTICLE_HEADER"])
            + _markup_flowables(body, LEASE_STYLES["CLAUSE_BODY"])
        )

    # Detect full bold heading: <b>4. Rent Payment</b>
    # Produced by _inject_display_label_after_section_number in the builder.
    _bold_heading_match = re.match(
        r"^<b>(\d+[\.)])\s+(.+?)</b>\s*$",
        clean,
        re.IGNORECASE | re.DOTALL,
    )
    if _bold_heading_match:
        return _markup_flowables(clean, LEASE_STYLES["ARTICLE_HEADER"])
...
def _as_single_paragraph(flowable):
    """Return a Paragraph when the flowable is one Paragraph or a one-item KeepTogether containing one Paragraph."""
    if isinstance(flowable, Paragraph):
        return flowable
    try:
        content = getattr(flowable, "_content", None)
        if content and len(content) == 1 and isinstance(content[0], Paragraph):
            return content[0]
    except Exception:
        return None
    return None
...
def _style_name(flowable) -> str:
    paragraph = _as_single_paragraph(flowable)
    if not paragraph:
        return ""
...
def _keep_header_with_first_child(story: list) -> list:
    """Wrap an ARTICLE_HEADER immediately followed by an INDENTED_BODY-style child.

    This is a post-processing widow fix for Special Conditions. It is intentionally
    style-based and narrow: only a header followed by the first indented child is
    grouped. Normal clause headers followed by regular body text are unchanged.
    """
    out = []
    i = 0
    while i < len(story):
        item = story[i]
        if _style_name(item) == "ArticleHeader":
            j = i + 1
            while j < len(story) and (_is_zero_spacer(story[j]) or _is_cond_page_break(story[j])):
                j += 1
            next_style = _style_name(story[j]) if j < len(story) else ""
            if next_style in ("IndentedBody", "IndentedChildBody", "ClauseBody"):
                # The first flowable in each section may already be wrapped in
                # KeepTogether([header]) by _story_for_text_sections().
                # ReportLab does not handle nested KeepTogether reliably, so
                # unwrap the header paragraph before re-wrapping it with its
                # first child/body paragraph.
                header_para = _as_single_paragraph(item) or item
                body_para = _as_single_paragraph(story[j]) or story[j]
                out.append(KeepTogether([header_para, body_para]))
                i = j + 1
                continue
        out.append(item)
        i += 1
...
def _story_for_text_sections(section_texts: Sequence[str]) -> list:
    ...
    return _keep_header_with_first_child(story)
