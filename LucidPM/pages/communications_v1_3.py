...
def attach_file_row_comms(filename: str) -> rx.Component:
    return rx.hstack(
        rx.checkbox(
            checked=CommunicationsState.attach_selected.contains(filename),
            on_change=CommunicationsState.toggle_attach_file(filename),
        ),
...
def attachment_picker_panel_comms() -> rx.Component:
    return rx.vstack(
        ...
        rx.hstack(
            rx.input(
                value=CommunicationsState.attach_folder,
                on_change=CommunicationsState.set_attach_folder,
                placeholder=r"C:\path\to\folder",
                width="100%",
            ),
            rx.button(
                "Browse",
                on_click=CommunicationsState.load_attach_file_list,
                variant="outline",
                size="2",
            ),
            width="100%",
        ...
        rx.cond(
            CommunicationsState.attach_folder_error != "",
            rx.text(
                CommunicationsState.attach_folder_error,
                size="1",
                color="#D96B6B",
        ...
        rx.cond(
            CommunicationsState.attach_file_list.length() > 0,
            rx.vstack(
                rx.text(
                ...
                rx.foreach(
                    CommunicationsState.attach_file_list,
        ...
        rx.cond(
            CommunicationsState.attach_selected.length() > 0,
            rx.text(
                CommunicationsState.attach_selected.length().to_string() + " file(s) selected",
                size="1",
                color=BRAND_DARK,
                weight="medium",
            ),
