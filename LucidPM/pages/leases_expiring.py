...
# Page width — matches vendors.py / properties.py pattern
...
def _same_suite(a: dict, b: dict) -> bool:
    """Best-effort same-suite comparison for lease transition matching."""
    for key in ("SuiteID", "TenantSuiteID"):
        av = a.get(key)
        bv = b.get(key)
    ...
    a_suite = str(a.get("TenantSuite") or "").strip().upper()
    b_suite = str(b.get("TenantSuite") or "").strip().upper()
    if a_suite and b_suite and a_suite == b_suite:
        return True

    return False


def _active_predecessor_rent(
    leases: list[dict],
    sched: list[dict],
    lease_id: int,
    today: datetime.date,
    ...
    """
    For a future successor lease, return the rent from the same tenant+suite
    lease that is actually active on the report run date. This prevents the
    current rent column from falling back to the future lease header rent.
    """
    this_lease = next((l for l in leases if int(l["LeaseID"]) == lease_id), None)
    if not this_lease:
        return None
    ...
    for l in leases:
        try:
            if int(l["LeaseID"]) == lease_id:
                continue
            if l.get("TenantID") != tenant_id:
                continue
            if not _same_suite(this_lease, l):
                continue

            start = _to_date(l.get("LeaseStart"))
            end = _to_date(l.get("LeaseEnd"))
            if start is not None and start <= today and (end is None or end >= today):
                candidates.append((start, l))
        except Exception:
            continue
    ...
    try:
        rent, _ = _rent_as_of(sched, int(active["LeaseID"]), today)
        if rent is not None:
            return rent
        rent_amt = active.get("RentAmount")
        return float(rent_amt) if rent_amt is not None else None
    except Exception:
        return None
                    ...
                    if today <= ann <= horizon:
                        if lease_end is None or ann < lease_end:
                            ...
                            if current_rent is None:
                                current_rent = _active_predecessor_rent(leases, sched, lid, today)
                            if current_rent is None:
