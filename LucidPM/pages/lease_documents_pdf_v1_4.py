...
from reportlab.platypus import SimpleDocTemplate, Spacer, Preformatted, KeepTogether, Paragraph

DEFAULT_DOCUMENT_ROOT = r"C:\Dell Inspirion\TenantCRM\LeaseDocuments"

try:
    from LucidPM_Reflex.lease_render_styles import (
        PAGE_SETUP,
        LEASE_STYLES,
        LEASE_MONOSPACE,
    ...
    LEASE_STYLES = {
        "CLAUSE_BODY": ParagraphStyle("ClauseBody", fontName="Times-Roman", fontSize=10, leading=12),
        "ARTICLE_HEADER": ParagraphStyle("ArticleHeader", fontName="Times-Bold", fontSize=10, leading=12, keepWithNext=True),
        ...
        "INDENTED_BODY": ParagraphStyle("IndentedBody", fontName="Times-Roman", fontSize=10, leading=12, leftIndent=18),
...
def _is_payment_schedule(block: str) -> bool:
    ...
    lettered = sum(1 for ln in lines if re.search(r"(^|\s)[a-z]\)", ln))
    return money_lines >= 2 and (lettered >= 2 or len(lines) >= 3)
...
def _paragraph_for_block(block: str):
    ...
    if _is_jurisdiction(clean):
        return [Paragraph(_xml(clean), LEASE_STYLES["JURISDICTION"])]
    ...
    if _is_payment_schedule(clean):
        return [Preformatted(escape(clean), LEASE_STYLES["PAYMENT_SCHEDULE"], maxLineLength=95)]
    ...
    if header:
        ...
        if rest:
            return [
                KeepTogether([
                    Paragraph(_xml(f"{marker} {rest}"), LEASE_STYLES["ARTICLE_HEADER"]),
                ])
            ]
...
def _signature_footer(canvas, doc):
    ...
    try:
        ...
        footer = Paragraph(SIGNATURE_FOOTER_TEXT, LEASE_STYLES.get("SIGNATURE_FOOTER", LEASE_MONOSPACE))
        w, h = footer.wrap(doc.width, 36)
        footer.drawOn(canvas, (width - w) / 2, 28)
    except Exception:
        return
