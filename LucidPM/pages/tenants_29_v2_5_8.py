...
class TenantState(AppState):
    ...
    def suggest_screening_result(self):
        """Suggest Overall Result, Risk Tier, and Deposit based on entered screening fields.

        This is intentionally advisory. The user can override every value before saving.
        """
        score = self._int_or_none(self.s_credit_score) or 0
        evictions = self._int_or_none(self.s_evictions) or 0
        bankruptcies = self._int_or_none(self.s_bankruptcies) or 0
        collections = self._int_or_none(self.s_collections) or 0
        charge_offs = self._int_or_none(self.s_charge_offs) or 0
        delinquent = self._int_or_none(self.s_delinquent_accounts) or 0
        criminal_records = str(self.s_criminal_result or "").strip().lower() == "records found"
        eviction_records = str(self.s_eviction_result or "").strip().lower() == "records found"
        ...
        if evictions > 0 or eviction_records:
            reasons.append("eviction history found")
        if bankruptcies > 0:
            reasons.append("bankruptcy reported")
        if criminal_records:
            reasons.append("criminal records found")
        if collections > 3 or charge_offs > 0 or delinquent > 2:
            reasons.append("elevated credit derogatories")

        if evictions > 0 or eviction_records or criminal_records or (score and score < 580):
            result = "Fail"
            tier = "High"
            deposit_multiplier = 2
        elif bankruptcies > 0 or collections > 3 or charge_offs > 0 or delinquent > 2 or (score and score < 620):
            result = "Conditional"
            tier = "Elevated"
            deposit_multiplier = 2
        elif collections > 0 or delinquent > 0 or (score and score < 700):
            result = "Conditional"
            tier = "Moderate"
            deposit_multiplier = 1

        rent = self._latest_monthly_rent_amount()
        if deposit_multiplier and rent > 0:
            self.s_deposit_recommended = str(int(round(rent * deposit_multiplier)))

        self.s_overall_result = result
        self.s_risk_tier = tier

        reason_text = ", ".join(reasons) if reasons else "no major adverse items entered"
        note = f"System suggestion: {result} / {tier} based on {reason_text}."
        existing = str(self.s_notes or "").strip()
        if "System suggestion:" not in existing:
            self.s_notes = (existing + "\n" + note).strip() if existing else note
        self.screening_form_success = "Suggested screening result applied. Review and edit before saving."
        ...
        try:
            if self.screening_mode == "edit" and int(self.selected_screening_id or 0) > 0:
                run_exec(
                    ...
                    values + (int(self.selected_screening_id), self.tenant_id),
                    db=self.db,
                )
...
def screening_form() -> rx.Component:
    return rx.vstack(
        ...
        rx.hstack(
            rx.button("Suggest result", on_click=TenantState.suggest_screening_result, variant="soft", color_scheme="purple", size="2"),
            rx.button(
