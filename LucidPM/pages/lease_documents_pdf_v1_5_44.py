...
def _article_header_paragraph(flowable):
    """Return the ARTICLE_HEADER Paragraph inside a flowable, if present."""
    if isinstance(flowable, Paragraph):
        style = getattr(flowable, "style", None)
        if getattr(style, "name", "") == "ArticleHeader":
            return flowable
        return None

    # KeepTogether stores its wrapped flowables on _content in ReportLab.
...
def _is_zero_spacing_flowable(flowable) -> bool:
    """Return True for story separators that should not break header grouping."""
    if isinstance(flowable, Spacer):
        return float(getattr(flowable, "height", 0) or 0) == 0
    if isinstance(flowable, CondPageBreak):
        return True
    return False


def _keep_headers_with_next(story: list) -> list:
    """Keep each ARTICLE_HEADER paragraph with the next substantive flowable."""
    output = []
    idx = 0
    while idx < len(story):
        current = story[idx]
        if _article_header_paragraph(current) is None:
        ...
        next_idx = idx + 1
        skipped = []
        while next_idx < len(story) and _is_zero_spacing_flowable(story[next_idx]):
            skipped.append(story[next_idx])
            next_idx += 1

        if next_idx < len(story):
            output.append(KeepTogether([current] + skipped + [story[next_idx]]))
            idx = next_idx + 1
            continue
...
def _story_for_text_sections(section_texts: Sequence[str]) -> list:
    ...
    return _keep_headers_with_next(story)
