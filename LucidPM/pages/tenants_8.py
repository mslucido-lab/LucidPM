...
class TenantState(AppState):
    ...
    sched_form_error: str = ""
    sched_form_success: str = ""

    # Increase type lookup
    ...
    @rx.var
    def sched_editing_banner(self) -> str:
        parts = [x for x in [self.s_start, self.s_increase_type] if x]
        return 'Editing: ' + ' · '.join(parts) if parts else 'Editing rent row'
    ...
    def _load_lease_lookups(self):
        ...
        # Rent increase types
        rows = run_query(
            "SELECT LeaseRentIncreaseTypeID, IncreaseTypeName FROM LeaseRentIncreaseTypes "            "WHERE IsActive = 1 ORDER BY SortOrder, IncreaseTypeName",
            db=self.db,
        )
        ...
        # Default s_increase_type to Base if not set
        if not self.s_increase_type and "Base" in self.increase_type_names:
            self.s_increase_type = "Base"
    ...
    def select_sched_row(self, sched_id: int):
        self.selected_sched_id = sched_id
        self.sched_mode = "edit"
        self.sched_form_error = ""
        self.sched_form_success = ""
        ...
        if not rows:
            return
        r = rows[0]
        raw_start = r.get("EffectiveStartDate")
        raw_end   = r.get("EffectiveEndDate")
        self.s_start = raw_start.strftime("%Y-%m-%d") if raw_start else ""
        self.s_end   = raw_end.strftime("%Y-%m-%d") if raw_end else ""
        try:
            self.s_rent = str(int(float(r.get("RentAmount") or 0)))
        except (TypeError, ValueError):
            self.s_rent = ""
        self.s_increase_type = str(r.get("IncreaseTypeName") or (self.increase_type_names[0] if self.increase_type_names else ""))
        self.s_notes = str(r.get("Notes") or "")

    def new_sched_row(self):
        self.selected_sched_id = 0
        self.sched_mode = "new"
        self.sched_form_error = ""
        self.sched_form_success = ""
        self.confirm_delete_sched = False
        self.s_start = datetime.date.today().strftime("%Y-%m-%d")
        self.s_end = ""
        self.s_rent = ""
        self.s_increase_type = "Base" if "Base" in self.increase_type_names else (self.increase_type_names[0] if self.increase_type_names else "")
        self.s_notes = ""

    def save_sched_row(self):
        self.sched_form_error = ""
        self.sched_form_success = ""
        if not self.s_start:
            self.sched_form_error = "Effective start date is required."
            return
        if not self.s_increase_type or self.s_increase_type not in self.increase_type_names:
            self.sched_form_error = "Increase type is required."
            return
        inc_type_id = self.increase_type_ids[self.increase_type_names.index(self.s_increase_type)]
        try:
            rent = float(self.s_rent) if self.s_rent.strip() else 0.0
        except ValueError:
            rent = 0.0
        try:
            eff_start = datetime.datetime.strptime(self.s_start, "%Y-%m-%d").date()
        except ValueError:
            self.sched_form_error = "Invalid start date."
            return
        try:
            eff_end = datetime.datetime.strptime(self.s_end, "%Y-%m-%d").date() if self.s_end else None
        except ValueError:
            eff_end = None
        if eff_end and eff_end < eff_start:
            self.sched_form_error = "Effective end cannot be before effective start."
            return
        # Overlap check — load existing rows for this lease
        existing = run_query(
            "SELECT LeaseRentScheduleID, EffectiveStartDate, EffectiveEndDate "
            "FROM LeaseRentSchedule WHERE LeaseID = ?",
            (self.selected_lease_id,), db=self.db,
        )
        for row in existing:
            if self.sched_mode == "edit" and int(row["LeaseRentScheduleID"]) == self.selected_sched_id:
                continue
            os = row.get("EffectiveStartDate")
            oe = row.get("EffectiveEndDate")
            if os is None:
                continue
            os = os.date() if hasattr(os, "date") else os
            oe = oe.date() if oe and hasattr(oe, "date") else oe
            # Ranges overlap if start_a < end_b AND start_b < end_a (treating None end as open)
            a_end = eff_end or datetime.date(9999, 12, 31)
            b_end = oe or datetime.date(9999, 12, 31)
            if eff_start < b_end and os < a_end:
                self.sched_form_error = "This date range overlaps an existing rent row for this lease."
                return
        if self.sched_mode == "edit":
            run_exec(
                "UPDATE LeaseRentSchedule SET EffectiveStartDate=?, EffectiveEndDate=?, "                "RentAmount=?, IncreaseTypeID=?, Notes=? WHERE LeaseRentScheduleID=?",
                (eff_start, eff_end, rent, inc_type_id, self.s_notes, self.selected_sched_id),
                db=self.db,
            )
            self.sched_form_success = "Rent row saved."
        else:
            run_exec(
                "INSERT INTO LeaseRentSchedule (LeaseID, EffectiveStartDate, EffectiveEndDate, "                "RentAmount, IncreaseTypeID, Notes) VALUES (?,?,?,?,?,?)",
                (self.selected_lease_id, eff_start, eff_end, rent, inc_type_id, self.s_notes),
                db=self.db,
            )
            self.sched_form_success = "Rent row created."
            self.new_sched_row()
        self._load_rent_schedule(self.selected_lease_id)

    def confirm_delete_sched_action(self):
        self.confirm_delete_sched = True

    def cancel_delete_sched(self):
        self.confirm_delete_sched = False

    def delete_sched_row(self):
        if self.selected_sched_id == 0:
            return
        run_exec("DELETE FROM LeaseRentSchedule WHERE LeaseRentScheduleID=?",
                 (self.selected_sched_id,), db=self.db)
        self._load_rent_schedule(self.selected_lease_id)
        self.new_sched_row()

    # Sched setters
    def set_s_start(self, v): self.s_start = v
    def set_s_end(self, v): self.s_end = v
    def set_s_rent(self, v): self.s_rent = v
    def set_s_increase_type(self, v): self.s_increase_type = v
...
def rent_schedule_row(r: RentScheduleRow) -> rx.Component:
    return rx.table.row(
        ...
        rx.table.cell(
            rx.button("Edit", size="1", variant="soft", color_scheme="blue",
                      on_click=TenantState.select_sched_row(r.sched_id))
        ),
        style=rx.cond(
        ...
        rx.cond(
            ...
            rx.vstack(
                ...
                rx.hstack(
                    ...
                    rx.spacer(),
                    rx.button("+ New rent row", on_click=TenantState.new_sched_row,
                              variant="outline", color_scheme="blue", size="2"),
                    align="center", width="100%",
                ),
                # Schedule table
                rx.cond(
                    ...
                    rx.text("No rent schedule rows yet.", color="#888", size="2"),
                ),
                # Schedule form
                rx.divider(),
                rx.cond(
                    TenantState.sched_mode == "edit",
                    edit_banner(TenantState.sched_editing_banner),
                    rx.text("New rent row", size="2", weight="bold", color=BRAND_DARK),
                ),
                rx.grid(
                    rx.vstack(
                        rx.text("Effective start *", size="1", color="#666"),
                        rx.input(value=TenantState.s_start, on_change=TenantState.set_s_start,
                                 type="date", size="2", width="100%"),
                        spacing="1", width="100%",
                    ),
                    rx.vstack(
                        rx.text("Effective end", size="1", color="#666"),
                        rx.input(value=TenantState.s_end, on_change=TenantState.set_s_end,
                                 type="date", size="2", width="100%"),
                        spacing="1", width="100%",
                    ),
                    rx.vstack(
                        rx.text("Rent amount ($)", size="1", color="#666"),
                        rx.input(value=TenantState.s_rent, on_change=TenantState.set_s_rent,
                                 placeholder="0", type="number", size="2", width="100%"),
                        spacing="1", width="100%",
                    ),
                    rx.vstack(
                        ...
                        rx.cond(
                            ...
                            rx.select(TenantState.increase_type_names,
                                      value=TenantState.s_increase_type,
                                      on_change=TenantState.set_s_increase_type,
                                      size="2", width="100%"),
                            rx.text("Loading...", size="2", color="#888"),
                        ),
                        spacing="1", width="100%",
                ...
                rx.vstack(
                    rx.text("Notes", size="1", color="#666"),
                    rx.text_area(value=TenantState.s_notes, on_change=TenantState.set_s_notes,
                                 placeholder="e.g. Annual CPI increase per lease terms",
                                 width="100%", rows="2"),
                    spacing="1", width="100%",
                ),
                rx.cond(TenantState.sched_form_error != "",
                        rx.callout(TenantState.sched_form_error, color="red", variant="soft"),
                        rx.fragment()),
                rx.cond(TenantState.sched_form_success != "",
                        rx.callout(TenantState.sched_form_success, color="green", variant="soft"),
                        rx.fragment()),
                rx.hstack(
                    rx.button(
                        rx.cond(TenantState.sched_mode == "edit", "Save rent row", "Create rent row"),
                        on_click=TenantState.save_sched_row, color_scheme="blue", size="2",
                    ),
                    rx.cond(
                        TenantState.sched_mode == "edit",
                        rx.cond(
                            ...
                            rx.hstack(
                                rx.text("Delete this rent row?", size="2", color="#c62828"),
                                rx.button("Yes, delete", on_click=TenantState.delete_sched_row,
                                          color_scheme="red", size="2"),
                                rx.button("Cancel", on_click=TenantState.cancel_delete_sched,
                                          variant="ghost", size="2"),
                                spacing="2", align="center",
                            ),
                            rx.button("Delete rent row",
                                      on_click=TenantState.confirm_delete_sched_action,
                                      color_scheme="red", variant="outline", size="2"),
                        ),
                        rx.fragment(),
                    ),
                    rx.button("Cancel", on_click=TenantState.new_sched_row, variant="ghost", size="2"),
                    spacing="3", align="center",
                ),
                spacing="4", width="100%", align_items="start",
            ),
