...
class DashboardState(AppState):
    ...
    dash_overdue_work_items: str = "—"
    dash_leases_expiring_60d: str = "—"
    ...
    def reload_on_db_change(self):
        """Called by AppState.toggle_db via yield — reloads all dashboard metrics."""
        self.dash_as_of = datetime.date.today().strftime("%m/%d/%Y")
        self._load_properties()
        self._refresh_all()
    ...
    def _load_operational_metrics(self):
        ...
        rows = run_query(
            ...
            f"WHERE ISNULL(ws.StatusName, w.Status) NOT IN ('Completed','Canceled','Deferred') {prop_clause}",
            tuple(prop_params), db=self.db,
        )
        ...
        rows = run_query(
            ...
            "LEFT JOIN WorkItemStatuses ws ON w.StatusID = ws.WorkItemStatusID "
            "LEFT JOIN Properties p ON w.PropertyID = p.PropertyID "
            f"WHERE ISNULL(ws.StatusName, w.Status) NOT IN ('Completed','Canceled','Deferred') "
            f"AND w.TargetDate IS NOT NULL AND w.TargetDate < ? {prop_clause}",
            tuple([today] + prop_params), db=self.db,
        )
        self.dash_overdue_work_items = str(rows[0]["n"]) if rows else "—"

        rows = run_query(
            "SELECT COUNT(*) AS n FROM Leases l "
            "LEFT JOIN Properties p ON l.PropertyID = p.PropertyID "
            f"WHERE l.LeaseEnd >= ? AND l.LeaseEnd <= ? {prop_clause}",
            tuple([today, sixty_days] + prop_params), db=self.db,
        )
        self.dash_leases_expiring_60d = str(rows[0]["n"]) if rows else "—"
...
def _leases_expiring_card() -> rx.Component:
    card = rx.box(
        rx.vstack(
            rx.hstack(
                rx.text("📄", size="6"),
                rx.text("Leases Expiring", size="2", color="#666"),
                spacing="2", align="center",
            ),
            rx.hstack(
                rx.vstack(
                    rx.text(DashboardState.dash_leases_expiring_60d, size="8",
                            weight="bold", color=BRAND_DARK),
                    rx.text("60d", size="2", color="#666"),
                    spacing="0", align_items="start",
                ),
                rx.divider(orientation="vertical",
                           style={"height": "48px", "margin": "0 24px"}),
                rx.vstack(
                    rx.text(DashboardState.dash_leases_expiring_90d, size="8",
                            weight="bold", color=BRAND_DARK),
                    rx.text("90d", size="2", color="#666"),
                    spacing="0", align_items="start",
                ),
        ...
        style={
            ...
            "cursor": "pointer",
            "_hover": {"border_color": BRAND_PRIMARY,
                       "box_shadow": "0 2px 8px rgba(74,99,168,0.15)"},
        },
    )
    return rx.link(card, href="/leases-expiring", style={"text_decoration": "none"})


def _work_items_card() -> rx.Component:
    card = rx.box(
        rx.vstack(
            # Top row: icon + label
            rx.hstack(
                rx.text("🛠", size="6"),
                rx.text("Work Items", size="2", color="#666"),
                spacing="2",
                align="center",
            ),
            # Bottom row: two stat columns side by side
            rx.hstack(
                # Open count
                rx.vstack(
                    rx.text(DashboardState.dash_open_work_items, size="8",
                            weight="bold", color=BRAND_DARK),
                    rx.text("Open", size="2", color="#666"),
                    spacing="0",
                    align_items="start",
                ),
                rx.divider(orientation="vertical",
                           style={"height": "48px", "margin": "0 16px"}),
                # Overdue count
                rx.vstack(
                    rx.text(DashboardState.dash_overdue_work_items, size="8",
                            weight="bold", color="#c62828"),
                    rx.text("Overdue", size="2", color="#c62828"),
                    spacing="0",
                    align_items="start",
                ),
                spacing="0",
    ...
    return rx.link(card, href="/work-items", style={"text_decoration": "none"})


def _metric_card(label: str, value: rx.Var, icon: str, href: str = "") -> rx.Component:
    card = rx.box(
        rx.vstack(
            rx.text(icon, size="6"),
        ...
        style={
            ...
            "padding": "20px",
            "box_shadow": "0 1px 4px rgba(74,99,168,0.07)",
            "cursor": rx.cond(href != "", "pointer", "default"),
            "_hover": {"border_color": rx.cond(href != "", BRAND_PRIMARY, "#dde3f0"),
                       "box_shadow": rx.cond(href != "", "0 2px 8px rgba(74,99,168,0.15)", "0 1px 4px rgba(74,99,168,0.07)")},
        },
    )
    if href:
        return rx.link(card, href=href, style={"text_decoration": "none"})
    return card
...
def dashboard_content() -> rx.Component:
    return rx.vstack(
        ...
        _rent_roll_card(),
        rx.grid(
            _metric_card("Active tenants", DashboardState.dash_active_tenants, "👥", href="/tenants"),
            _work_items_card(),
            _leases_expiring_card(),
            _metric_card("Overdue follow-ups", DashboardState.dash_overdue_followups, "⏰", href="/follow-ups"),
            columns="4",
            spacing="4",
            width="100%",
        ),
