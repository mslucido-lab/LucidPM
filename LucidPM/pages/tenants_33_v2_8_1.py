...
class TenantState(AppState):
    ...
    @rx.var
    def address_label_options(self) -> list[str]:
        opts = [ADDRESS_LABEL_NONE] + ADDRESS_LABEL_BASE
        if self.a_label and self.a_label not in opts:
            opts.append(self.a_label)
        return opts
...
def addresses_tab() -> rx.Component:
    return rx.vstack(
        ...
        rx.grid(
            _form_field("Label", rx.cond(
                TenantState.address_label_options.length() > 0,
                rx.select(TenantState.address_label_options,
                    value=TenantState.a_label, on_change=TenantState.set_a_label, size="2", width="100%"),
                rx.text(ADDRESS_LABEL_NONE, size="2", color="#666"),
            )),
