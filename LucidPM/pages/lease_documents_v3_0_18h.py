...
def lease_template_section_row(row: LeaseTemplateSectionRow) -> rx.Component:
    return rx.table.row(
        rx.table.cell(
            rx.hstack(
                rx.button("↑", size="1", variant="ghost", color_scheme="gray", on_click=LeaseDocumentState.move_section_up(row.section_id), disabled=~row.can_move_up),
                rx.button("↓", size="1", variant="ghost", color_scheme="gray", on_click=LeaseDocumentState.move_section_down(row.section_id), disabled=~row.can_move_down),
                spacing="1",
            )
        ),
        rx.table.cell(rx.text(row.sort_order, size="2")),
