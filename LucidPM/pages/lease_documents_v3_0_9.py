...
class LeaseDocumentState(AppState):
    ...
    def _find_clause_markers(self, raw: str):
        """Find likely clause starts in real pasted lease text."""
        import re
        text = str(raw or "").replace("\r\n", "\n").replace("\r", "\n")

        # PDF copy/paste sometimes turns clause starts into inline text:
        # "... casualty. 12. In addition ...". Move likely clause starts to a new line.
        text = re.sub(r"(?<!^)\s+((?:[0-9]{1,3})(?:\.[0-9]+)*[\.)])\s+(?=[A-Z])", r"\n\1 ", text)
