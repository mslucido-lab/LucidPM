...
def _work_items_card() -> rx.Component:
    card = rx.box(
        rx.vstack(
            rx.text("🛠", size="6"),
            rx.text(DashboardState.dash_open_work_items, size="8", weight="bold", color=BRAND_DARK),
            rx.text("Open work items", size="2", color="#666"),
            rx.cond(
                DashboardState.dash_overdue_work_items != "0",
                rx.hstack(
                    rx.text(
                        DashboardState.dash_overdue_work_items + " overdue",
                        size="2", weight="bold", color="#c62828",
                    ),
