...
class AdminSettingsState(AppState):
    ...
    def _get_fernet(self) -> Fernet:
        ...
        if rows and rows[0].get("SettingValue"):
            key = str(rows[0].get("SettingValue"))
        else:
            key = Fernet.generate_key().decode("utf-8")
            self._upsert_setting("LocalEncryptionKey", key)
    ...
    def save_email_password(self):
        ...
        if not self.email_password:
            return
        try:
            encrypted = self._encrypt_value(self.email_password)
            run_exec(
    ...
    def save_ai_api_key(self):
        ...
        if not self.ai_api_key:
            return
        try:
            encrypted = self._encrypt_value(self.ai_api_key)
            run_exec(
    ...
    def test_email_connection(self):
        ...
        try:
            ...
            if not pw_rows or not pw_rows[0].get("PasswordEncrypted"):
                ...
                return
            pw = self._decrypt_value(str(pw_rows[0]["PasswordEncrypted"]))
        except Exception as ex:
        ...
        try:
            ...
            imap.login(self.email_username, pw)
            imap.logout()
            results.append("IMAP connected")
        except Exception as ex:
            results.append(f"IMAP failed: {ex}")

        try:
            smtp = smtplib.SMTP(self.email_smtp_server, self.email_smtp_port)
            smtp.starttls()
            smtp.login(self.email_username, pw)
            smtp.quit()
    ...
    def test_ai_connection(self):
        try:
            ...
            if not key_rows or not key_rows[0].get("APIKeyEncrypted"):
                ...
                return
            key = self._decrypt_value(str(key_rows[0]["APIKeyEncrypted"]))
