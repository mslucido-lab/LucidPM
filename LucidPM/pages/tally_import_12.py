...
def _build_address_block(row_norm: dict) -> str:
    ...
    city = _raw_value_by_excel_column(row_norm, "M") or pick(row_norm, "city")
    state = _raw_value_by_excel_column(row_norm, "N") or pick(row_norm, "state")
    zip_code = _raw_value_by_excel_column(row_norm, "O") or pick(row_norm, "zipcode", "zip", "postalcode")

    city_state = ", ".join(part for part in [city, state] if part)
    city_state_zip = " ".join(part for part in [city_state, zip_code] if part).strip()
    return "\n".join(part for part in [line1, city_state_zip] if part)
...
def _note_value(label: str, keys: tuple[str, ...], row_norm: dict) -> str:
    ...
    if label == "Present address":
        return _raw_value_by_excel_column(row_norm, "P") or pick(row_norm, *keys)
...
NOTE_FIELD_DEFS: list[tuple[str, tuple[str, ...], str]] = [
    ...
    _note_field("Address", "addressline1", "address1", "streetaddress", "city", "state", "zipcode", "zip"),
