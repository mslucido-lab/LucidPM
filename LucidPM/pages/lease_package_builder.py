...
# v3.0.16 - Collapses template section list by default so Generate stays visible.
# v3.0.18 - Responsive lease package builder width and wrapping layout.
# v3.0.17 - Adds applicant screening gate before lease package generation.
# v3.0.16a - Keeps collapsible package sections list, no backend generation changes.
# v3.0.15 - Removes stray preview diagnostic block from generation loop.
# v3.0.14 - Fixes mixed-mode diagnostic variable reference in generation loop.
# v3.0.13 - Restores mixed mode: only {{SectionNumber}} sections use dynamic numbering; non-token sections keep authored/metadata numbers.
# v3.0.12 - Applies dynamic numbering to all numbered content sections, even when content lacks {{SectionNumber}}.
# v3.0.11 - Rebuilds dynamic SectionNumber headings robustly so numbered sections cannot disappear.
# v3.0.10 - Increments SectionNumber only when the rendered output visibly starts with the expected number.
# v3.0.9 - Counts only sections using {{SectionNumber}} and converts dynamic markers into proper heading lines.
# v3.0.8 - Excludes Header and lettered child sections from SectionNumber counter and injects DisplayLabel after dynamic section numbers.
# v3.0.7 - Suppresses metadata heading injection when content uses dynamic {{SectionNumber}}.
# v3.0.6 - Adds dynamic {{SectionNumber}} token injection for sequential content-section numbering.
# v3.0.5f - strips inline ReportLab tags before detecting lettered child conditions so <b>a.</b> still suppresses 22a headings.
# v3.0.5e - supports Special Conditions header-only sections and suppresses auto headings for lettered child conditions.
...
# Dynamic page width, matches Communications responsive page pattern.
# page_shell uses a resizable sidebar and 32px left/right content padding.
...
class LeasePackageBuilderState(AppState):
    ...
    # Applicant screening gate
    screening_gate_status: str = ""
    screening_override_acknowledged: bool = False
    ...
    @rx.var
    def template_sections_summary_text(self) -> str:
        if self.template_sections:
            template_name = str(self.selected_package_template_label or "selected template").strip() or "selected template"
            return f"{len(self.template_sections):,} sections loaded - {template_name}"
    ...
    def package_summary_lines(self) -> list[str]:
        ...
        for section in included_rows:
            ...
            if sid <= 0:
                ...
                continue
            if section_data and self._has_renderable_text(section_data):
                content_count += 1
                kind = "Content"
            else:
                pdf_count += 1
    ...
    def set_selected_tenant(self, label: str):
        ...
        self.merge_missing_tokens = []
        self.merge_known_tokens = []
        self.screening_gate_status = ""
        self.screening_override_acknowledged = False
        self._load_leases_for_selected_tenant()
    ...
    def expand_template_sections(self):
        self.sections_expanded = True

    def hide_template_sections(self):
        self.sections_expanded = False
    ...
    def set_screening_override_acknowledged(self, v: bool):
        self.screening_override_acknowledged = v

    def check_screening_gate(self):
        self.screening_gate_status = ""
        tenant_id = self._selected_tenant_id()
        if tenant_id <= 0:
            return
        try:
            status_rows = run_query(
                "SELECT ISNULL(s.TenantStatusName, '') AS TenantStatusName "
                "FROM Tenants t LEFT JOIN TenantStatuses s ON t.TenantStatusID = s.TenantStatusID "
                "WHERE t.TenantID = ?",
                (tenant_id,), db=self.db,
            )
            status_name = str(status_rows[0].get("TenantStatusName") or "") if status_rows else ""
            if status_name != "Applicant":
                self.screening_gate_status = "clear"
                return

            screening_rows = run_query(
                "SELECT TOP 1 OverallResult FROM TenantScreenings "
                "WHERE TenantID = ? AND CompletedDate IS NOT NULL "
                "ORDER BY CreatedDate DESC, TenantScreeningID DESC",
                (tenant_id,), db=self.db,
            )
            if not screening_rows:
                self.screening_gate_status = "warn_no_screening"
                return
            result = str(screening_rows[0].get("OverallResult") or "").strip()
            if result.lower() == "fail":
                self.screening_gate_status = "warn_fail"
                return
            self.screening_gate_status = "clear"
        except Exception as ex:
            self.screening_gate_status = "warn_no_screening"
            self.form_error = f"Could not check applicant screening status: {ex}"

    def _screening_gate_blocks_generation(self) -> bool:
        return self.screening_gate_status in ("warn_no_screening", "warn_fail") and not self.screening_override_acknowledged
    ...
    def _section_heading_text(self, section: LeasePackageSection) -> str:
        """Build the auto heading for a section from ArticleNumber / DisplayLabel metadata."""
        ...
        if article and label:
            ...
            if article.endswith(".") or article.endswith(")") or "." in article:
                return f"{article} {label}"
            return f"{article}. {label}"
        return article or label

    def _is_lettered_child_condition(self, section: LeasePackageSection, body: str) -> bool:
        """Return True for Special Conditions child rows like 22a / 21b.

        These child sections already start their body with the visible marker
        such as "a." or "b.". Do not prepend the metadata heading
        again, because that produces duplicate labels like "22a" above
        "a. Tenant shall...".
        """
        article = str(getattr(section, "article_number", "") or "").strip()
        label = str(getattr(section, "display_label", "") or "").strip()
        # The renderer supports ReportLab inline markup such as <b>a.</b>.
        # Strip tags for detection only so formatting does not prevent
        # suppression of metadata headings like 22a / 22b.
        clean_body = re.sub(r"<[^>]+>", "", str(body or "")).lstrip()

        marker = ""
        match = re.match(r"^([0-9]{1,3})([a-z])$", article, flags=re.IGNORECASE)
        if match:
        ...
        if not marker:
            return False
        return bool(re.match(rf"^{re.escape(marker)}[\.)]\s+", clean_body, flags=re.IGNORECASE))

    def _has_renderable_text(self, section: LeasePackageSection) -> bool:
        """Return True when a section should flow through the text renderer.

        Header-only sections are valid when they have ArticleNumber or
        DisplayLabel metadata but no Content and no static PDF file.
        """
        if str(getattr(section, "content", "") or "").strip():
            return True
        if os.path.isfile(str(getattr(section, "file_path", "") or "")):
            return False
        return bool(self._section_heading_text(section))

    def _uses_dynamic_section_number(self, section: LeasePackageSection) -> bool:
        """Return True when authored content uses the dynamic SectionNumber token."""
        raw_content = str(getattr(section, "content", "") or "")
        return bool(re.search(r"\{\{\s*SectionNumber\s*\}\}", raw_content))

    def _section_consumes_section_number(
        self,
        section: LeasePackageSection,
        rendered_text: str,
        expected_number: int | None = None,
    ) -> bool:
        """Return True when this section should consume the dynamic counter.

        Mixed mode rule:
        - Sections containing {{SectionNumber}} consume the dynamic counter.
        - Sections without {{SectionNumber}} keep their authored/metadata number
          and do not affect the dynamic counter.
        - Header/opening sections, static PDFs, and lettered child rows do not
          consume the dynamic counter.
        """
        if not self._uses_dynamic_section_number(section):
            return False

        section_type = str(getattr(section, "section_type", "") or "").strip().lower()
        if section_type in ("header", "cover", "cover page"):
            return False

        if self._is_lettered_child_condition(section, rendered_text):
            return False

        body = re.sub(r"<[^>]+>", "", str(rendered_text or "")).lstrip()
        if not body:
            # Dynamic section exists but rendered blank. Do not consume the
            # counter. The generated output will make the authoring issue visible.
            return False

        if expected_number is not None:
            return bool(re.match(rf"^{int(expected_number)}[\.)]\s*", body))

        return bool(re.match(r"^\d+[\.)]\s*", body))

    def _strip_opening_number_marker(self, body: str) -> tuple[str, str]:
        """Remove only the authored opening number marker from rendered text.

        This deliberately does not try to parse a title from the authored line.
        The visible heading title comes from DisplayLabel, then SectionName.
        This prevents malformed tag patterns from swallowing an entire clause.
        """
        raw = str(body or "").strip()
        if not raw:
            return "", ""

        # Handles these common authored forms after token replacement:
        #   <b>4.</b> Body
        #   <b>4.</b><br/>Body
        #   <b>4.</b>
        #   Body
        wrapped = re.match(
            r"^\s*(?:<b>|<strong>)?\s*(?P<num>\d+[\.)])\s*(?:</b>|</strong>)?\s*(?P<rest>.*)$",
            raw,
            flags=re.IGNORECASE | re.DOTALL,
        )
        if wrapped:
            rest = str(wrapped.group("rest") or "").lstrip()
            rest = re.sub(r"^(?:<br\s*/?>\s*)+", "", rest, flags=re.IGNORECASE).lstrip()
            return wrapped.group("num"), rest

        plain = re.match(r"^\s*(?P<num>\d+[\.)])\s*(?P<rest>.*)$", raw, flags=re.DOTALL)
        if plain:
            return plain.group("num"), str(plain.group("rest") or "").lstrip()

        return "", raw

    def _inject_display_label_after_section_number(self, section: LeasePackageSection, rendered_text: str) -> str:
        """Convert an authored {{SectionNumber}} marker into a visible heading.

        ArticleNumber can stay populated for admin sorting. Visible numbering is
        driven by {{SectionNumber}} in the content. The heading title is pulled
        from DisplayLabel first, then SectionName. The remaining body always
        stays below the heading.
        """
        body = str(rendered_text or "").strip()
        if not body:
            return body

        number_marker, remaining = self._strip_opening_number_marker(body)
        if not number_marker:
            return body

        heading_title = (
            str(getattr(section, "display_label", "") or "").strip()
            or str(getattr(section, "section_name", "") or "").strip()
            or str(getattr(section, "template_section_label", "") or "").strip()
        )

        heading = f"<b>{number_marker} {heading_title}</b>" if heading_title else f"<b>{number_marker}</b>"
    ...
    def _compose_section_render_text(
        self,
        section: LeasePackageSection,
        rendered_text: str,
        dynamic_number: int | None = None,
    ) -> str:
        """Prepend or normalize legal headings when appropriate.

        Mixed mode rule:
        - If content contains {{SectionNumber}}, visible heading is rebuilt
          from the dynamic counter plus DisplayLabel/SectionName.
        - If content does not contain {{SectionNumber}}, legacy metadata
          heading behavior remains unchanged.
        """
        body = str(rendered_text or "").strip()

        if self._is_lettered_child_condition(section, body):
            return body

        if self._uses_dynamic_section_number(section):
            return self._inject_display_label_after_section_number(section, body)

        heading = self._section_heading_text(section)
        if not heading:
            return body
        ...
        if body_lower.startswith(heading.lower()):
            return body
    ...
    def generate_package(self):
        ...
        self.check_screening_gate()
        if self._screening_gate_blocks_generation():
            if self.screening_gate_status == "warn_fail":
                self.form_error = "Applicant screening result is Fail. Check the override box to generate anyway."
            else:
                self.form_error = "Applicant has no completed screening on file. Check the override box to generate anyway."
            return
        ...
        missing_sources = [
            ...
            if not self._has_renderable_text(p) and not os.path.isfile(p.file_path)
        ]
        if missing_sources:
        ...
        try:
            ...
            for idx, p in enumerate(selected, start=1):
                ...
                if self._has_renderable_text(p):
                    ...
                    consumes_section_number = self._section_consumes_section_number(
                        p,
                        rendered_text,
                        section_number,
                    )
                    rendered_text = self._compose_section_render_text(
                        p,
                        rendered_text,
                        section_number if consumes_section_number else None,
                    )
                    pending_text_sections.append(rendered_text)
                    if consumes_section_number:
                        section_number += 1
                    elif self._uses_dynamic_section_number(p):
                        label = p.template_section_label or p.section_name or f"Section {p.section_id}"
                        raise RuntimeError(
                            f"{label} contains {{SectionNumber}} but did not render a visible dynamic heading. "
                            "Check that the content starts with {{SectionNumber}} or a supported inline wrapper such as <b>{{SectionNumber}}.</b>."
                        )
        ...
        try:
            ...
            for idx, section in enumerate(selected, start=1):
                ...
                if self._has_renderable_text(section):
                    section_context = {**context, "SectionNumber": str(section_number)}
                    rendered_text, unresolved = render_text_template(content, section_context) if content else ("", [])
                    validation = validate_template_tokens(content, context) if content else {"tokens": []}
            ...
            if self.merge_missing_tokens:
                ...
                self.merge_error = "Preview generated. No merge tokens were found in the selected text sections."
        except Exception as ex:
        ...
        try:
            ...
            for idx, row in enumerate(rows, start=1):
                ...
                temp_section_meta = LeasePackageSection(
                    template_section_label=label,
                    section_name=str(row.get("SectionName") or label),
                ...
                if text_to_render or self._has_renderable_text(temp_section_meta):
                    content_count += 1
                    validation = validate_template_tokens(text_to_render, context) if text_to_render else {"missing": []}
                    unresolved = validation.get("missing", []) or []
                    if unresolved:
                        errors.append(f"{label}: " + ", ".join(sorted(set(unresolved))[:10]))
        ...
        try:
            ...
            for idx, row in enumerate(rows, start=1):
                ...
                if text_to_render or self._has_renderable_text(temp_section_meta):
                    ...
                    section_context = {**context, "SectionNumber": str(section_number)}
                    rendered_text, unresolved_after_render = render_text_template(text_to_render, section_context) if text_to_render else ("", [])
                    consumes_section_number = self._section_consumes_section_number(
                        temp_section_meta,
                        rendered_text,
                        section_number,
                    ...
                    if consumes_section_number:
                        section_number += 1
                    elif self._uses_dynamic_section_number(temp_section_meta):
                        errors.append(
                            f"{label}: contains {{SectionNumber}} but did not render a visible dynamic heading"
                        )
                        continue
    ...
    return rx.box(
        rx.vstack(
            ...
            rx.grid(
                ...
                spacing="4",
                width="100%",
                style=RESPONSIVE_COMPACT_GRID_STYLE,
            ),
            spacing="3",
            width="100%",
            align_items="start",
...
def screening_gate_panel() -> rx.Component:
    return rx.cond(
        LeasePackageBuilderState.screening_gate_status == "warn_no_screening",
        rx.box(
            rx.vstack(
                rx.callout("Applicant has no completed screening on file. Lease generation is blocked unless you acknowledge the override.", color_scheme="orange", variant="soft"),
                rx.hstack(
                    rx.checkbox(checked=LeasePackageBuilderState.screening_override_acknowledged, on_change=LeasePackageBuilderState.set_screening_override_acknowledged),
                    rx.text("I understand and want to generate the lease package anyway.", size="2"),
                    align="center", spacing="2",
                ),
                spacing="2", width="100%", align_items="start",
            ),
            style={"background": "#fffaf0", "border": "1px solid #f2d28b", "border_left": "4px solid #f59e0b", "border_radius": "10px", "padding": "12px", "width": "100%"},
        ),
        rx.cond(
            LeasePackageBuilderState.screening_gate_status == "warn_fail",
            rx.box(
                rx.vstack(
                    rx.callout("Applicant screening result is Fail. Lease generation is blocked unless you acknowledge the override.", color_scheme="red", variant="soft"),
                    rx.hstack(
                        rx.checkbox(checked=LeasePackageBuilderState.screening_override_acknowledged, on_change=LeasePackageBuilderState.set_screening_override_acknowledged),
                        rx.text("I understand and want to generate the lease package anyway.", size="2"),
                        align="center", spacing="2",
                ...
                style={"background": "#fff5f5", "border": "1px solid #fecaca", "border_left": "4px solid #dc2626", "border_radius": "10px", "padding": "12px", "width": "100%"},
            ),
            rx.fragment(),
        ),
    )
            ...
            rx.vstack(
                ...
                rx.cond(
                    ...
                    rx.box(
                        rx.vstack(
                            rx.hstack(
                                rx.text(LeasePackageBuilderState.template_sections_summary_text, size="2", weight="bold", color=BRAND_DARK),
                                rx.spacer(),
                                rx.cond(
                                    LeasePackageBuilderState.sections_expanded,
                                    rx.button("▲ Hide sections", size="1", variant="ghost", color_scheme="gray", on_click=LeasePackageBuilderState.hide_template_sections),
                                    rx.button("▼ Review sections", size="1", variant="ghost", color_scheme="blue", on_click=LeasePackageBuilderState.expand_template_sections),
                                ),
                                width="100%",
                                align="center",
                            ),
                        ...
                        style={"background": "#f8faff", "border": "1px solid #d8e1f5", "border_radius": "8px", "padding": "12px", "width": "100%"},
                    ),
                    rx.fragment(),
                ),
                spacing="4",
