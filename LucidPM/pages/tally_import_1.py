...
def preview_rows(rows: list[dict]) -> list[dict]:
    ...
    for r in rows:
        out.append({
            "Name":     pick(r, "individualname", "fullname", "applicantname", "name"),
            "Business": pick(r, "businessname", "companyname"),
            "Email":    pick(r, "email", "businessemail"),
            "Phone":    pick(r, "phone", "businessphone", "phonenumber"),
...
def _get_prospect_status_id(db: str) -> Optional[int]:
    rows = run_query(
        "SELECT TenantStatusID FROM TenantStatuses WHERE TenantStatusName = 'Prospect'",
        db=db,
    )
...
def _get_matching_prospect(phone: str, email: str, db: str) -> Optional[dict]:
    """Match existing Prospect by phone (digits only) then email."""
    rows = run_query(
        ...
        "FROM Prospects WHERE ConvertedTenantID IS NULL "
...
def _build_import_notes(row_norm: dict) -> str:
    mapping = [
        ("Applicant",          pick(row_norm, "individualname", "fullname", "applicantname", "name")),
        ("Business",           pick(row_norm, "businessname", "companyname")),
        ("Business email",     pick(row_norm, "businessemail")),
        ...
        ("Owner name",         pick(row_norm, "nameofowner", "ownername")),
        ("Nature of business", pick(row_norm, "natureofthebusiness", "natureofbusiness", "businessdescription")),
        ("Annual sales",       pick(row_norm, "estimatedannualsales", "annualsales")),
        ("Bankruptcy",         pick(row_norm, "haveyoueverfiledforbankruptcy", "bankruptcy")),
        ("Remarks",            pick(row_norm, "additionalremarksandinformation", "additionalremarks", "remarks", "notes")),
...
def import_tally_row(row_norm: dict, db: str) -> tuple[str, Optional[int]]:
    ...
    """
    applicant_name   = pick(row_norm, "individualname", "fullname", "applicantname", "name")
    business_name    = pick(row_norm, "businessname", "companyname")
    email            = pick(row_norm, "email", "businessemail")
    phone            = pick(row_norm, "phone", "businessphone", "phonenumber")
    ssn              = pick(row_norm, "socialsecuritynumber", "ssn", "socsecno", "socialsecurity")
    dl_number        = pick(row_norm, "driverlicensenumberstate", "driverslicensenumberstate",
                            "driverlicense", "dlnumberstate", "dlnostate")
    dob_raw          = pick(row_norm, "dateofbirth", "dob", "birthdate")
    ...
    if prospect_status_id is None:
        raise RuntimeError("Prospect status not found. Add 'Prospect' to TenantStatuses before importing.")
    ...
    matched_prospect = _get_matching_prospect(phone, email, db)
