                ...
                if lease_end is not None and lease_end < today:
                    ...
                    if today <= ann <= horizon:
                        if lease_end is None or ann < lease_end:
                            lid = int(lease_id)
                            prev_day = ann - datetime.timedelta(days=1)
                            current_rent, _ = _rent_as_of(sched, lid, prev_day)
...
def _event_row(row: LeaseEventRow) -> rx.Component:
    bg, color = ("#fce4ec", "#b71c1c") if row.is_expiration else ("#e8f5e9", "#1b5e20")
    return rx.table.row(
        rx.table.cell(rx.text(row.event_date, size="2", weight="bold", color="#333")),
        rx.table.cell(
            rx.box(
                rx.text(row.event_type, size="1", weight="bold", style={"color": color}),
