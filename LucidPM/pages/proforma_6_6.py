...
v6.4 — Suppresses recommended rent during known lease gaps between a terminated lease and a future lease.

v6.5 — Displays true zero-dollar proforma values as $0 instead of blank.

v6.6 — Applies LeaseStart/RentDueDay proration rules and gives new leases starting in-month precedence over prior leases.
"""
    ...
    def _calc_month_amount(self, segments: list[dict], year: int, month: int, lease_start: datetime.date = None, rent_due_day: int = None) -> float:
        ...
        for seg in valid:
            ...
            if month_start <= seg["start"] <= month_end:
                ...
                # Proration is driven by LeaseStart day vs RentDueDay.
                # If the lease starts on its rent due day, treat the first month
                # as a full contractual billing period. Otherwise prorate from
                # LeaseStart through month end, or the segment end if earlier.
                if rent_due_day is not None and ls.day == int(rent_due_day):
                    return round(rent, 2)
    ...
    def _do_compute(self):
        ...
        for month_idx, month_name in enumerate(MONTH_NAMES):
            ...
            for suite in suites:
                ...
                def effective_lease_end(lease: dict) -> datetime.date:
                    end_date = to_date(lease.get("LeaseEnd")) or datetime.date(9999, 12, 31)
                    if str(lease.get("LeaseStatus") or "").strip() == "Terminated":
                        term_date = to_date(lease.get("LeaseTerminationDate"))
                        if term_date is not None:
                            end_date = min(end_date, term_date)
                    return end_date

                # If an executed lease starts within this month, it controls the
                # month. Do not blend the prior lease, recommended rent, or gap
                # logic into the same month. Proration/full-month treatment is
                # handled by _calc_month_amount using LeaseStart and RentDueDay.
                starting_this_month = []
                for l in suite_leases:
                    ls = to_date(l.get("LeaseStart"))
                    if ls is None:
                        continue
                    le = effective_lease_end(l)
                    if month_start <= ls <= month_end and le >= ls:
                        starting_this_month.append(l)

                controlling_leases = starting_this_month
                if controlling_leases:
                    ...
                    for l in suite_leases:
                        ...
                        if ls <= month_end and le >= month_start:
                            controlling_leases.append(l)

                for l in controlling_leases:
                    ls = to_date(l.get("LeaseStart"))
                    if ls is None:
                        continue
                    active_found = True
                ...
                if not active_found:
                    # If this month falls in a known gap between a terminated lease and
                    # a future lease for the same suite, show $0. We are not collecting
                    # rent during that gap, so do not apply recommended rent.
                    in_known_gap = False
                    for prior_lease in suite_leases:
                        prior_start = to_date(prior_lease.get("LeaseStart"))
                        prior_end = to_date(prior_lease.get("LeaseTerminationDate")) if str(prior_lease.get("LeaseStatus") or "").strip() == "Terminated" else to_date(prior_lease.get("LeaseEnd"))
                        if prior_start is None or prior_end is None or prior_end >= month_start:
                            continue
                        for future_lease in suite_leases:
                            future_start = to_date(future_lease.get("LeaseStart"))
                            if future_start and future_start > month_end and prior_end < future_start:
                                in_known_gap = True
                                break
                        if in_known_gap:
                            break
