...
(function() {
    if (window.__lucidInboxResizerBound) {
        return;
    }
    window.__lucidInboxResizerBound = true;
    ...
    function isResizerTarget(target) {
        return target && target.closest && target.closest('#inbox-panel-resizer');
    }

    function startResize(e) {
        if (!isResizerTarget(e.target)) {
            return;
        }
...
def communications_content() -> rx.Component:
    return rx.box(
        ...
        min_width="calc(100vw - 284px)",
        max_width="calc(100vw - 284px)",
        flex_shrink="0",
        style={
