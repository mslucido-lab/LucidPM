...
from LucidPM_Reflex.pages.communications_report import communications_report_page, CommReportState
from LucidPM_Reflex.pages.lease_documents import lease_documents_page, LeaseDocumentState
from LucidPM_Reflex.pages.settings import settings_page, SettingsState
from LucidPM_Reflex.pages.lease_documents_pdf import is_safe_document_path
...
async def lease_generated_pdf_endpoint(request: Request):
    """Download a generated lease PDF package by LeaseGeneratedDocumentID."""
    params = request.query_params
    db = params.get("db", TEST_DB_NAME)
    # Accept both generated_id and legacy id so older page links still work.
    raw_generated_id = params.get("generated_id") or params.get("id") or "0"
    try:
        generated_id = int(raw_generated_id)
    except (TypeError, ValueError):
        generated_id = 0
    if generated_id <= 0:
        return Response(content="Missing generated_id", status_code=400)

    rows = run_query(
        "SELECT GeneratedFileName, StoredFilePath FROM LeaseGeneratedDocuments WHERE LeaseGeneratedDocumentID = ?",
        (generated_id,), db=db,
    )
    if not rows:
        return Response(content="Generated lease PDF not found", status_code=404)

    filename = str(rows[0].get("GeneratedFileName") or "lease_package.pdf")
    path = str(rows[0].get("StoredFilePath") or "")
    if not is_safe_document_path(path):
        return Response(content="Unsafe file path", status_code=403)
    if not os.path.isfile(path):
        return Response(content="File not found on disk", status_code=404)
...
app.add_page(communications_report_page, route="/communications-report", on_load=CommReportState.on_load)
...
app.add_page(settings_page, route="/admin/settings", on_load=SettingsState.on_load)
app.add_page(lease_documents_page, route="/lease-documents", on_load=LeaseDocumentState.on_load)
