...
def application_report_pdf(tenant_id: int, db: str = "TenantCRM", background_tasks: BackgroundTasks = None):
    import os as _os
    import tempfile
    ...
    safe_name = str(tenant.get("TenantName", "Applicant")).replace(" ", "_").replace("/", "_")
    if background_tasks is not None:
        background_tasks.add_task(_os.unlink, output_path)
    return FileResponse(
        output_path,
        media_type="application/pdf",
        filename=f"Application_{safe_name}.pdf",
    )
