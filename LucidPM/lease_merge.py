...
def _lease_term_words(months: int) -> str:
    """Return written-out month count in legal format: 'six (6) months'"""
    _num_words = {
...
def _lease_term_description(start: dt.date | None, end: dt.date | None) -> str:
    ...
    return _lease_term_words(months)


def _lease_term_months(start: dt.date | None, end: dt.date | None) -> int:
    """Return lease term length in months using the same inclusive rule as _lease_term_description()."""
    if not start or not end:
        return 0
...
def _extension_term_months_from_lease(start: dt.date | None, end: dt.date | None) -> int:
    """Infer renewal extension months when no explicit option schedule row exists.

    Business rule:
      - If the lease term is under 12 months, extension equals the lease term.
      - If the lease term is 12 months or more, extension is 12 months.
    """
    lease_months = _lease_term_months(start, end)
    if lease_months <= 0:
        return 0
    return lease_months if lease_months < 12 else 12


def _get_option_schedule_row(lease_id: int, db: str) -> dict:
    """Return the first LeaseRentSchedule row tagged as an option term increase."""
    ...
    rows = run_query(
        "SELECT TOP 1 lrs.EffectiveStartDate, lrs.EffectiveEndDate, lrs.RentAmount, "
        f"{increase_select} "
        "FROM LeaseRentSchedule lrs "
        f"{increase_join} "
        "WHERE lrs.LeaseID = ? "
        "AND UPPER(LTRIM(RTRIM(" + increase_select.split(' AS ')[0] + "))) = 'OPTION TERM INCREASE' "
...
def _ordinal_word(n: int) -> str:
    """Return ordinal word for amendment numbering: 1 -> FIRST, 2 -> SECOND, etc."""
    words = {
...
def _short_date_no_leading_zero(value: Any) -> str:
    d = _date(value)
...
def _load_rent_schedule_rows(lease_id: int, db: str) -> list[dict]:
    ...
    # PaymentScheduleBlock and TotalRent must exclude renewal option rows explicitly.
    # Do not rely on LeaseEnd date clipping because option rows can be malformed,
    # overlapping, or future-dated. _get_option_schedule_row() remains responsible
    # for reading Option Term Increase rows for renewal tokens.
    increase_join = ""
    option_filter = ""
    ...
    if "IncreaseTypeID" in cols and _table_exists("LeaseRentIncreaseTypes", db):
        ...
        lookup_name = "IncreaseTypeName" if "IncreaseTypeName" in lookup_cols else "IncreaseType"
        increase_join = f"LEFT JOIN LeaseRentIncreaseTypes rit ON lrs.IncreaseTypeID = rit.{lookup_pk} "
        option_filter = f"AND UPPER(LTRIM(RTRIM(ISNULL(rit.{lookup_name}, '')))) <> 'OPTION TERM INCREASE' "
    elif "IncreaseType" in cols:
        option_filter = "AND UPPER(LTRIM(RTRIM(ISNULL(lrs.IncreaseType, '')))) <> 'OPTION TERM INCREASE' "
    ...
    return run_query(
        ...
        "WHERE lrs.LeaseID = ? "
        f"{option_filter}"
        f"ORDER BY lrs.EffectiveStartDate, lrs.{order_col}",
        (lease_id,),
        db=db,
    )
...
def get_lease_merge_context(tenant_id: int, lease_id: int, db: str = TEST_DB_NAME) -> dict[str, str]:
    ...
    if parent_lease_id:
        count_rows = run_query(
            "SELECT COUNT(*) AS AmendmentCount FROM Leases "
            "WHERE ParentLeaseID = ? AND LeaseID <= ?",
            (int(parent_lease_id), int(lease_id)),
            db=db,
        )
        amendment_number = count_rows[0].get("AmendmentCount", 1) if count_rows else 1
        amendment_number_word = _ordinal_word(int(amendment_number or 1))

    prior_amendments_text = ""
    if parent_lease_id:
        prior_rows = run_query(
            "SELECT LeaseID, LeaseStart FROM Leases "
            "WHERE ParentLeaseID = ? AND LeaseID < ? "
            "ORDER BY LeaseID",
            (int(parent_lease_id), int(lease_id)),
            db=db,
        ...
        for i, row in enumerate(prior_rows, start=1):
            label = _ordinal_word(i).title()
            row_start = _date(row.get("LeaseStart"))
            if row_start:
                prior_parts.append(
                    f'effective date of {_long_date(row_start)} (the "{label} Amendment")'
                )
            else:
                prior_parts.append(
                    f'the "{label} Amendment" [EFFECTIVE DATE MISSING - LeaseID {row.get("LeaseID")}]'
                )

        if prior_parts:
            if len(prior_parts) == 1:
                joined = prior_parts[0]
            elif len(prior_parts) == 2:
                joined = f"{prior_parts[0]} and {prior_parts[1]}"
            else:
                joined = ", ".join(prior_parts[:-1]) + ", and " + prior_parts[-1]
            prior_amendments_text = f', and previously amended with {joined}'
    ...
    option_schedule_row = _get_option_schedule_row(lease_id, db)
    option_start = _date(option_schedule_row.get("EffectiveStartDate"))
    option_end = _date(option_schedule_row.get("EffectiveEndDate"))
    option_term_months = _lease_term_months(option_start, option_end) if option_schedule_row else 0
    if option_term_months <= 0:
    ...
    context = {
        ...
        # Long-form lease start date for formal clause text (e.g. "March 1, 2026")
        "LeaseStartLong": f"{lease_start.strftime('%B')} {lease_start.day}, {lease_start.year}" if lease_start else "",
        ...
        # Long-form lease end date for formal clause text (e.g. "March 1, 2026")
        "LeaseEndLong": f"{lease_end.strftime('%B')} {lease_end.day}, {lease_end.year}" if lease_end else "",
        ...
        # Prefer an explicit LeaseRentSchedule row tagged "Option Term Increase".
        # If no option row exists, infer only the term from the lease length and leave rent blank.
        "ExtensionTermMonths": str(option_term_months) if option_term_months else "",
        "ExtensionTermDescription": _lease_term_words(option_term_months) if option_term_months else "",
        ...
        "AmendmentNumberProperCase": amendment_number_word.title() if amendment_number_word else "",
        "AmendmentEffectiveDate": _long_date(lease_start) if parent_lease_id else "",
        "AmendmentEffectiveDateLong": _long_date(lease_start) if parent_lease_id else "",
        "AmendmentEffectiveDateProperCase": _long_date(lease_start) if parent_lease_id else "",
        "AmendmentEffectiveDateUpper": _long_date(lease_start).upper() if parent_lease_id and lease_start else "",
        "AmendmentEndDate": _short_date_no_leading_zero(lease_end) if parent_lease_id else "",
        "AmendmentEndDateLong": _long_date(lease_end) if parent_lease_id else "",
    ...
    context["OriginalLeaseEffectiveDate"] = context["OriginalLeaseStartDate"]
    ...
    # Amendment extension term sentence — only resolves on amendment leases (ParentLeaseID set)
    if parent_lease_id and lease_start and lease_end and context.get("LeaseTermDescription") and use_type:
        context["AmendmentTermBlock"] = (
            f"{context['LeaseTermDescription']}, "
            f"beginning the {_ordinal(lease_start.day)} day of "
            f"{lease_start.strftime('%B')}, {lease_start.year} "
            f"(the \u201cExtension Date\u201d), "
            f"and ending the {_ordinal(lease_end.day)} day of "
            f"{lease_end.strftime('%B')}, {lease_end.year} "
            f"to be occupied as {use_type}"
        )
    else:
        context["AmendmentTermBlock"] = ""
