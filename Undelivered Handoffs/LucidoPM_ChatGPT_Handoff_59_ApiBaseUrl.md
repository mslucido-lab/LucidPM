...
*Prepared: 2026-09-07 · Azure Migration — Stage 3 prerequisite (does not need Azure)*

---

## What This Is

Nine live page modules build absolute download URLs to the Reflex **backend**
...
- Change which database a link targets — the `&db=` param is passed through
  unchanged (except the `tenants.py` literal swap in Step 11, which is a no-op
  today).
...
**Files that change:** `LucidPM/state.py` (+1 helper) and 9 page modules under
`LucidPM/pages/` (import + f-string swaps, ~12 lines total).
...
| File | Line(s) | `@rx.var` | Endpoint |
|---|---|---|---|
| `pages/communications.py` | 1694 | `pdf_url` | `/api/communications-pdf` |
| `pages/communications_report.py` | 321 | `pdf_url` | `/api/communications-pdf` |
| `pages/leases_expiring.py` | 539 | `pdf_url` | `/api/leases-expiring-pdf` |
| `pages/lease_package_builder.py` | 364, 370 | `generated_download_url`, `selected_generated_download_url` | `/api/lease-generated-pdf` |
| `pages/lease_package_builder.py` | 1825 | (row build in `_load_generated_packages`) | `/api/lease-generated-pdf` |
| `pages/proforma.py` | 85, 93 | `pdf_download_url`, `bank_package_url` | `/api/proforma-pdf`, `/api/bank-package-pdf` |
| `pages/property_financials.py` | 91 | `pdf_download_url` | `/api/property-financials-pdf` |
| `pages/rent_roll.py` | 336 | `pdf_download_url` | `/api/rent-roll-pdf` |
| `pages/tenants.py` | 704 | `application_report_url` | `/api/application-report-pdf` |

All nine already have `import reflex as rx` and a `from LucidPM.state import …`.
Each site is a plain Python f-string inside an `@rx.var` body — evaluated on the
backend, rendered by the frontend as an `href`.
...
f-string(s). The `/api/...` path and every query param stay exactly as they are.
...
**Step 3 — `pages/communications_report.py`**

Import (line ~18): add `api_base_url` to the block (currently
...
**Step 4 — `pages/leases_expiring.py`**
...
**Step 5 — `pages/lease_package_builder.py`**
...
**Step 7 — `pages/property_financials.py`**
...
```
→ add `PROD_DB_NAME` (Step 11) **and** `api_base_url`:
```python
from LucidPM.state import (
    AppState, run_query, run_exec, fmt_date, resolve_upload_filename,
...
### Step 10 — `pages/lease_documents.py` comment only
...
### Step 11 — `pages/tenants.py`: the two-line var (H58 review finding 2)
...
**Local, default ports (`reflex run`) — must be identical to today:**

- [ ] `reflex run` compiles; no import error from the new `api_base_url` symbol.
- [ ] Rent Roll → "Download PDF" opens the rent-roll PDF (URL in the browser is
      `http://localhost:8000/api/rent-roll-pdf?...`, unchanged).
- [ ] Repeat for: Proforma (PDF **and** Bank Package), Property Financials,
      Leases Expiring, Communications report, a Tenant's Application Report,
      and a generated Lease Package download (both the just-generated link and a
      row in the generated-packages list).
- [ ] `grep -rn "localhost:8000" LucidPM/*.py LucidPM/pages/*.py` returns **only**
      `_vN`/numbered siblings — zero hits in the 9 live files or their comments.

**Local, non-default backend port (proves the fix):**

- [ ] `reflex run --frontend-port 3002 --backend-port 8002`, open the app on
      3002. Every download link above now points at `http://localhost:8002/...`
...
**Config override (simulates the Stage 3 deployment):**

- [ ] `REFLEX_API_URL=https://example.test reflex run` → a download link's
      `href` is `https://example.test/api/...` (it will not resolve; you are
      only checking the base is substituted, not hardcoded).
...
1. Apply Steps 1–11.
2. Run the **default-ports** checklist first (the "did we change local
   behaviour" gate — the URLs must be character-for-character the same).
3. Then the non-default-port and `REFLEX_API_URL` checks.
4. Commit — e.g. `Env-aware API base URL: retire hardcoded localhost:8000 (H59)`.
5. Move this doc to `Completed Handoffs/`.

### Then (housekeeping, separate commits — optional, per the incremental rule)

`CLAUDE.md`'s "_vN cleanup happens only when a file is next touched" rule now
applies to all 9 files. Each has a large sibling set under `LucidPM/pages/`
(`proforma_1.py … proforma_6_6.py`, `rent_roll_3.py … rent_roll_8_FIXED.py`,
`property_financials_8.py`, `property_financials _7.py` (note the space),
`communications_report` has none, `leases_expiring_4.py … _7.py`,
`tenants_1.py … tenants_24.py`, `lease_package_builder_*`). Once the URL change
is verified, move each touched file's siblings into `Archived Versions/` and
commit that **separately** (one commit, or one per file — match how H58 split
`b47b6f8` from `d18a256`). This is bulky; it is fine to defer it to its own
session — do **not** let it hold up the URL fix.
...
c:\Inspirion\Dev\TenantCRM\LucidPM\
  ...
  LucidPM\pages\communications_report.py    ← Step 3
  LucidPM\pages\leases_expiring.py          ← Step 4
  LucidPM\pages\lease_package_builder.py    ← Step 5  (3 sites)
  LucidPM\pages\proforma.py                 ← Step 6  (2 sites)
  LucidPM\pages\property_financials.py      ← Step 7
  LucidPM\pages\rent_roll.py                ← Step 8  (+ docstring)
  LucidPM\pages\tenants.py                  ← Steps 9 + 11
  LucidPM\pages\lease_documents.py          ← Step 10 (comment only)
...
*One helper in `state.py`, an import line + an f-string swap in 9 pages, one
comment, one bonus literal fix. Local behaviour byte-identical (Reflex's
`api_url` default is the same string that was hardcoded). Unblocks Stage 3.1 —
a containerised app can point every download link at its real ingress host with
one env var.*
