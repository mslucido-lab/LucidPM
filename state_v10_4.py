...
def send_email(
    ...
    if smtp_port == 465:
        ...
        # First try Outlook-style Auto on the configured port. For Outlook this
        # is server=smtp.biz.rr.com, port=587, encryption=Auto.
        if _try("SMTP Auto", _send_auto, smtp_port):
            return
        # Then try strict STARTTLS for normal providers.
        if _try("SMTP STARTTLS", _send_starttls, smtp_port):
            return
        # Then retry without TLS to mimic Outlook Auto fallback on legacy servers.
        if _try("SMTP Plain", _send_plain, smtp_port):
            return
        # Final fallback: implicit SSL.
        if _try("SMTP SSL", _send_ssl, 465):
            return

    detail = " | ".join(f"{label} {port}: {type(ex).__name__}: {ex}" for label, port, ex in attempts)
    raise RuntimeError(
        "SMTP send failed after Outlook-style Auto, STARTTLS, Plain SMTP, and SSL fallback attempts. "
        + detail
    )
