...
def _ordinal(n: int) -> str:
    """Return ReportLab-safe superscript ordinal: 1<super>st</super>, 2<super>nd</super>."""
    if 11 <= (n % 100) <= 13:
        suffix = "th"
    else:
        suffix = {1: "st", 2: "nd", 3: "rd"}.get(n % 10, "th")
    return f"{n}<super>{suffix}</super>"
...
def _format_grouped_rent_schedule(payment_rows: list[tuple[dt.date, Decimal]], lease_end: dt.date) -> str:
    """Format >24 payment rows as grouped rent periods.

    Total rent still comes from the full monthly payment list. This only changes
    the display format for readability on longer leases.
    """
    if not payment_rows:
        return ""

    groups: list[tuple[dt.date, dt.date, Decimal]] = []
    group_start = payment_rows[0][0]
    group_amount = payment_rows[0][1]

    for idx in range(1, len(payment_rows)):
        current_date, current_amount = payment_rows[idx]
        if current_amount != group_amount:
            groups.append((group_start, current_date - dt.timedelta(days=1), group_amount))
            group_start = current_date
            group_amount = current_amount

    groups.append((group_start, lease_end, group_amount))

    lines = []
    for i, (start, end, amount) in enumerate(groups):
        lines.append(
            f"{_alpha_label(i)} {_fmt_long_date(start)} to {_fmt_long_date(end)}: {fmt_money(amount)} per month"
        )
    return "\n".join(lines)
...
def _payment_schedule_block(lease: dict, lease_id: int, db: str) -> tuple[str, str]:
    ...
    if len(payment_rows) <= 24:
        ...
        block = _format_grouped_rent_schedule(payment_rows, lease_end)
...
def get_lease_merge_context(tenant_id: int, lease_id: int, db: str = TEST_DB_NAME) -> dict[str, str]:
    ...
    if lease_start and lease_end and context.get("LeaseTermDescription") and use_type:
        context["LeaseTermBlock"] = (
            f"a term of {context['LeaseTermDescription']}, "
