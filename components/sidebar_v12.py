...
(function() {
    ...
    applySavedWidth();
    setTimeout(applySavedWidth, 50);
    setTimeout(applySavedWidth, 250);

    // Install global handlers only once. Use event delegation so handlers survive page navigation.
    ...
    document.addEventListener('click', function(e) {
        var target = e.target;
        if (!target || target.id !== 'lucid-sidebar-toggle') return;
    ...
    // Reapply persisted width after Reflex swaps page DOM.
