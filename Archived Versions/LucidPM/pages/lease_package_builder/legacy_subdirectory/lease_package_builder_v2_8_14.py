...
class LeasePackageBuilderState(AppState):
    ...
    def regenerate_selected_generated_package(self):
        ...
        self.form_error = ""
        self.form_success = ""
        if int(self.selected_generated_id or 0) <= 0:
        ...
        try:
            ...
            rows = run_query(
                ...
                "ORDER BY lps.SortOrder, lps.LeasePackageSectionID",
                (int(self.selected_generated_id),),
                db=self.db,
            )
            if not rows:
            ...
            if not pdf_paths_to_merge:
                ...
                return
            root = normalize_storage_root(self.output_storage_root)
            original_name = self.selected_generated_file_name or self.package_name or "lease_package.pdf"
            stem, ext = os.path.splitext(original_name)
            regen_name = f"{stem}_regenerated{ext or '.pdf'}"
            ...
            filename = os.path.basename(output_path)
            run_exec(
                "UPDATE LeaseGeneratedDocuments SET GeneratedFileName = ?, StoredFilePath = ?, GeneratedOn = ? "
                "WHERE LeaseGeneratedDocumentID = ?",
                (filename, output_path, datetime.datetime.now(), int(self.selected_generated_id)),
                db=self.db,
            )
            self.last_generated_document_id = int(self.selected_generated_id)
            self.last_generated_path = output_path
            self.form_success = f"Selected lease package regenerated: {filename} ({merged_page_count} page(s))"
            self.load_generated_packages()
            self.select_generated_package(int(self.last_generated_document_id))
        except Exception as ex:
        ...
        rx.box(
            rx.vstack(
                rx.hstack(
                    ...
                    rx.button("Regenerate selected PDF", size="1", variant="soft", color_scheme="green", on_click=LeasePackageBuilderState.regenerate_selected_generated_package),
                    width="100%",
                    align="center",
                ),
                rx.grid(
