...
class LeasePackageBuilderState(AppState):
    ...
    def _section_consumes_section_number(self, section: LeasePackageSection, rendered_text: str) -> bool:
        ...
        Header/opening sections do not consume legal clause numbers. Lettered
        Special Conditions child rows also do not consume legal clause numbers.
        """
        ...
        return self._has_renderable_text(section)

    def _inject_display_label_after_section_number(self, section: LeasePackageSection, rendered_text: str) -> str:
        """Insert DisplayLabel after an authored {{SectionNumber}} marker.

        This lets ArticleNumber remain useful for admin sorting while the
        visible legal number remains dynamic. It supports common authored
        patterns such as:
            <b>{{SectionNumber}}.</b>
            {{SectionNumber}}.
            {{SectionNumber}})
        ...
        # Common bold/italic/underline wrapped form: <b>1.</b>
        pattern_wrapped = r"^(\s*(?:<[^/][^>]*>)*)(\d+)([\.)])((?:</[^>]+>)*)"
        replacement = r"\1\2\3 " + label + r"\4"
        updated = re.sub(pattern_wrapped, replacement, body, count=1)
        if updated != body:
            return updated

        # Plain form: 1.
        pattern_plain = r"^(\s*)(\d+)([\.)])"
        replacement_plain = r"\1\2\3 " + label
        return re.sub(pattern_plain, replacement_plain, body, count=1)
