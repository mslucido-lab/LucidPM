...
def _story_for_text_sections(section_texts: Sequence[str]) -> list:
    ...
    section_index = 0
    while section_index < len(texts):
        text = texts[section_index]
        blocks = _split_blocks(text)

        # Keep a parent-only Special Conditions heading with the first child
        # condition even though they are stored as separate library sections.
        if (
            len(blocks) == 1
            and _is_special_conditions_parent(blocks[0])
            and section_index + 1 < len(texts)
        ):
            next_blocks = _split_blocks(texts[section_index + 1])
            if next_blocks and _is_special_conditions_child(next_blocks[0]):
                grouped_flowables = []
                for grouped_block in [blocks[0], next_blocks[0]]:
                    grouped_flowables.extend(_paragraph_for_block(grouped_block))
                if grouped_flowables:
                    story.append(CondPageBreak(int(3.5 * 72)))
                    story.append(KeepTogether(grouped_flowables))
                for remaining_block in next_blocks[1:]:
                    flowables = _paragraph_for_block(remaining_block)
                    if flowables:
                        story.append(CondPageBreak(int(0.75 * 72)))
                        story.extend(flowables)
                if section_index + 1 < len(texts) - 1:
