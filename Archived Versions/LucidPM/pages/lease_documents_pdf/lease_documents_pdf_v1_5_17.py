...
Patched 5/3/2026:
- v1.5.17: adds safe header/content tags for styled lease header blocks before XML escaping.
...
try:
    ...
    LEASE_STYLES = {
        ...
        "ARTICLE_HEADER": ParagraphStyle("ArticleHeader", fontName="Times-Bold", fontSize=10, leading=12, alignment=4, leftIndent=36, firstLineIndent=-18, keepWithNext=True, spaceBefore=6, spaceAfter=4),
...
CONTENT_TAG_RE = re.compile(r"^\[([A-Z_]+)\](.*)$", flags=re.DOTALL)


def _tagged_block(block: str) -> tuple[str, str]:
    """Return (tag, content) for safe lease-rendering tags."""
    text = str(block or "").strip()
    m = CONTENT_TAG_RE.match(text)
    if not m:
        return "", text
...
def _paragraph_for_tagged_block(tag: str, content: str):
    """Render safe content tags using predefined styles before XML escaping."""
    ...
    style_by_tag = {
        ...
        "HEADER_BODY": "CLAUSE_BODY",
        "PREMISES_BLOCK": "CLAUSE_BODY",
        "CLAUSE_BODY": "CLAUSE_BODY",
        "ARTICLE_HEADER": "ARTICLE_HEADER",
    }

    style_name = style_by_tag.get(tag)
    if not style_name:
        return []

    preserve_breaks = tag in {"JURISDICTION"}
    return [Paragraph(_xml(content, preserve_line_breaks=preserve_breaks), LEASE_STYLES[style_name])]
...
def _schedule_table_flowable(lines: list[str]):
    ...
    frame_width = letter[0] - PAGE_SETUP.get("leftMargin", 54) - PAGE_SETUP.get("rightMargin", 54)
...
def _paragraph_for_block(block: str):
    ...
    tag, tagged_content = _tagged_block(clean)
    if tag:
        tagged_flowables = _paragraph_for_tagged_block(tag, tagged_content)
        if tagged_flowables:
            return tagged_flowables
