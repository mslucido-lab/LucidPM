...
def _schedule_table_flowable(lines: list[str]):
    """Render schedule rows as a centered block within the clause body."""
    if not lines:
        return []
    content = "\n".join(ln.rstrip() for ln in lines if str(ln or "").strip())
    if not content:
        return []

    pre = Preformatted(escape(content), LEASE_STYLES["PAYMENT_SCHEDULE"], maxLineLength=95)

    # A single-cell table with fixed width gives ReportLab a reliable centered schedule block.
    table_width = 5.15 * 72
    tbl = Table([[pre]], colWidths=[table_width], hAlign="CENTER")
