...
def _markup_flowables(text: str, style) -> list:
    ...
    if not _looks_like_reportlab_markup(raw):
        return [Paragraph(_xml(raw), style)]
    ...
    for part in parts:
        ...
        if "<para" in cleaned.lower():
            flowables.append(Paragraph(_sanitize_reportlab_markup(cleaned), style))
        else:
            flowables.append(Paragraph(_markup_or_xml(cleaned), style))
    return flowables
