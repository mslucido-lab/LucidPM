...
def render_text_to_pdf(section_text: str, output_path: str) -> str:
    """Render token-resolved lease text to a PDF file.

    Uses Courier so fixed-width text blocks, including PaymentScheduleBlock,
    keep alignment during the end-to-end validation phase.
    """
    ...
    for block in blocks:
        clean = block.rstrip()
        if clean:
            story.append(Preformatted(escape(clean), style, maxLineLength=110))
        story.append(Spacer(1, 8))

    if not story:
        story.append(Preformatted("", style))

    doc.build(story)

    if not os.path.isfile(output_path) or os.path.getsize(output_path) == 0:
        raise RuntimeError(f"render_text_to_pdf() failed to produce output at {output_path}")
