...
def _payment_schedule_block(lease: dict, lease_id: int, db: str) -> tuple[str, str]:
    ...
    # Always generate one payment row per month of the lease term.
    # LeaseRentSchedule rows are rate periods, not payment rows. A five-year
    # lease with annual increases will usually have five schedule rows, but
    # the lease payment schedule still needs sixty monthly payment rows.
