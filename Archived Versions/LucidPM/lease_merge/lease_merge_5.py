...
def _payment_schedule_block(lease: dict, lease_id: int, db: str) -> tuple[str, str]:
    """Build a monthly payment schedule and total rent for the lease term."""
    ...
    if not lease_end:
        ...
        return f"a) {lease_start.strftime('%B')} {lease_start.day}, {lease_start.year}: {fmt_money(rent_dec)}", fmt_money(rent_dec)

    schedule_dates = [_date(r.get("EffectiveStartDate")) for r in schedule_rows if _date(r.get("EffectiveStartDate"))]
    explicit_monthly = len(schedule_dates) >= 2 and all(lease_start <= d <= lease_end for d in schedule_dates)
    if explicit_monthly:
        months_in_term = (lease_end.year - lease_start.year) * 12 + lease_end.month - lease_start.month + 1
        explicit_monthly = len(schedule_dates) >= min(months_in_term, 3)

    payment_rows: list[tuple[dt.date, Decimal]] = []
    if explicit_monthly:
        for row in schedule_rows:
            d = _date(row.get("EffectiveStartDate"))
            if d and lease_start <= d <= lease_end:
                payment_rows.append((d, _money_decimal(row.get("RentAmount"))))
    else:
        ...
        while current_month <= last_month:
            ...
            if current_month.year == lease_start.year and current_month.month == lease_start.month and lease_start > due_date:
    ...
    lines = [
        f"{_alpha_label(i)} {due_date.strftime('%B')} {due_date.day}, {due_date.year}: {fmt_money(amount)}"
        for i, (due_date, amount) in enumerate(payment_rows)
    ]
