...
class PropertyFinancialsAnalyticsState(AppState):
    ...
    def margin_chart_data(self) -> list[dict]:
        """NOI margin % by year with pre-computed color for threshold display."""
        result = []
        for row in self.chart_data:
            ...
            color = (
                CHART_MARGIN_GREEN if margin >= 60
                else CHART_MARGIN_AMBER if margin >= 45
                else CHART_MARGIN_RED
            )
            result.append({
                "year": row["year"],
                "margin": margin,
...
def margins_chart() -> rx.Component:
    return chart_container(
        rx.recharts.bar_chart(
            rx.recharts.bar(
                data_key="margin",
                fill=CHART_MARGIN_GREEN,
                fill_opacity=0.65,
                stroke=CHART_MARGIN_GREEN,
                stroke_width=1.0,
                name="NOI Margin %",
            ),
            rx.recharts.x_axis(data_key="year"),
