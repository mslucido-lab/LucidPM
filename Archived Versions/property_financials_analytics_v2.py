...
class PropertyFinancialsAnalyticsState(AppState):
    ...
    async def on_load(self):
        ...
        for p in prop_rows:
            ...
            sqft_map[pid] = float(sqft_rows[0]["total"] or 0)
        ...
        self.property_options = ["0"] + sorted(prop_map.keys(), key=lambda k: prop_map.get(k, k))
        self.compare_period = years[0] if years else ""

    async def reload_on_db_change(self):
        self.financials_data = []
        self.property_options = ["0"]
            ...
            rx.vstack(
                ...
                rx.hstack(
                    rx.select(
                        ...
                        value=PropertyFinancialsAnalyticsState.selected_property_id,
                        on_change=PropertyFinancialsAnalyticsState.set_selected_property_id,
                    ),

                    rx.vstack(
                        rx.text(
                            "Cap Rate: " + PropertyFinancialsAnalyticsState.cap_rate.to_string() + "%",
                            size="1",
                            color="#666",
                        ),
                        rx.slider(
