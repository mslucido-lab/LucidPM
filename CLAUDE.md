...
- **Handoff 60 — SQL connect retry-on-resume** (`Undelivered Handoffs/`, this session, not started). The follow-up H58 flagged. Bounded, transient-only retry loop around the single `pyodbc.connect` in `state.get_conn()` for Azure serverless auto-pause (first connect after a 60-min idle fails while the DB resumes ~20–60s). New `LUCIDPM_SQL_CONNECT_RETRIES` (default 2) + `LUCIDPM_SQL_RETRY_BACKOFF` (default 3), validated like the H58 vars. Retries only SQLSTATE `08*`/`HYT*` + a native-code list (`40613` etc.); auth failures (`28000`/18456) and bad-db (`4060`) fail fast on attempt 1. Byte-identical when the DB answers first try (local SQL Express never enters the loop). `state.py` + `.env.example` only, no call-site changes. Checklist A/B/C need no Azure; D is the Cycle 2.2 resume measurement that tunes the defaults.
