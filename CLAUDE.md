...
- **Handoff 60 — SQL connect retry-on-resume** (`Undelivered Handoffs/`, this session, Codex pre-impl reviewed, not started). The follow-up H58 flagged. Bounded, transient-only retry loop around the single `pyodbc.connect` in `state.get_conn()` for Azure serverless auto-pause (first connect after a 60-min idle fails while the DB resumes ~20–60s). New `LUCIDPM_SQL_CONNECT_RETRIES` (default 2) + `LUCIDPM_SQL_RETRY_BACKOFF` (default 3), validated like the H58 vars. Retries **only** exact SQLSTATE `{HYT00, HYT01, 08S01}` + an Azure native-code list (`40613`, `40197`, `40501`, `40540`, `40143`, `49918-20`, `10928/9`, `4221`) — bare `08001`/DNS/firewall and `10060` fail fast (Codex review point 1). On budget exhaustion re-raises the **first** transient error, last kept as `__context__` (review point 2). `retries=N` → N stderr log lines (review point 3). Byte-identical when the DB answers first try. `state.py` + `.env.example` only. Checklist B = isolated subprocess predicate/loop checks; D = the Cycle 2.2 resume measurement that tunes the defaults.
