...
- **Hardcoded `http://localhost:8000` in PDF/report URLs — cloud-blocker, fix before Stage 3** (H58 review finding 4, 2026-09-07). `pages/tenants.py:704` (`application_report_url`) and `pages/lease_package_builder.py` lines 364, 370, 1825 build download/report links against a literal `localhost:8000`. Works only on the dev laptop with the backend on 8000; in a container (or any non-default port) every such link 404s. Fix = request-relative URLs or derive the host from the ingress. Also `pages/tenants.py:703` `db_name = self.db or "TenantCRM"` — same stale-literal class Handoff 58 Step 6 fixed in `LucidPM.py` (harmless today since `self.db` is always truthy; swap to `PROD_DB_NAME` on the next `tenants.py` touch, per H58 review finding 2).
