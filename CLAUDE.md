...
Reworking the `/admin/lease-templates` page so the two heavily-used modules (Package Templates + Section Library) stop forcing constant tab-hopping. Origin doc: `Undelivered Handoffs/Refining Lease Template process.md`. A written proposal + a clickable prototype were delivered as private Artifacts (links in the `project_lease_template_studio` memory).
