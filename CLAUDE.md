...
- With that pin set, `reflex run --backend-only` boots and connects, but crashes compiling the `tenants`/`communications` pages: `EventHandlerArgTypeMismatchError` on their file-upload handlers. Root-caused (not a version problem — confirmed unchanged back to Reflex 0.7.0): those two files bind `on_drop=` directly to the handler, which Reflex's type-checker rejects; `waiting_list.py` already uses the correct modern pattern (`on_drop=State.handler(rx.upload_files(...))`) and should compile fine. Written up as **Handoff 49** (3 call sites, one-line-pattern fix each) in `Undelivered Handoffs/`, not yet implemented — full end-to-end boot (all pages compiling) is not yet confirmed.
- **Not done this session** (explicitly deferred, tracked as backlog, not overlooked): DB schema/migration baselining (loose SQL scripts already exist under `LucidPM/pages/TEST/`, not yet consolidated), automated tests, CI, VS Code workspace config, and the bulk `_vN` archive cleanup (stays incremental per policy — the restructure moved these files but didn't "touch" their content, so it doesn't trigger archiving).
- Local commit of this restructure/scaffold/docs/Handoff-49 work is **pending Mark's explicit approval** — not yet committed.
