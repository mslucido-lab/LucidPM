...
- **Handoff 59 — Env-aware API base URL** (`Undelivered Handoffs/`, this session, Codex pre-impl reviewed ×2, not started). Retire the hardcoded `http://localhost:8000` in 7 page modules' PDF/report download `@rx.var`s (+ 1 comment-only) → `state.api_base_url()` (reads Reflex `api_url`; default unchanged). Stage 3 prerequisite, no Azure dependency, parallel with Cycle 2.2. Folds in H58 review finding 2 (`tenants.py:703` literal); archives the dead orphan `communications_report.py`. See the standing-backlog entry below.
