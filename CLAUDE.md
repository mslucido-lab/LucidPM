...
### Queued — four handoffs written, none started, Mark to sequence

- **Handoff 58 — Env-driven SQL connection + SQL-auth branch** (`Undelivered Handoffs/`, this session, uncommitted, revised). Azure Migration Stage 0.3 — the blocker in front of Cycle 2.2. `state.py` `get_conn()` reads server/db/auth from `os.getenv` (today's hardcoded values as defaults → local behaviour byte-identical when nothing is set); adds a `sql` auth branch alongside `windows`; hand-rolled `.env` loader (no new dependency). **Test/Prod toggle stays** — both Azure DBs (`TenantCRM` + `TenantCRM_Test`, both loaded + verified in Cycle 2.1) are reachable by `lucidadmin`; `LUCIDPM_PROD_DB` / `LUCIDPM_TEST_DB` point at the Azure names. Optional `LUCIDPM_SINGLE_DB=<name>` locks to one DB + hides the toggle (for a future multi-user deployment). Files: `state.py`, `components/sidebar.py` (guard Switch button on `is_single_db`), `.env.example`. `LucidPM.py` untouched. Verifying the Azure `.env` path doubles as Cycle 2.2's smoke test / Gate 2 input.
