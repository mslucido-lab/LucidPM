...
# v2.7.0 - Adds package template selector, template sections, and section overrides.
...
class LeasePackageBuilderState(AppState):
    ...
    def generate_merge_preview(self):
        ...
        try:
            ...
            raw_text = "\n\n".join(
                str(r.get("PieceText") or "").strip()
                for r in rows
                if str(r.get("PieceText") or "").strip()
            )
            ...
            self.merge_preview = rendered_text
            self.merge_missing_tokens = unresolved
            self.merge_known_tokens = validation.get("known_tokens", [])
            if unresolved:
