...
# v2.8.4 - Uses SectionID-backed package template select values to avoid label-based section resolution.
...
class LeasePackageBuilderState(AppState):
    ...
    def load_available_pieces(self):
        ...
        rows = run_query(
            ...
            "p.StoredFilePath, ISNULL(p.Content, '') AS Content, p.SortOrder, ISNULL(pr.PropertyName,'') AS PropertyName, "
    ...
    def load_template_sections(self):
        ...
        rows = run_query(
            ...
            "ISNULL(lts.SectionType, '') AS SectionType, lts.DefaultPieceID, "
    ...
    def _selected_sections_for_generation(self) -> list[LeasePackagePiece]:
        if self.selected_package_template_id > 0 and self.template_sections:
            ...
            rows = run_query(
                ...
                "INNER JOIN LeaseSourceDocuments s ON p.LeaseSourceDocumentID = s.LeaseSourceDocumentID "
                "LEFT JOIN Properties pr ON s.PropertyID = pr.PropertyID "
                f"WHERE p.LeaseDocumentPieceID IN ({placeholders})",
                tuple(selected_piece_ids),
                db=self.db,
            )
            piece_by_id = {int(r["LeaseDocumentPieceID"]): r for r in rows}
            ...
            for section in selected_rows:
                ...
                ordered.append(LeasePackagePiece(
                    ...
                    piece_id=int(r["LeaseDocumentPieceID"]),
                    piece_name=str(r.get("PieceName") or section.section_label or "Section"),
                    piece_type=str(r.get("PieceType") or section.section_type or ""),
                    exhibit_code=str(r.get("ExhibitCode") or ""),
                    property_name=str(r.get("PropertyName") or ""),
                    source_template=str(r.get("TemplateName") or ""),
            ...
            if generated_id:
                for idx, p in enumerate(selected, start=1):
                    try:
                        ...
                        # Older schemas may not have this legacy audit table. LeasePackageSections is the current audit path.
                        pass
                ...
                if self.selected_package_template_id > 0 and self.template_sections:
                    for section in self.template_sections:
                        ...
                        run_exec(
                            ...
                            "(LeaseGeneratedDocumentID, LeaseTemplateSectionID, SortOrder, IsIncluded, PieceID, Content, IsDirty, ContentSnapshot) "
                            "VALUES (?, ?, ?, ?, ?, ?, 0, ?)",
                            (
                                generated_id,
    ...
    def _lease_piece_text_column(self) -> str:
        """Return the first text-like column available on LeaseDocumentPieces."""
        rows = run_query(
            "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS "
            "WHERE TABLE_SCHEMA = 'dbo' AND TABLE_NAME = 'LeaseDocumentPieces'",
            db=self.db,
        )
    ...
    def generate_merge_preview(self):
        ...
        try:
            ...
            if not text_col:
                self.merge_error = (
                    "No text content column found on LeaseDocumentPieces. "
            ...
            rows = run_query(
                f"SELECT LeaseDocumentPieceID, PieceName, SortOrder, [{text_col}] AS PieceText "
                f"FROM LeaseDocumentPieces WHERE LeaseDocumentPieceID IN ({placeholders}) "
                "ORDER BY SortOrder, PieceName",
                tuple(selected_ids),
                db=self.db,
            )
            tenant_id = self._selected_tenant_id()
            ...
            for r in rows:
                piece_name = str(r.get("PieceName") or "Untitled Section").strip()
            ...
            if not any_text:
                self.merge_error = (
                    f"The selected pieces have no text in LeaseDocumentPieces.{text_col}. "
