...
# v2.8.5 - Supports renamed LeaseDocumentSections schema plus SectionID-backed template selections.
...
class LeasePackageTemplateSectionRow(rx.Base):
    ...
    default_piece_id: int = 0
    selected_piece_id: int = 0
    selected_piece_label: str = ""
    included: bool = True


class PieceOption(rx.Base):
    value: str = ""
    label: str = ""
...
class LeasePackageBuilderState(AppState):
    ...
    piece_option_labels: list[str] = []
    piece_option_ids: list[int] = []
    piece_options: list[PieceOption] = []
    ...
    def reload_on_db_change(self):
        ...
        self.piece_option_labels = []
        self.piece_option_ids = []
        self.piece_options = []
        self._load_tenants()
        if self.tenant_labels:
            self.selected_tenant_label = self.tenant_labels[0]
    ...
    def _lease_section_table(self) -> str:
        if self._table_exists("LeaseDocumentSections"):
            return "LeaseDocumentSections"
        return "LeaseDocumentPieces"

    def _lease_section_id_col(self) -> str:
        table = self._lease_section_table()
        return "LeaseDocumentSectionID" if self._column_exists(table, "LeaseDocumentSectionID") else "LeaseDocumentPieceID"

    def _lease_section_name_col(self) -> str:
        table = self._lease_section_table()
        return "SectionName" if self._column_exists(table, "SectionName") else "PieceName"

    def _lease_section_type_col(self) -> str:
        table = self._lease_section_table()
        return "SectionType" if self._column_exists(table, "SectionType") else "PieceType"

    def _template_default_section_col(self) -> str:
        return "DefaultSectionID" if self._column_exists("LeaseTemplateSections", "DefaultSectionID") else "DefaultPieceID"

    def _package_section_ref_col(self) -> str:
        return "SectionID" if self._column_exists("LeasePackageSections", "SectionID") else "PieceID"

    def _generated_section_audit_target(self) -> tuple[str, str]:
        if self._table_exists("LeaseGeneratedDocumentSections"):
            table = "LeaseGeneratedDocumentSections"
            col = "LeaseDocumentSectionID" if self._column_exists(table, "LeaseDocumentSectionID") else "SectionID"
            return table, col
        if self._table_exists("LeaseGeneratedDocumentPieces"):
            return "LeaseGeneratedDocumentPieces", "LeaseDocumentPieceID"
        return "", ""
    ...
    def load_available_pieces(self):
        ...
        self.available_pieces = [
            LeasePackagePiece(
                piece_id=int(r["SectionID"]),
                piece_name=str(r.get("SectionName") or ""),
                piece_type=str(r.get("SectionType") or ""),
                ...
                selected=int(r["SectionID"]) in self.selected_piece_ids,
            )
            for r in rows
        ]
    ...
    def _piece_label(self, piece_id: int) -> str:
        if piece_id <= 0:
            return "(No section)"
        for idx, pid in enumerate(self.piece_option_ids):
            if int(pid) == int(piece_id):
                return self.piece_option_labels[idx]
        for p in self.available_pieces:
            if int(p.piece_id) == int(piece_id):
                parts = [p.piece_type, p.piece_name]
                if p.exhibit_code:
                    parts.append(f"Exhibit {p.exhibit_code}")
                return " | ".join(x for x in parts if x)
        return "(No section)"

    def _piece_id_for_label(self, label: str) -> int:
        try:
            idx = self.piece_option_labels.index(label)
            return int(self.piece_option_ids[idx])
        except Exception:
            return 0

    def _refresh_piece_options(self):
        labels = ["(No section)"]
        ids = [0]
        for p in self.available_pieces:
            suffix = []
            ...
            suffix.append(f"ID={int(p.piece_id)}")
            extra = f" ({' / '.join(suffix)})" if suffix else ""
            labels.append(f"{p.piece_type} | {p.piece_name}{extra}")
            ids.append(int(p.piece_id))
        self.piece_option_labels = labels
        self.piece_option_ids = ids
        self.piece_options = [
            PieceOption(value=str(pid), label=label)
    ...
    def load_template_sections(self):
        ...
        out = []
        selected_ids = []
        valid_ids = set(int(x) for x in self.piece_option_ids)
        for r in rows:
            default_piece_id = int(r.get("DefaultSectionID") or 0)
            selected_piece_id = default_piece_id if default_piece_id in valid_ids else 0
            included = bool(selected_piece_id > 0 or r.get("IsRequired"))
            if included and selected_piece_id > 0:
                selected_ids.append(selected_piece_id)
            out.append(LeasePackageTemplateSectionRow(
                ...
                default_piece_id=default_piece_id,
                selected_piece_id=selected_piece_id,
                selected_piece_label=self._piece_label(selected_piece_id),
                included=included,
            ))
        self.template_sections = out
        self.selected_piece_ids = selected_ids
        self.load_available_pieces()

    def set_template_section_piece(self, template_section_id: int, value: str):
        sid = int(template_section_id)
        try:
            piece_id = int(value or 0)
        except Exception:
            piece_id = 0
        valid_ids = set(int(pid) for pid in self.piece_option_ids)
        if piece_id not in valid_ids:
            piece_id = 0
        updated = []
        for section in self.template_sections:
            if int(section.template_section_id) == sid:
                updated.append(LeasePackageTemplateSectionRow(
                    ...
                    default_piece_id=section.default_piece_id,
                    selected_piece_id=piece_id,
                    selected_piece_label=self._piece_label(piece_id),
                    included=section.included,
                ))
            else:
                updated.append(section)
        self.template_sections = updated
        self._sync_selected_piece_ids_from_template()
    ...
    def toggle_template_section_included(self, template_section_id: int):
        ...
        for section in self.template_sections:
            ...
            updated.append(LeasePackageTemplateSectionRow(
                ...
                is_required=section.is_required,
                default_piece_id=section.default_piece_id,
                selected_piece_id=section.selected_piece_id,
                selected_piece_label=section.selected_piece_label,
                included=included,
            ))
        self.template_sections = updated
        self._sync_selected_piece_ids_from_template()

    def _sync_selected_piece_ids_from_template(self):
        if self.selected_package_template_id <= 0:
            return
        self.selected_piece_ids = [
            int(section.selected_piece_id)
            for section in self.template_sections
            if section.included and int(section.selected_piece_id or 0) > 0
        ]
        self.load_available_pieces()

    def _selected_sections_for_generation(self) -> list[LeasePackagePiece]:
        if self.selected_package_template_id > 0 and self.template_sections:
            ...
            Important: generation must use one row per included template section.
            Do not rely on selected_piece_ids or ambiguous select labels here.
            The UI state tells us which PieceID each template section currently points to,
            then we re-query LeaseDocumentPieces by that exact PieceID so each section gets
            its own Content and StoredFilePath.
            """
            ordered: list[LeasePackagePiece] = []
            ...
            selected_piece_ids = [int(section.selected_piece_id) for section in selected_rows]
            placeholders = ",".join("?" for _ in selected_piece_ids)
            ...
            rows = run_query(
                ...
                tuple(selected_piece_ids),
                db=self.db,
            )
            piece_by_id = {int(r["SectionID"]): r for r in rows}

            for section in selected_rows:
                pid = int(section.selected_piece_id or 0)
                r = piece_by_id.get(pid)
                if not r:
                    continue
                ordered.append(LeasePackagePiece(
                    template_section_id=int(section.template_section_id or 0),
                    template_section_label=str(section.section_label or ""),
                    piece_id=int(r["SectionID"]),
                    piece_name=str(r.get("SectionName") or section.section_label or "Section"),
                    piece_type=str(r.get("SectionType") or section.section_type or ""),
                    exhibit_code=str(r.get("ExhibitCode") or ""),
                    property_name=str(r.get("PropertyName") or ""),
                    source_template=str(r.get("TemplateName") or ""),
    ...
    def _validate_duplicate_dynamic_template_sections(self, selected: list[LeasePackagePiece]) -> list[str]:
        """Stop silent repeated pages when multiple template rows point to the same dynamic section."""
        seen: dict[int, LeasePackagePiece] = {}
        errors: list[str] = []
        for section in selected:
            if not str(section.content or "").strip():
                continue
            pid = int(section.piece_id or 0)
            if pid <= 0:
                continue
            if pid in seen:
                first = seen[pid]
                first_label = first.template_section_label or first.piece_name or f"Template section {first.template_section_id}"
                second_label = section.template_section_label or section.piece_name or f"Template section {section.template_section_id}"
                errors.append(
                    f"{first_label} and {second_label} both point to the same tokenized section ID {pid} ({section.piece_name})."
                )
            else:
                seen[pid] = section
        return errors
    ...
    def _validate_template_section_selections(self) -> list[str]:
        ...
        for section in self.template_sections:
            if not section.included:
                continue
            if int(section.selected_piece_id or 0) <= 0:
                label = section.section_label or f"Template section {section.template_section_id}"
                errors.append(f"{label}: no selected section")
        return errors

    def _validate_tokens_before_generation(self, selected: list[LeasePackagePiece], context: dict) -> list[str]:
        ...
        for p in selected:
            ...
            unresolved = validation.get("unresolved", []) or validation.get("missing", []) or []
            if unresolved:
                label = p.template_section_label or p.piece_name
    ...
    def generate_package(self):
        ...
        missing_sources = [
            p.piece_name for p in selected
        ...
        try:
            ...
            if token_errors:
                self.form_error = "Cannot generate. Unresolved or blank tokens found before rendering: " + " | ".join(token_errors[:5])
                return

            pdf_paths_to_merge: list[str] = []
            rendered_content_by_template_section_id: dict[int, str] = {}
            rendered_content_by_piece_id: dict[int, str] = {}
            ...
            for idx, p in enumerate(selected, start=1):
                ...
                if content:
                    ...
                    if unresolved:
                        self.form_error = (
                            f"Cannot generate package. Section '{p.piece_name}' has unresolved tokens: "
                    ...
                    rendered_content_by_piece_id[int(p.piece_id)] = rendered_text
                else:
            ...
            if generated_id:
                ...
                for idx, p in enumerate(selected, start=1):
                    ...
                    try:
                        ...
                        # LeasePackageSections is the current audit path. This audit insert is best effort.
                        pass
                ...
                if self.selected_package_template_id > 0 and self.template_sections:
                    for section in self.template_sections:
                        ...
                        if not rendered_snapshot:
                            rendered_snapshot = rendered_content_by_piece_id.get(int(section.selected_piece_id or 0), "")
                        ...
                        run_exec(
                            ...
                            (
                                ...
                                section.selected_piece_id if section.selected_piece_id > 0 else None,
    ...
    def _lease_piece_text_column(self) -> str:
        """Return the first text-like column available on the section table."""
        table = self._lease_section_table()
        rows = run_query(
    ...
    def generate_merge_preview(self):
        """
        Preview token replacement for selected pieces that have text content.
        ...
        try:
            text_col = self._lease_piece_text_column()
            if not text_col:
                self.merge_error = (
                    "No text content column found on the lease section table. "
                    "Your current pieces are PDF-only, so PDF package generation still works, "
                    "but merge preview needs tokenized text content."
                )
                return

            selected_ids = [p.piece_id for p in selected]
            if not selected_ids:
                self.merge_error = "Selected pieces could not be resolved from the available piece list."
                return
            ...
            section_name_col = self._lease_section_name_col()
            rows = run_query(
                f"SELECT [{section_id_col}] AS SectionID, [{section_name_col}] AS SectionName, SortOrder, [{text_col}] AS PieceText "
            ...
            for r in rows:
                piece_name = str(r.get("SectionName") or "Untitled Section").strip()
                piece_text = str(r.get("PieceText") or "").strip()
                if not piece_text:
                    continue

                any_text = True
                rendered_text, unresolved = render_text_template(piece_text, context)
                validation = validate_template_tokens(piece_text, context)
            ...
            if not any_text:
                self.merge_error = (
                    f"The selected sections have no text in the section table column {text_col}. "
                    "Your current pieces appear to be PDF-only."
                )
                return
...
def piece_option_item(option: PieceOption) -> rx.Component:
    ...
    return rx.table.row(
        ...
        rx.table.cell(
            rx.select.root(
                ...
                rx.select.content(
                    rx.foreach(LeasePackageBuilderState.piece_options, piece_option_item),
                    position="popper",
                ),
                value=s.selected_piece_id.to_string(),
                on_change=lambda value: LeasePackageBuilderState.set_template_section_piece(s.template_section_id, value),
                size="1",
                width="100%",
            )
        ),
