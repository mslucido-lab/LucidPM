...
# v2.8.3 - Hardens template-section generation by resolving each included section directly and blocking duplicate dynamic section reuse.
...
class LeasePackageBuilderState(AppState):
    ...
    def set_template_section_piece(self, template_section_id: int, label: str):
        sid = int(template_section_id)
        piece_id = self._piece_id_for_label(label)
        updated = []
...
def template_section_row(s: LeasePackageTemplateSectionRow) -> rx.Component:
    return rx.table.row(
        ...
        rx.table.cell(
            rx.select(
                LeasePackageBuilderState.piece_option_labels,
                value=s.selected_piece_label,
