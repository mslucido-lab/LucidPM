...
# v2.8.20 - Reorders workflow so preview comes before summary and generate, with a soft missing-token warning near generate.
...
class LeasePackageBuilderState(AppState):
    ...
    def regenerate_selected_generated_package(self):
        ...
        try:
            ...
            root = normalize_storage_root(self.output_storage_root)
            original_name = str(source_doc.get("GeneratedFileName") or self.selected_generated_file_name or self.package_name or "lease_package.pdf")
            stem, ext = os.path.splitext(original_name)
            stamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            regen_name = f"{stem}_regen_{stamp}{ext or '.pdf'}"
...
def regenerate_preview_panel() -> rx.Component:
    return rx.cond(
        ...
        rx.box(
            rx.vstack(
                rx.hstack(
                    rx.text("Regenerate preview", size="3", weight="bold", color=BRAND_DARK),
                    rx.spacer(),
                ...
                rx.hstack(
                    rx.cond(
                        ...
                        rx.button("Create new version", size="2", color_scheme="green", on_click=LeasePackageBuilderState.regenerate_selected_generated_package),
                        rx.fragment(),
                    ),
        ...
        rx.box(
            rx.vstack(
                rx.hstack(
                    ...
                    rx.button("Preview regenerate", size="1", variant="soft", color_scheme="green", on_click=LeasePackageBuilderState.preview_regenerate_selected_generated_package),
                    width="100%",
                    align="center",
                ),
                rx.grid(
                    ...
                    rx.vstack(
                        ...
                        rx.text(LeasePackageBuilderState.selected_generated_file_name, size="2", weight="bold"),
                        spacing="1",
                    ),
                    rx.vstack(
                        rx.text("Path", size="1", color="#666"),
                        rx.text(
...
def lease_package_builder_content() -> rx.Component:
    return rx.vstack(
        ...
        rx.box(
            rx.vstack(
                rx.text("4. Preview package", size="4", weight="bold", color=BRAND_DARK),
                rx.text(
        ...
        rx.box(
            rx.vstack(
                rx.text("5. Pre-generation summary", size="4", weight="bold", color=BRAND_DARK),
        ...
        rx.box(
            rx.vstack(
                rx.text("6. Generate package", size="4", weight="bold", color=BRAND_DARK),
                rx.grid(
                    rx.vstack(
