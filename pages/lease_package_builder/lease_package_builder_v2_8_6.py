...
# v2.8.6 - Section-only schema cleanup; removes legacy section-table compatibility.
...
class LeasePackageBuilderState(AppState):
    ...
    def _selected_sections_for_generation(self) -> list[LeasePackageSection]:
        if self.selected_package_template_id > 0 and self.template_sections:
            ...
            Do not rely on selected_section_ids or label text here.
            The UI state tells us which SectionID each template section currently points to,
            then we re-query LeaseDocumentSections by that exact SectionID so each section gets
            ...
            for section in selected_rows:
                pid = int(section.selected_section_id or 0)
                r = section_by_id.get(pid)
                if not r:
                    continue
        ...
        selected = [p for p in self.available_sections if p.section_id in self.selected_section_ids]
        selected.sort(key=lambda p: (p.sort_order, p.section_type, p.section_name))
        return selected
    ...
    def _lease_section_text_column(self) -> str:
    ...
    def generate_merge_preview(self):
        ...
        try:
            text_col = self._lease_section_text_column()
            if not text_col:
                self.merge_error = (
                    "No text content column found on the lease section table. "
                    "Your current sections are PDF-only, so PDF package generation still works, "
            ...
            section_name_col = self._lease_section_name_col()
            rows = run_query(
                f"SELECT [{section_id_col}] AS SectionID, [{section_name_col}] AS SectionName, SortOrder, [{text_col}] AS SectionText "
