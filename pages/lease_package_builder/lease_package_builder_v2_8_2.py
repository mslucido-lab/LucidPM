...
# v2.8.2 - Fix template section content mapping so duplicate section labels do not reuse the first tokenized section.
...
class LeasePackageBuilderState(AppState):
    ...
    def _selected_sections_for_generation(self) -> list[LeasePackagePiece]:
        if self.selected_package_template_id > 0 and self.template_sections:
            # Build the generation list from the template rows themselves, not from
            # selected_piece_ids. This preserves one output item per included template
            # section and prevents duplicate/ambiguous select labels from reusing the
            # first section's Content.
            ordered: list[LeasePackagePiece] = []
            piece_by_id = {int(p.piece_id): p for p in self.available_pieces}

            for section in sorted(self.template_sections, key=lambda s: (int(s.sort_order or 0), int(s.template_section_id or 0))):
                ...
                if piece is not None:
                    ordered.append(LeasePackagePiece(
                        ...
                        piece_name=str(piece.piece_name or section.section_label or "Section"),
                        piece_type=str(piece.piece_type or section.section_type or ""),
                        exhibit_code=str(piece.exhibit_code or ""),
                        property_name=str(piece.property_name or ""),
                        source_template=str(piece.source_template or ""),
                        sort_order=int(section.sort_order or piece.sort_order or 0),
                        file_path=str(piece.file_path or ""),
                        content=str(piece.content or ""),
                        selected=True,
                    ))
                    continue

                # Safety fallback. If available_pieces is stale, query the selected
                # section directly by ID so generation still uses the correct content.
                rows = run_query(
                    "SELECT p.LeaseDocumentPieceID, p.PieceName, p.PieceType, ISNULL(p.ExhibitCode,'') AS ExhibitCode, "
                    "p.StoredFilePath, ISNULL(p.Content, '') AS Content, p.SortOrder, ISNULL(pr.PropertyName,'') AS PropertyName, "
                    "ISNULL(s.TemplateName, '') AS TemplateName "
    ...
    def _validate_tokens_before_generation(self, selected: list[LeasePackagePiece], context: dict) -> list[str]:
        ...
        for p in selected:
            ...
            if unresolved:
                errors.append(f"{p.piece_name}: " + ", ".join(sorted(set(unresolved))[:10]))
        return errors
