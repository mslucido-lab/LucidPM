...
# v2.6.1 - Preserves existing PDF package builder and adds safe merge preview.
...
class LeasePackageBuilderState(AppState):
    ...
    def generate_merge_preview(self):
        ...
        if self.selected_lease_id <= 0:
            ...
            return
        if not self.selected_piece_ids:
            self.merge_error = "Select at least one template piece."
            return
...
def lease_package_builder_content() -> rx.Component:
    return rx.vstack(
        ...
        rx.box(
            rx.vstack(
                rx.text("4. Preview merge fields", size="4", weight="bold", color=BRAND_DARK),
                rx.text(
                    "This tests token replacement only. It does not change the existing PDF package generator.",
                    size="2",
                    color="#555",
                ),
                rx.hstack(
                ...
                rx.text_area(
                    ...
                    placeholder="Merged lease text will appear here if selected pieces have tokenized text content.",
                    width="100%",
                    height="360px",
                ),
                spacing="4",
