...
def _payment_clause_flowables(block: str):
    ...
    for raw in lines:
        ...
        flush_schedule()
        flowables.append(Paragraph(_markup_or_xml(line), LEASE_STYLES["CLAUSE_BODY"]))
...
def _paragraph_for_block(block: str):
    ...
    if _is_doc_title(clean):
        return [Paragraph(_markup_or_xml(clean), LEASE_STYLES["DOC_TITLE"])]
    if _is_jurisdiction(clean):
        return [Paragraph(_markup_or_xml(clean, preserve_line_breaks=True), LEASE_STYLES["JURISDICTION"])]
    if _is_witnesseth(clean):
        return [Paragraph(_markup_or_xml(clean), LEASE_STYLES["WITNESSETH"])]
    if _is_party_label(clean):
        return [Paragraph(_markup_or_xml(clean), LEASE_STYLES["PARTY_LABEL"])]
    if _is_party_name(clean):
        return [Paragraph(_markup_or_xml(clean), LEASE_STYLES["PARTY_NAME"])]
    ...
    if _is_article_heading(clean):
        return [Paragraph(_markup_or_xml(clean), LEASE_STYLES["ARTICLE_CENTERED"])]
    ...
    if header:
        ...
        if rest:
            return [Paragraph(_markup_or_xml(f"{marker} {rest}"), LEASE_STYLES["ARTICLE_HEADER"])]
        return [Paragraph(_markup_or_xml(marker), LEASE_STYLES["ARTICLE_HEADER"])]

    if re.match(r"^[a-z]\)\s+", clean) or re.match(r"^[ivxlcdm]+[\.)]\s+", clean, flags=re.IGNORECASE):
        return [Paragraph(_markup_or_xml(clean), LEASE_STYLES["INDENTED_BODY"])]

    return [Paragraph(_markup_or_xml(clean), LEASE_STYLES["CLAUSE_BODY"])]
