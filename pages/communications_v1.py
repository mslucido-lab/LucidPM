...
class CommunicationsState(AppState):
    ...
    def log_message_to_tenant(self):
        ...
        if not self.log_tenant_id or self.log_tenant_id <= 0:
            ...
            return
        subject = str(self.selected_message.get("subject") or "(No subject)")
        body = str(self.selected_message.get("body") or "")
        from_address = str(self.selected_message.get("from_address") or "")
        try:
            prop_rows = run_query(
            ...
            run_exec(
                ...
                VALUES (?, SYSDATETIME(), 'Email', ?, ?, ?, 'Received', ?)
                """,
                (int(self.log_tenant_id), subject, body, from_address, prop_id),
                db=self.db,
            )
