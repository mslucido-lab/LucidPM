...
def _story_for_text_sections(section_texts: Sequence[str]) -> list:
    ...
    for section_index, text in enumerate(texts):
        ...
        for block in blocks:
            ...
            # Parent-only headings like "22. SPECIAL CONDITIONS" should not sit
            # alone above the footer with their child clauses starting on the
            # next page. Require enough remaining space for the heading plus the
            # first lettered child clause before rendering the parent.
            min_remaining = int(2.25 * 72) if _is_special_conditions_parent(block) else int(0.75 * 72)
            story.append(CondPageBreak(min_remaining))
