...
class ProformaState(AppState):
    ...
    def on_load(self):
        ...
        self.year_options = list(range(datetime.date.today().year + 2, 2000, -1))
    ...
    def _calc_month_amount(self, segments: list[dict], year: int, month: int) -> float:
    ...
    def _do_compute(self):
        ...
        lease_rows = run_query(
            ...
            "t.Suite AS TenantSuite "
            "FROM Leases l "
            "INNER JOIN Tenants t ON l.TenantID = t.TenantID "
            "WHERE l.PropertyID = ? ORDER BY l.LeaseStart, l.LeaseID",
            (prop_id,), db=self.db,
        )
            ...
            for suite in suites:
                ...
                for l in suite_leases:
                    ...
                    if ls <= month_end and le >= month_start:
                        ...
                        amount += self._calc_month_amount(segs, year, month_num)
                ...
                if not active_found:
                    ...
                    if rec and rec > 0:
                        # Don't show recommended rent in the same month a lease expired mid-month
                        allow = True
                        for l in suite_leases:
                            le = to_date(l.get("LeaseEnd"))
...
def suite_header_cell(s: str) -> rx.Component:
    return rx.table.column_header_cell(rx.text(s, size="2", weight="bold"))
            ...
            rx.hstack(
                rx.vstack(
                    ...
                    rx.select(
                        ...
                        value=ProformaState.proforma_year,
