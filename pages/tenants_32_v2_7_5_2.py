...
class TenantState(AppState):
    ...
    # Attachment picker
    attach_selected_paths: list[str] = []
    attach_error: str = ""
    ...
    def open_file_picker(self):
        """Call backend endpoint to open native Windows file dialog."""
        import httpx
        self.attach_error = ""
        try:
            response = httpx.get("http://localhost:8000/api/pick-files", timeout=60)
            data = response.json()
            if data.get("error"):
                self.attach_error = str(data.get("error") or "File picker failed.")
                self.attach_selected_paths = []
                return
            paths = data.get("paths", [])
            self.attach_selected_paths = [str(p) for p in paths if p]
        except Exception as ex:
            self.attach_error = f"File picker failed: {ex}"
    ...
    def send_compose_email(self):
        ...
        try:
            send_email(
                ...
                db=self.db,
                attachment_paths=self.attach_selected_paths if self.attach_selected_paths else None,
            )
        except Exception as ex:
...
def attach_file_row_tenant(path: str) -> rx.Component:
    return rx.text(
        "📎 " + path,
        size="2",
        color=BRAND_DARK,
...
def attachment_picker_panel_tenant() -> rx.Component:
    return rx.vstack(
        rx.hstack(
            ...
            rx.button(
                "Browse...",
                on_click=TenantState.open_file_picker,
                variant="outline",
                size="2",
            ),
            rx.cond(
                TenantState.attach_selected_paths.length() > 0,
                rx.button(
                    "Clear",
        ...
        rx.cond(
            TenantState.attach_error != "",
            rx.text(TenantState.attach_error, size="1", color="#D96B6B"),
            rx.fragment(),
        ),
        rx.cond(
            TenantState.attach_selected_paths.length() > 0,
            rx.vstack(
                rx.foreach(
                    TenantState.attach_selected_paths,
