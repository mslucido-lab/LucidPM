...
def attach_file_row_comms(path: str) -> rx.Component:
    return rx.text(
        "📎 " + path,
        size="2",
        color=BRAND_DARK,
...
def attachment_picker_panel_comms() -> rx.Component:
    return rx.vstack(
        rx.hstack(
            ...
            rx.button(
                "Browse...",
                on_click=CommunicationsState.open_file_picker,
                variant="outline",
                size="2",
            ),
            rx.cond(
                CommunicationsState.attach_selected_paths.length() > 0,
                rx.button(
                    "Clear",
        ...
        rx.cond(
            CommunicationsState.attach_error != "",
            rx.text(CommunicationsState.attach_error, size="1", color="#D96B6B"),
            rx.fragment(),
        ),
        rx.cond(
            CommunicationsState.attach_selected_paths.length() > 0,
            rx.vstack(
                rx.foreach(
                    CommunicationsState.attach_selected_paths,
