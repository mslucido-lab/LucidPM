...
class PropertyFinancialsState(AppState):
    ...
    def _recompute_preview(self):
        try:
            ...
            noi  = rev - opex
            self.noi_preview = f"${noi:,.0f}"
            if self.cap_rate > 0 and noi > 0:
                est = noi / (self.cap_rate / 100.0)
                self.estimated_value_preview = f"${est:,.0f}"
    ...
    def _recompute_valuations(self):
        ...
        for row in self.years:
            ...
            updated.append(FinancialYear(
                ...
                noi=row.noi,
                estimated_value=f"${est:,.0f}" if est > 0 else "",
                price_per_sf=f"${psf:,.2f}" if psf > 0 else "",
                notes=row.notes,
            ))
        self.years = updated
...
def property_financials_content() -> rx.Component:
    return rx.box(
        rx.vstack(
            ...
            rx.hstack(
                ...
                rx.vstack(
                    ...
                    rx.slider(
                        ...
                        on_change=PropertyFinancialsState.set_cap_rate,
                        width="200px",
                    ),
                    spacing="1",
                ),
            ...
            # Summary table
            rx.cond(
                PropertyFinancialsState.years.length() > 0,
                rx.box(
                    rx.table.root(
                        rx.table.header(
                            rx.table.row(
                        ...
                        rx.table.body(rx.foreach(PropertyFinancialsState.years, year_row)),
