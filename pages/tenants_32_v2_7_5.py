...
v2.7.5 — Adds PDF attachment picker to tenant outbound email compose modal.
...
class TenantState(AppState):
    ...
    # Attachment picker
    attach_folder: str = DEFAULT_ATTACHMENT_FOLDER
    attach_file_list: list[str] = []
    attach_selected: list[str] = []
    attach_folder_error: str = ""
    ...
    def set_attach_folder(self, v: str):
        self.attach_folder = v

    def load_attach_file_list(self):
        """List PDF files in the current attach_folder."""
        self.attach_folder_error = ""
        self.attach_selected = []
        folder = str(self.attach_folder or "").strip()
        if not folder or not os.path.isdir(folder):
            self.attach_file_list = []
            self.attach_folder_error = "Folder not found. Check the path and try again."
            return
        try:
            files = [
            ...
            self.attach_file_list = sorted(files)
            if not files:
                self.attach_folder_error = "No PDF files found in this folder."
        except Exception as ex:
            self.attach_file_list = []
            self.attach_folder_error = f"Could not read folder: {ex}"

    def toggle_attach_file(self, filename: str):
        """Toggle a file in the selected attachments list."""
        if filename in self.attach_selected:
            self.attach_selected = [f for f in self.attach_selected if f != filename]
        else:
            self.attach_selected = self.attach_selected + [filename]
    ...
    def open_compose_email(self):
        ...
        self.compose_body = ""
        self.compose_sending = False
        self.attach_folder = DEFAULT_ATTACHMENT_FOLDER
        self.attach_file_list = []
        self.attach_selected = []
        self.attach_folder_error = ""
    ...
    def send_compose_email(self):
        ...
        attachment_paths = [
            os.path.join(self.attach_folder, f)
            for f in self.attach_selected
            if os.path.isfile(os.path.join(self.attach_folder, f))
        ]
...
def attach_file_row_tenant(filename: str) -> rx.Component:
    return rx.hstack(
        rx.checkbox(
            checked=TenantState.attach_selected.contains(filename),
            on_change=TenantState.toggle_attach_file(filename),
        ),
...
def attachment_picker_panel_tenant() -> rx.Component:
    return rx.vstack(
        ...
        rx.hstack(
            rx.input(
                value=TenantState.attach_folder,
                on_change=TenantState.set_attach_folder,
                placeholder=r"C:\path\to\folder",
                width="100%",
            ),
            rx.button(
                "Browse",
                on_click=TenantState.load_attach_file_list,
                variant="outline",
                size="2",
            ),
            width="100%",
        ...
        rx.cond(
            TenantState.attach_folder_error != "",
            rx.text(
                TenantState.attach_folder_error,
                size="1",
                color="#D96B6B",
        ...
        rx.cond(
            TenantState.attach_file_list.length() > 0,
            rx.vstack(
                rx.text(
                    "Select files to attach (PDF only):",
                    size="1",
                    color="#666",
                ),
                rx.foreach(
                    TenantState.attach_file_list,
        ...
        rx.cond(
            TenantState.attach_selected.length() > 0,
            rx.text(
                TenantState.attach_selected.length().to_string() + " file(s) selected",
                size="1",
                color=BRAND_DARK,
                weight="medium",
            ),
