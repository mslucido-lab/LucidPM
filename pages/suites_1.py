...
class SuiteState(AppState):
    ...
    def _load_property_options(self):
        ...
        self._property_names = [str(r["PropertyName"]) for r in rows]
        self._property_ids   = [int(r["PropertyID"]) for r in rows]
        self.property_filter_options = ["All"] + self._property_names
        if self._property_names:
            self.f_property = self._property_names[0]
    ...
    def select_suite(self, suite_id: int):
        ...
        self.f_property = str(r.get("PropertyName") or (self._property_names[0] if self._property_names else ""))
        sq = r.get("SquareFeet")
    ...
    def new_suite(self):
        ...
        self.f_property = self._property_names[0] if self._property_names else ""
    ...
    def save_suite(self):
        ...
        if not self.f_label.strip():
            ...
            return
        if not self.f_property or self.f_property not in self._property_names:
            self.form_error = "Property is required."
            return
        prop_id = self._property_ids[self._property_names.index(self.f_property)]
