...
OPEN_STATUSES = {"New", "Open", "In Progress", "Waiting on Vendor",
                 "Waiting on Tenant", "Scheduled", "Deferred"}
...
class WorkItemState(AppState):
    ...
    def select_winning_bid(self):
        ...
        try:
            run_exec(
                "UPDATE WorkItemBids SET IsSelected=0, UpdatedDate=? WHERE WorkItemID=?",
                (now, int(self.selected_id)), db=db,
            )
            run_exec(
                "UPDATE WorkItemBids SET IsSelected=1, SelectedDate=?, UpdatedDate=? WHERE WorkItemBidID=?",
                (now, now, int(self.selected_bid_id)), db=db,
            )
            self.bid_form_success = "Bid selected as winner."
            self._load_bids(self.selected_id)
        except Exception as ex:
                ...
                rx.table.header(
                    rx.table.row(
                        rx.table.column_header_cell("Title / Category",
                            style={"font_size": "11px", "color": BRAND_PRIMARY, "padding": "6px 10px"}),
                        rx.table.column_header_cell("Property / Tenant",
                            style={"font_size": "11px", "color": BRAND_PRIMARY, "padding": "6px 10px"}),
                        rx.table.column_header_cell("Status / Due",
                            style={"font_size": "11px", "color": BRAND_PRIMARY, "padding": "6px 10px"}),
