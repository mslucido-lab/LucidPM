...
Patched 5/4/2026:
- v1.5.28: prevents Special Conditions parent headers from orphaning at the bottom of a page.
- v1.5.27: escapes stray angle brackets inside trusted markup so bold child clauses do not break ReportLab parsing.
...
- v1.5.33: adds safe ReportLab Paragraph fallback that strips outer <para> wrappers and balances inline tags before final plain-text fallback.
- v1.5.32: balances nested outer ReportLab tags when <br/> splits bold/font/para markup.
- v1.5.31: safely preserves outer inline tags when <br/> tags split markup into paragraphs.
- v1.5.30: keeps Special Conditions parent with first child and treats <br/> tags as paragraph breaks.
- v1.5.29: strengthens Special Conditions parent anti-orphan page-break threshold.
- v1.5.34: flattens text-section blocks for robust Special Conditions parent/first-child anti-orphan handling.
- v1.5.35: forces Special Conditions parent sections to start on a new page when followed by child conditions.
- v1.5.39c: centers grouped 3+ year payment schedule tables using a full-frame wrapper.
- v1.5.38: centers grouped 3+ year payment schedule tables directly in the page frame.
- v1.5.37: renders grouped 3+ year payment schedules as a wide 2-column table instead of compressed 4-column layout.
...
from reportlab.platypus import SimpleDocTemplate, Spacer, Preformatted, KeepTogether, Paragraph, CondPageBreak, PageBreak, Table, TableStyle
...
def _page_setup_with_word_margins() -> dict:
    """Return page setup with Word-baseline margins and reserved footer space.

    The signature footer is drawn manually in the bottom margin on every generated
    text page. ReportLab decides pagination from the document frame before that
    footer is drawn. If the frame extends too low, headings can appear above the
    footer while their body text rolls to the next page. Reserve enough bottom
    margin here so pagination decisions account for the footer area.
    """
    setup = dict(PAGE_SETUP)
    setup["leftMargin"] = 0.5 * inch
    setup["rightMargin"] = 0.5 * inch
    setup["bottomMargin"] = max(float(setup.get("bottomMargin", 0) or 0), 1.35 * inch)
    return setup
...
def _sanitize_reportlab_markup(text: str) -> str:
    ...
    This lets authored content use <b>, <i>, <u>, <font>, <para>, and <br/>
    while preventing accidental text like signi<ng from being parsed as a tag.
    Physical line breaks are collapsed to spaces so legal paragraphs justify
    across the full available line width. Use <br/> for intentional breaks.
    """
...
def _strip_para_tags(markup: str) -> str:
    """Remove ReportLab <para> wrappers while preserving inline markup.

    SimpleDocTemplate already supplies the paragraph style. Stripping <para>
    is a safe fallback for malformed nested markup such as <para><b><font>...
    when ReportLab reports unclosed para/b/font tags.
    """
    text = str(markup or "")
...
def _balance_inline_tags(markup: str) -> str:
    """Append missing closing tags for simple inline ReportLab markup.

    This is a defensive fallback for user-authored snippets. It preserves common
    inline styling and prevents a malformed split from aborting package generation.
    """
...
def _safe_paragraph(markup: str, style):
    """Create a ReportLab Paragraph without allowing malformed markup to crash generation."""
    text = str(markup or "").strip()
    attempts = []
    if text:
    ...
    # Final safety net: remove all tags and render escaped text so generation completes.
    plain = re.sub(r"<[^>]+>", "", text)
    try:
...
def _schedule_table_flowable(lines: list[str]):
    ...
    if has_two_column_rows:
        ...
        # Grouped 3+ year schedules are one payment period per row. Render them
        # as a wider two-column table so the amount stays on the same line.
        table_width = min(5.75 * 72, clause_body_width)
    ...
    if not has_two_column_rows:
        # Grouped schedules are narrower than the normal 4-column monthly table.
        # Center the inner table inside a full-frame wrapper so it is visually
        # centered between the page margins, not left-biased by clause indentation.
        outer = Table(
            [[tbl]],
...
def _outer_inline_tags(text: str) -> tuple[list[tuple[str, str, str]], str]:
    """Return nested outer ReportLab tags that wrap the whole text, plus inner text.

    Handles common authored patterns such as:
      <b>paragraph one<br/>paragraph two</b>
      <para><b><font size="10">paragraph one<br/>paragraph two</font></b></para>

    The returned wrappers preserve each opening tag, tag name, and closing tag so
    every paragraph created from a <br/> split can be rewrapped with balanced tags.
    """
    ...
    while True:
        ...
        wrappers.append((open_tag, tag_name, close_tag))
...
def _rewrap_inline_tags(block: str, wrappers: list[tuple[str, str, str]]) -> str:
    """Wrap a paragraph block with balanced tags stripped by _outer_inline_tags."""
...
def _split_blocks(text: str) -> list[str]:
    ...
    # Treat authored <br/> tags as paragraph breaks for legal body text.
    # If the whole block is wrapped in ReportLab tags, strip and reapply those
    # wrappers so every split paragraph has balanced markup. This prevents
    # ReportLab errors like "parse ended with unclosed tags b font para."
    wrappers, inner_text = _outer_inline_tags(text)
...
def _plain_text_for_detection(block: str) -> str:
    """Strip ReportLab markup and normalize whitespace for pattern detection."""
    plain = re.sub(r"<[^>]+>", "", str(block or "")).strip()
    return re.sub(r"\s+", " ", plain)


def _is_special_conditions_parent(block: str) -> bool:
    """True for parent Special Conditions headers that need room for first child."""
    plain = _plain_text_for_detection(block)
    return bool(re.match(r"^[0-9]{1,3}\.\s*SPECIAL CONDITIONS\b", plain, flags=re.IGNORECASE))


def _is_special_conditions_child(block: str) -> bool:
    """True for lettered Special Conditions child clauses."""
    plain = _plain_text_for_detection(block)
    return bool(re.match(r"^[a-k][\.)]\s+", plain, flags=re.IGNORECASE))
...
def _story_for_text_sections(section_texts: Sequence[str]) -> list:
    ...
    Special handling:
      - Keep the parent "22. SPECIAL CONDITIONS" header with the first lettered
        child condition even when those are separate library sections.
      - Flatten section blocks before rendering so this works whether the parent
        is a header-only section, a section with helper text, or the last block
        in a larger rendered text run.
    """
    story = []
    ...
    # Flatten all rendered text sections into a single block stream. This lets
    # anti-orphan logic see across library-section boundaries.
    ...
    idx = 0
    while idx < len(flat_blocks):
        block = flat_blocks[idx]

        # Special Conditions should never begin at the bottom of a page above
        # the signature footer. Start the parent header on a clean page, then let
        # child conditions flow naturally from there. This is intentionally a hard
        # rule because the footer is drawn outside the normal story flow.
        if _is_special_conditions_parent(block):
            if story:
