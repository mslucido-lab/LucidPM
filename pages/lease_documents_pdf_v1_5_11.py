...
try:
    ...
    LEASE_STYLES = {
        "CLAUSE_BODY": ParagraphStyle("ClauseBody", fontName="Times-Roman", fontSize=10, leading=13, alignment=4, leftIndent=18, firstLineIndent=-18, spaceAfter=6),
        "ARTICLE_HEADER": ParagraphStyle("ArticleHeader", fontName="Times-Bold", fontSize=10, leading=12, keepWithNext=True, spaceBefore=6, spaceAfter=8),
        ...
        "INDENTED_BODY": ParagraphStyle("IndentedBody", fontName="Times-Roman", fontSize=10, leading=13, alignment=4, leftIndent=36, firstLineIndent=0, spaceAfter=6),
...
def _is_payment_schedule_line(line: str) -> bool:
    ...
    return bool(re.search(month, text, flags=re.IGNORECASE)) and bool(
        re.search(r"(^|\s)[a-l]\)", text, flags=re.IGNORECASE) or re.search(r"\d{1,2},\s*\d{4}", text)
    )


def _split_payment_schedule_row(line: str) -> list[str]:
    """Split one schedule line into one or two left-aligned schedule cells.

    Times-Roman is proportional, so spaces cannot be used for alignment.
    This parser turns lines like:
      a) September 1, 2025: $1,300.00   g) March 1, 2026: $1,300.00
    into two table cells.
    """
    text = str(line or "").strip()
    if not text:
        return []

    # Match each lettered schedule item and stop before the next lettered item.
    parts = re.findall(
        r"[a-l]\)\s+.*?(?=\s+[a-l]\)\s+|$)",
        text,
        flags=re.IGNORECASE,
...
def _schedule_table_flowable(lines: list[str]):
    """Render schedule rows as a centered two-column block within the clause body."""
    rows = []
    for ln in lines or []:
        ...
        if len(parts) == 1:
            parts.append("")
        rows.append([
            Paragraph(_xml(parts[0]), LEASE_STYLES["PAYMENT_SCHEDULE"]),
            Paragraph(_xml(parts[1]), LEASE_STYLES["PAYMENT_SCHEDULE"]),
        ])

    if not rows:
        return []

    # Center the schedule block, but left-align each column.
    # This keeps Times-Roman while avoiding broken proportional-space alignment.
    table_width = 4.95 * 72
    col_w = table_width / 2
...
def _payment_clause_flowables(block: str):
    ...
    for raw in lines:
        ...
        flush_schedule()
        flowables.append(Paragraph(_xml(line), LEASE_STYLES["CLAUSE_BODY"]))
...
def _paragraph_for_block(block: str):
    ...
    if header:
        ...
        combined = f"{marker}  {rest}" if rest else marker
        hanging_style = ParagraphStyle(
            "ClauseHanging",
            parent=LEASE_STYLES["CLAUSE_BODY"],
            firstLineIndent=-0.25 * inch,
        )
        return [Paragraph(_xml(combined), hanging_style)]
