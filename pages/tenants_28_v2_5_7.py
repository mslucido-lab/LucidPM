...
class TenantState(AppState):
    ...
    def load_screening_records(self):
        ...
        try:
            rows = run_query(
                ...
                "CreditScore, Evictions, Bankruptcies, CriminalResult, EvictionResult, "
                "CreditSourceType, RiskTier, DepositRecommended "
    ...
    def save_screening_record(self):
        ...
        try:
            ...
            self.show_screening_form = False
            self.load_screening_records()
            self.screening_form_success = "Screening record saved."
        except Exception as ex:
...
def screening_form() -> rx.Component:
    return rx.vstack(
        rx.text("Add screening record", size="3", weight="bold", color=BRAND_DARK),
        rx.grid(
        ...
        rx.grid(
            _form_field("Overall Result", rx.select(["", "Pass", "Fail", "Conditional"], value=TenantState.s_overall_result, on_change=TenantState.set_s_overall_result, size="2", width="100%")),
            ...
            _form_field("Risk Tier", rx.select(["", "Low", "Moderate", "Elevated", "High"], value=TenantState.s_risk_tier, on_change=TenantState.set_s_risk_tier, size="2", width="100%")),
            columns="3", spacing="4", width="100%",
        ),
        rx.grid(
        ...
        rx.grid(
            ...
            _form_field("Income:Rent Ratio", rx.input(value=TenantState.s_income_to_rent, on_change=TenantState.set_s_income_to_rent, type="number", size="2", width="100%")),
            _form_field("Income:Debt Ratio", rx.input(value=TenantState.s_income_to_debt, on_change=TenantState.set_s_income_to_debt, type="number", size="2", width="100%")),
            _form_field("Income:Debt Incl. Rent", rx.input(value=TenantState.s_income_to_debt_incl_rent, on_change=TenantState.set_s_income_to_debt_incl_rent, type="number", size="2", width="100%")),
            columns="4", spacing="4", width="100%",
        ),
        rx.grid(
            _form_field("Criminal Search", rx.select(["", "Clear", "Records Found"], value=TenantState.s_criminal_result, on_change=TenantState.set_s_criminal_result, size="2", width="100%")),
            _form_field("Eviction Search", rx.select(["", "Clear", "Records Found"], value=TenantState.s_eviction_result, on_change=TenantState.set_s_eviction_result, size="2", width="100%")),
        ...
        rx.hstack(
            rx.button("Save screening record", on_click=TenantState.save_screening_record, color_scheme="blue", size="2"),
