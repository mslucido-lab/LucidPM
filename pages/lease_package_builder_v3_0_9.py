...
class LeasePackageBuilderState(AppState):
    ...
    def _section_consumes_section_number(self, section: LeasePackageSection, rendered_text: str) -> bool:
        """Return True when this rendered section should consume a section number.

        Only sections that actually use the dynamic {{SectionNumber}} token
        consume a legal article number. This keeps document headers, static
        PDFs, and authored child sections from shifting the counter.
        """
    ...
    def _strip_opening_number_marker(self, body: str) -> tuple[str, str, str, str]:
        """Return opening number marker pieces and body with marker removed.

        Supports:
            <b>1.</b> Body
            <b>1. Existing Title</b> Body
            1. Body
            1) Body

        Returns (open_tags, number_marker, existing_title, remaining_body).
        """
        raw = str(body or "").strip()

        # Common ReportLab inline markup pattern.
        wrapped = re.match(
            r"^(?P<open>(?:<[^/][^>]*>)*)\s*"
            r"(?P<num>\d+[\.)])\s*"
            r"(?P<title>.*?)"
        ...
        if wrapped:
            open_tags = wrapped.group("open") or ""
            number_marker = wrapped.group("num") or ""
            title = (wrapped.group("title") or "").strip()
            close_tags = wrapped.group("close") or ""
            rest = (wrapped.group("rest") or "").lstrip()
            # Only treat text inside tags as an authored title when a closing
            # tag exists. Otherwise plain body text belongs below the heading.
            existing_title = title if close_tags else ""
            remaining = rest if close_tags else (title + " " + rest).strip()
            return open_tags, number_marker, existing_title, remaining

        plain = re.match(r"^(?P<num>\d+[\.)])\s*(?P<rest>.*)$", raw, flags=re.DOTALL)
        if plain:
            return "", plain.group("num"), "", (plain.group("rest") or "").lstrip()

        return "", "", "", raw

    def _inject_display_label_after_section_number(self, section: LeasePackageSection, rendered_text: str) -> str:
        """Convert an authored {{SectionNumber}} marker into a proper heading.

        ArticleNumber can stay populated for admin sorting. Visible numbering is
        driven by {{SectionNumber}} in the content. If DisplayLabel exists, it
        becomes the heading title. The body starts below the heading.
        """
        label = str(getattr(section, "display_label", "") or "").strip()
        ...
        open_tags, number_marker, existing_title, remaining = self._strip_opening_number_marker(body)

        if not number_marker:
            return body

        heading_title = label or existing_title
        if heading_title:
            heading_text = f"{number_marker} {heading_title}"
        else:
            heading_text = number_marker

        heading = f"{open_tags}{heading_text}</b>" if open_tags and "<b" in open_tags.lower() and "</b>" not in open_tags.lower() else f"{open_tags}{heading_text}"
    ...
    def _compose_section_render_text(self, section: LeasePackageSection, rendered_text: str) -> str:
        """Prepend clause metadata as a legal heading when appropriate.

        ArticleNumber remains useful for admin sorting/library organization.
        If the authored section content uses {{SectionNumber}}, numbering is
        content-driven and the metadata heading must not be prepended.
        """
        body = str(rendered_text or "").strip()
        ...
        try:
            ...
            for idx, p in enumerate(selected, start=1):
                ...
                if self._has_renderable_text(p):
                    ...
                    consumes_section_number = self._section_consumes_section_number(p, rendered_text)
        ...
        try:
            ...
            for idx, section in enumerate(selected, start=1):
                ...
                if self._has_renderable_text(section):
                    ...
                    consumes_section_number = self._section_consumes_section_number(section, rendered_text)
        ...
        try:
            ...
            for idx, row in enumerate(rows, start=1):
                ...
                if text_to_render or self._has_renderable_text(temp_section_meta):
                    ...
                    consumes_section_number = self._section_consumes_section_number(temp_section_meta, rendered_text)
