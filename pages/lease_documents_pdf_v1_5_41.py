...
Patched 5/4/2026:
- v1.5.41: baseline from v1.5.26; preserves header behavior, adds safe <br/> paragraph handling for body clauses, and grouped 3+ year payment schedule layout.
...
def _safe_paragraph(markup: str, style):
    """Create a Paragraph, falling back to balanced or plain text if markup is malformed."""
    text = str(markup or "").strip()
    attempts = []
    if text:
...
def _should_preserve_br_inside_single_block(text: str) -> bool:
    """Do not split header-style blocks on <br/>. This preserves lease header layout."""
    plain = _plain_text_for_detection(text)
    if _is_doc_title(plain) or _is_jurisdiction(plain) or _is_witnesseth(plain):
        return True
    if _is_party_label(plain) or _is_party_name(plain) or _is_signature_block(plain):
        return True
    # Short header-ish blocks with one or two lines should not be converted into body paragraphs.
    lines = [ln.strip() for ln in plain.split("\n") if ln.strip()]
    if len(lines) <= 3 and len(plain) <= 180 and not re.search(r"[.!?].+<br", str(text or ""), flags=re.IGNORECASE):
        return True
    return False
...
def _split_blocks(text: str) -> list[str]:
    ...
    # Preserve header blocks exactly. Only body clauses use <br/> as paragraph separators.
    if "<br" in text.lower() and not _should_preserve_br_inside_single_block(text):
