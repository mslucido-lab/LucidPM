...
# VERSION: 2.8.9
# - Fixed section delete/archive and section edit/toggle queries for databases where
#   LeaseDocumentSections primary key is SectionID instead of LeaseDocumentSectionID.
...
#      LeaseGeneratedDocumentSections. Verify the column name SectionID
...
class LeaseDocumentState(AppState):
    ...
    def _load_sections(self):
        if not self.selected_source_document_id:
            ...
            return
        rows = run_query(
            "SELECT p.SectionID, p.SectionType, p.SectionName, p.ExhibitCode, "
            ...
            "WHERE p.LeaseSourceDocumentID = ? ORDER BY p.SortOrder, p.SectionID",
            (self.selected_source_document_id,), db=self.db,
        )
        self.sections = [
            SectionRow(
    ...
    def _load_all_sections(self):
        ...
        try:
            rows = run_query(
                ...
                "ORDER BY sd.TemplateName, p.SortOrder, p.SectionID",
                db=self.db,
            )
        except Exception:
            rows = []
    ...
    def _validate_section_range(self, start: int, end: int, ignore_section_id: int = 0, require_non_overlap: bool = True) -> bool:
        ...
        rows = run_query(
            "SELECT SectionID, StartPage, EndPage, SectionName "
            "FROM LeaseDocumentSections "
            "WHERE LeaseSourceDocumentID = ? AND IsActive = 1 AND SectionID <> ?",
    ...
    def _is_metadata_only_section_update(self, section_id: int, start: int, end: int) -> bool:
        if not section_id:
            return False
        rows = run_query(
            "SELECT StartPage, EndPage FROM LeaseDocumentSections WHERE SectionID = ?",
    ...
    def edit_section(self, section_id: int):
        ...
        self.form_success = ""
        rows = run_query(
            "SELECT SectionID, SectionType, SectionName, ExhibitCode, StartPage, EndPage, "
            "SortOrder, IsReusable, IsActive, Content FROM LeaseDocumentSections WHERE SectionID = ?",
            (int(section_id),), db=self.db,
        )
        if not rows:
    ...
    def delete_section(self, section_id: int):
        ...
        rows = run_query(
            "SELECT StoredFilePath FROM LeaseDocumentSections WHERE SectionID = ?",
        ...
        try:
            used = run_query(
                ...
                "FROM LeaseGeneratedDocumentSections WHERE SectionID = ?",
                (section_id_int,), db=self.db,
            )
            ...
            if used or used_in_package_sections:
                run_exec(
                    "UPDATE LeaseDocumentSections SET IsActive = 0 WHERE SectionID = ?",
                    (section_id_int,), db=self.db,
                )
                self.form_success = "Section is used by a lease package - archived instead of deleted."
            else:
                run_exec("DELETE FROM LeaseDocumentSections WHERE SectionID = ?", (section_id_int,), db=self.db)
    ...
    def set_section_reusable_flag(self, section_id: int, reusable: bool):
        ...
        try:
            run_exec(
                "UPDATE LeaseDocumentSections SET IsReusable = ? WHERE SectionID = ?",
    ...
    def toggle_section_reusable(self, section_id: int):
        ...
        try:
            rows = run_query(
                "SELECT IsReusable FROM LeaseDocumentSections WHERE SectionID = ?",
                (int(section_id),), db=self.db,
            )
            if not rows:
                self.form_error = "Section not found."
    ...
    def toggle_section_active(self, section_id: int):
        ...
        try:
            run_exec(
                "UPDATE LeaseDocumentSections SET IsActive = CASE WHEN IsActive = 1 THEN 0 ELSE 1 END WHERE SectionID = ?",
                (int(section_id),), db=self.db,
            )
            self._load_sections()
            self._load_all_sections()
    ...
    def save_section_content(self):
        ...
        try:
            run_exec(
                "UPDATE LeaseDocumentSections SET Content = ? WHERE SectionID = ?",
    ...
    def create_section(self):
        ...
        if self.p_section_type == "Exhibit" and code:
            dup = run_query(
                "SELECT TOP 1 SectionID FROM LeaseDocumentSections "
                "WHERE LeaseSourceDocumentID = ? AND SectionType = 'Exhibit' "
                "AND UPPER(ISNULL(ExhibitCode,'')) = UPPER(?) AND SectionID <> ?",
    ...
    def _load_reusable_section_options(self):
        ...
        try:
            rows = run_query(
                "SELECT p.SectionID, p.SectionType, p.SectionName, ISNULL(p.ExhibitCode,'') AS ExhibitCode, "
                "p.SortOrder, ISNULL(sd.TemplateName, '') AS TemplateName, ISNULL(sd.OriginalFileName, '') AS OriginalFileName, "
                "sd.PropertyID, sd.LeaseSourceDocumentID "
                "FROM LeaseDocumentSections p "
                "INNER JOIN LeaseSourceDocuments sd ON p.LeaseSourceDocumentID = sd.LeaseSourceDocumentID "
                "WHERE ISNULL(p.IsReusable, 1) = 1 AND ISNULL(p.IsActive, 1) = 1 "
                "AND ISNULL(sd.IsActive, 1) = 1 "
                "ORDER BY ISNULL(sd.TemplateName, ''), p.SectionType, p.SortOrder, p.SectionName, p.SectionID",
                db=self.db,
            )
        except Exception:
            rows = []
    ...
    def _section_label_for_id(self, section_id) -> str:
        ...
        rows = run_query(
            ...
            "ISNULL(sd.TemplateName, '') AS TemplateName, ISNULL(sd.OriginalFileName, '') AS OriginalFileName, "
            "sd.PropertyID, sd.LeaseSourceDocumentID "
