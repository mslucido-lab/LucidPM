...
class LeaseDocumentState(AppState):
    ...
    def _ensure_selected_template_before_slot_save(self) -> bool:
        """Recover selected template ID if Reflex state lost it during a tab/render transition."""
        if int(self.selected_template_id or 0) > 0:
            return True

        if self.selected_template_label and self.selected_template_label in self.lease_template_labels:
            try:
                idx = self.lease_template_labels.index(self.selected_template_label)
                self.selected_template_id = int(self.lease_template_ids[idx])
                self.lt_template_mode = "edit"
                return int(self.selected_template_id or 0) > 0
            except Exception:
                pass
        ...
        try:
            ...
            if rows:
                self.selected_template_id = int(rows[0].get("LeaseTemplateID") or 0)
                self.lt_template_mode = "edit"
                return int(self.selected_template_id or 0) > 0
        except Exception:
            return False
