...
MVP scope:
  ...
  3. Split the source PDF into reusable base lease, exhibit, and addendum sections.

This version stores files on disk and keeps metadata in SQL Server.
"""

# VERSION: 2.7.5 - Section-only schema cleanup; removes legacy section naming from admin code.
...
class LeaseDocumentState(AppState):
    ...
    def set_p_section_type(self, v: str):
        ...
        if v == "Base Lease":
            self.p_exhibit_code = ""
            # Default new Base Lease sections to reusable, but do not override while editing.
            if int(self.editing_section_id or 0) == 0:
                self.p_is_reusable = True
            if not self.p_section_name.strip():
                self.p_section_name = "Base Lease"
        ...
        rx.card(
            rx.vstack(
                ...
                rx.grid(
                    ...
                    rx.vstack(rx.text("Section name", size="1", color="#666"), rx.input(value=LeaseDocumentState.p_section_name, on_change=LeaseDocumentState.set_p_section_name, placeholder="Base Lease or Special Terms", width="100%"), spacing="1"),
