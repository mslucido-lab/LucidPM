...
# VERSION: 2.8.0
# - Refactored from linear scroll layout to 4-tab layout (Load / Parse & Section / Section Library / Package Templates).
# - Fixed IsOptional / IsRequired mutual-exclusion: replaced two independent booleans with
#   a single sec_inclusion_mode string ("Required" | "Optional" | "Inactive").
# - Removed duplicate SECTION_TYPES assignment.
# - Active tab tracked in state (admin_lease_tab) so tab selection survives re-renders.
...
class LeaseDocumentState(AppState):
    ...
    def _load_sections(self):
        if not self.selected_source_document_id:
            ...
            return
        rows = run_query(
            "SELECT p.LeaseDocumentSectionID, p.SectionType, p.SectionName, p.ExhibitCode, "
        ...
        self.sections = [
            SectionRow(
                section_id=int(r["LeaseDocumentSectionID"]),
    ...
    def _validate_section_range(self, start: int, end: int, ignore_section_id: int = 0, require_non_overlap: bool = True) -> bool:
        ...
        rows = run_query(
            "SELECT LeaseDocumentSectionID, StartPage, EndPage, SectionName "
    ...
    def edit_section(self, section_id: int):
        ...
        self.form_success = ""
        rows = run_query(
            "SELECT LeaseDocumentSectionID, SectionType, SectionName, ExhibitCode, StartPage, EndPage, "
        ...
        if not rows:
            ...
            return
        r = rows[0]
        self.editing_section_id = int(r["LeaseDocumentSectionID"])
    ...
    def delete_section(self, section_id: int):
        ...
        try:
            used = run_query(
                "SELECT TOP 1 LeaseGeneratedDocumentSectionID "
                "FROM LeaseGeneratedDocumentSections WHERE LeaseDocumentSectionID = ?",
                (section_id_int,), db=self.db,
            )
            used_in_package_sections = run_query(
                "SELECT TOP 1 LeasePackageSectionID FROM LeasePackageSections WHERE SectionID = ?",
                (section_id_int,), db=self.db,
            )
            if used or used_in_package_sections:
                run_exec(
                    "UPDATE LeaseDocumentSections SET IsActive = 0 WHERE LeaseDocumentSectionID = ?",
                    (section_id_int,), db=self.db,
                )
                self.form_success = "Section is used by a lease package — archived instead of deleted."
            else:
                run_exec("DELETE FROM LeaseDocumentSections WHERE LeaseDocumentSectionID = ?", (section_id_int,), db=self.db)
    ...
    def _load_reusable_section_options(self):
        try:
            rows = run_query(
                "SELECT p.LeaseDocumentSectionID, p.SectionType, p.SectionName, ISNULL(p.ExhibitCode,'') AS ExhibitCode, "
                ...
                "ORDER BY p.SectionType, p.SortOrder, p.SectionName",
                db=self.db,
            )
        except Exception:
            rows = []
        ...
        ids = [0]
        for r in rows:
            pid = int(r.get("LeaseDocumentSectionID") or 0)
    ...
    def _section_label_for_id(self, section_id) -> str:
        ...
        rows = run_query(
            "SELECT SectionType, SectionName, ISNULL(ExhibitCode,'') AS ExhibitCode "
            "FROM LeaseDocumentSections WHERE LeaseDocumentSectionID = ?",
            (pid,), db=self.db,
        )
        if not rows:
            return ""
    ...
    def _selected_default_section_id(self) -> Optional[int]:
        if self.sec_default_section_label in self.reusable_section_labels:
            pid = self.reusable_section_ids[self.reusable_section_labels.index(self.sec_default_section_label)]
            return int(pid) if pid else None
        return None
        ...
        try:
            if self.section_mode == "edit" and int(self.selected_section_id or 0) > 0:
                ...
                run_exec(
                    ...
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                    (
                        int(self.selected_template_id), sort_order, self.sec_label.strip(),
    ...
    return rx.table.row(
        ...
        rx.table.cell(
            rx.button(
                ...
                on_click=lambda: LeaseDocumentState.toggle_section_reusable(row.section_id),
            )
        ),
        rx.table.cell(
            rx.button(
                ...
                on_click=lambda: LeaseDocumentState.toggle_section_active(row.section_id),
            )
        ),
        rx.table.cell(
            rx.cond(
        ...
        rx.table.cell(
            rx.hstack(
                rx.button("Edit", size="1", variant="soft", color_scheme="blue", on_click=lambda: LeaseDocumentState.edit_section(row.section_id)),
                rx.button("Delete", size="1", variant="soft", color_scheme="red", on_click=lambda: LeaseDocumentState.delete_section(row.section_id)),
                spacing="2",
            )
        ),
        style=rx.cond(
...
def _tab_load() -> rx.Component:
    """Tab 1 — Load: upload source PDFs and manage storage root."""
    return rx.vstack(
        _feedback_callouts(),
        ...
        rx.card(
            rx.vstack(
                rx.text("Storage root", size="3", weight="bold", color=BRAND_DARK),
                rx.text(
                    "Root folder for the admin document library. Files are stored on disk.",
                    size="2", color="#666",
                ),
                rx.input(
                    value=LeaseDocumentState.storage_root,
                    on_change=LeaseDocumentState.set_storage_root,
                    placeholder=r"C:\TenantCRM\LeaseDocuments",
                    width="100%",
                ),
                rx.box(
                ...
                rx.cond(
                    ...
                    rx.vstack(
                        ...
                        rx.text("Enabled from Admin Settings. Hidden in normal workflow.", size="1", color="#777"),
                        rx.hstack(
        ...
        rx.card(
            rx.vstack(
                ...
                rx.text(
                    "Each source PDF becomes the origin document for section splitting. "
                    "After uploading, switch to the Parse & Section tab to split it.",
                    size="2", color="#666",
                ),
                rx.grid(
        ...
        rx.card(
            rx.vstack(
                rx.text("Ingested source documents", size="3", weight="bold", color=BRAND_DARK),
                rx.text(
                    "Click Split → to select a document and jump to the Parse & Section tab.",
                    size="2", color="#666",
                ),
                rx.table.root(
                    rx.table.header(
                        rx.table.row(
                            ...
                            rx.table.column_header_cell("Template name"),
...
def _tab_templates() -> rx.Component:
    ...
    return rx.vstack(
        ...
        rx.grid(
            ...
            # Right column — template detail + section slot editor
            rx.vstack(
                # Template header form
                rx.card(
                    rx.vstack(
                        rx.text(
                            rx.cond(LeaseDocumentState.selected_template_id > 0, "Edit package template", "New package template"),
                            size="3", weight="bold", color=BRAND_DARK,
                        ),
                        rx.grid(
                            rx.vstack(
                                rx.text("Template name", size="1", color="#666"),
                                rx.input(value=LeaseDocumentState.lt_template_name, on_change=LeaseDocumentState.set_lt_template_name, placeholder="Broadway Modified-Gross", width="100%"),
                                spacing="1", width="100%",
                            ),
                            rx.vstack(
                                rx.text("Property", size="1", color="#666"),
                                rx.select(LeaseDocumentState.property_names, value=LeaseDocumentState.lt_property, on_change=LeaseDocumentState.set_lt_property, width="100%"),
                                spacing="1", width="100%",
                            ),
                            columns="2",
                            spacing="3",
                        ...
                        rx.vstack(
                            ...
                            rx.text_area(value=LeaseDocumentState.lt_description, on_change=LeaseDocumentState.set_lt_description, width="100%", height="60px"),
                            spacing="1", width="100%",
                        ),
                        rx.checkbox("Active", checked=LeaseDocumentState.lt_is_active, on_change=LeaseDocumentState.set_lt_is_active),
                        rx.hstack(
                            rx.button(
                                rx.cond(LeaseDocumentState.selected_template_id > 0, "Save Template", "Create Template"),
                                on_click=LeaseDocumentState.save_lease_template,
                                color_scheme="blue",
                            ),
                            rx.button("Clear", on_click=LeaseDocumentState.new_lease_template, variant="soft", color_scheme="gray"),
                            spacing="3",
                        ),
                        spacing="3",
                        width="100%",
                ...
                rx.card(
                    rx.vstack(
                        rx.hstack(
                            rx.text("Section slots", size="3", weight="bold", color=BRAND_DARK),
                            rx.spacer(),
                            rx.button(
                                "New Section Slot",
                        ...
                        # Section slot form
                        rx.cond(
                            LeaseDocumentState.selected_section_id > 0,
                            rx.callout.root(
                                rx.callout.text(
                                    rx.hstack(
                                        rx.text("Editing slot ID", size="2"),
                                        rx.badge(LeaseDocumentState.selected_section_id.to_string(), color_scheme="amber", variant="soft"),
                                        spacing="2",
                                        align="center",
                                    )
                                ),
                        ...
                        rx.grid(
                            rx.vstack(
                                ...
                                rx.input(value=LeaseDocumentState.sec_label, on_change=LeaseDocumentState.set_sec_label, placeholder="Article 3 — Rent", width="100%"),
                                spacing="1", width="100%",
                            ),
                            rx.vstack(
                                rx.text("Sort order", size="1", color="#666"),
                                rx.input(value=LeaseDocumentState.sec_sort_order, on_change=LeaseDocumentState.set_sec_sort_order, width="100%"),
                                spacing="1", width="100%",
                            ),
                            rx.vstack(
                                rx.text("Section type", size="1", color="#666"),
                                rx.select(SECTION_TYPES, value=LeaseDocumentState.sec_section_type, on_change=LeaseDocumentState.set_sec_section_type, width="100%"),
                                spacing="1", width="100%",
                            ),
                            columns="3",
                            spacing="3",
                        ...
                        rx.vstack(
                            ...
                            rx.select(
                                LeaseDocumentState.reusable_section_labels,
                                value=LeaseDocumentState.sec_default_section_label,
                                on_change=LeaseDocumentState.set_sec_default_section_label,
                                width="100%",
                            ),
                            rx.text("Reusable, active sections only.", size="1", color="#777"),
                            spacing="1",
                            width="100%",
                        ),
                        rx.vstack(
                            rx.text("Inclusion", size="1", color="#666"),
                            rx.select(
                                INCLUSION_MODES,
                                value=LeaseDocumentState.sec_inclusion_mode,
                                on_change=LeaseDocumentState.set_sec_inclusion_mode,
                                width="100%",
                            ),
                            rx.text("Required — always included. Optional — user may remove. Inactive — hidden from builder.", size="1", color="#777"),
                            spacing="1",
                            width="100%",
                        ),
                        rx.hstack(
                            rx.button(
                                rx.cond(LeaseDocumentState.selected_section_id > 0, "Save Slot", "Add Slot"),
                        ...
                        rx.cond(
                            ...
                            rx.table.root(
                                rx.table.header(
                                    rx.table.row(
                                        ...
                                        rx.table.column_header_cell("Inclusion"),
                                        rx.table.column_header_cell("Actions"),
                                    )
                                ),
                            ...
                            rx.text("No section slots yet. Add one above.", size="2", color="#888"),
                        ),
                        spacing="3",
                        width="100%",
                        align_items="start",
