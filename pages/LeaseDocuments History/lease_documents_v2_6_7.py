...
# VERSION: 2.6.7 - adds admin lease template manager UI and section CRUD.
...
class LeaseDocumentState(AppState):
    ...
    reusable_piece_labels: list[str] = ["(No default piece)"]
    ...
    sec_default_piece_label: str = "(No default piece)"
    ...
    def _validate_piece_range(self, start: int, end: int, ignore_piece_id: int = 0, require_non_overlap: bool = True) -> bool:
        ...
        duplicate pieces without first cleaning the database manually.
        """
    ...
    def edit_piece(self, piece_id: int):
        ...
        if not rows:
            self.form_error = "Piece not found."
            return
        r = rows[0]
    ...
    def delete_piece(self, piece_id: int):
        ...
        try:
            ...
            if used:
                ...
                self.form_success = "Piece is used by a generated package, so it was archived instead of deleted."
            else:
                ...
                if stored_path and os.path.isfile(stored_path):
                    try:
                        ...
                        pass
                self.form_success = "Piece deleted."
    ...
    def set_piece_reusable_flag(self, piece_id: int, reusable: bool):
        """Persist whether this piece is available in the tenant lease builder."""
        self.form_error = ""
        self.form_success = ""
        try:
            run_exec(
    ...
    def save_piece_content(self):
        ...
        if int(self.editing_piece_id or 0) <= 0:
            self.form_error = "Select a piece with Edit before saving template content."
            return
        try:
            run_exec(
    ...
    def create_piece(self):
        ...
        try:
            ...
            if current_edit_id:
                ...
                if old_path and old_path != piece_path and os.path.isfile(old_path):
                    try:
                        ...
                        pass
                self.form_success = "Piece updated."
                self.editing_piece_id = 0
            else:
                run_exec(
            ...
            self.form_error = f"Could not save piece: {ex}"
    ...
    def reset_template_section_form(self):
        ...
        self.sec_default_piece_label = self.reusable_piece_labels[0] if self.reusable_piece_labels else "(No default piece)"
    ...
    def edit_template_section(self, section_id: int):
        ...
        self.sec_default_piece_label = self._piece_label_for_id(default_piece_id) if default_piece_id else "(No default piece)"
    ...
    def save_template_section(self):
        ...
        self.form_success = ""
        if int(self.selected_template_id or 0) <= 0:
            self.form_error = "Save or select a lease template first."
            return
...
def lease_template_manager_card() -> rx.Component:
    return rx.card(
        rx.vstack(
            rx.text("4. Manage lease templates", size="4", weight="bold", color=BRAND_DARK),
            rx.text(
                "Define reusable lease templates and ordered sections. This does not connect templates to package generation yet.",
                size="2",
                color="#666",
            ),
            rx.grid(
                rx.box(
                    rx.vstack(
                        rx.hstack(
                            rx.text("Lease templates", size="3", weight="bold", color=BRAND_DARK),
                            rx.spacer(),
                            rx.button(
                                "New Template",
                        ...
                        rx.cond(
                            ...
                            rx.text("No lease templates yet.", size="2", color="#888"),
                        ),
                        spacing="3",
                        width="100%",
                        align_items="start",
                ...
                rx.box(
                    rx.vstack(
                        rx.text("Template details", size="3", weight="bold", color=BRAND_DARK),
                        rx.grid(
                            rx.vstack(
                                ...
                                rx.input(
                                    ...
                                    placeholder="Broadway Standard Lease",
                                    width="100%",
                                ),
                                spacing="1",
                                width="100%",
            ...
            rx.vstack(
                ...
                rx.vstack(
                    rx.text("Default piece", size="1", color="#666"),
                    rx.select(
                    ...
                    rx.text("Reusable, active pieces only. Labels are sorted and grouped by PieceType.", size="1", color="#777"),
                    spacing="1",
                    width="100%",
                ),
                rx.hstack(
                ...
                rx.cond(
                    ...
                    rx.table.root(
                        rx.table.header(
                            rx.table.row(
                                ...
                                rx.table.column_header_cell("Default Piece"),
                    ...
                    rx.text("No sections for the selected template yet.", size="2", color="#888"),
                ),
                spacing="3",
                width="100%",
                align_items="start",
    ...
    return rx.vstack(
        ...
        rx.card(
            rx.vstack(
                ...
                rx.vstack(
                    rx.text("Template content for merge fields", size="2", weight="bold", color=BRAND_DARK),
                    rx.text("Optional. PDF splitting still works without this. Add {{Tokens}} here when a piece should merge tenant and lease data.", size="1", color="#666"),
                    rx.text_area(
                    ...
                    rx.hstack(
                        ...
                        rx.text("Use Edit on an existing piece before saving content only.", size="1", color="#777"),
                        spacing="3",
                        align="center",
                    ),
                    rx.box(
                ...
                rx.hstack(
                    rx.button(
                        rx.cond(LeaseDocumentState.editing_piece_id > 0, "Update Piece", "Split and Save Piece"),
