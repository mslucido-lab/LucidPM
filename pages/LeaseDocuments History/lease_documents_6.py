...
class LeaseDocumentState(AppState):
    ...
    def _validate_piece_range(self, start: int, end: int, ignore_piece_id: int = 0) -> bool:
        if start < 1 or end < start or end > self.selected_source_page_count:
            ...
            return False
        rows = run_query(
            "SELECT LeaseDocumentPieceID, StartPage, EndPage, PieceName FROM LeaseDocumentPieces WHERE LeaseSourceDocumentID = ?",
            (self.selected_source_document_id,), db=self.db,
        )
        for r in rows:
            if ignore_piece_id and int(r.get("LeaseDocumentPieceID") or 0) == int(ignore_piece_id):
                continue
    ...
    def edit_piece(self, piece_id: int):
        ...
        rows = run_query(
            "SELECT LeaseDocumentPieceID, PieceType, PieceName, ExhibitCode, StartPage, EndPage, "
            "SortOrder, IsReusable, IsActive FROM LeaseDocumentPieces WHERE LeaseDocumentPieceID = ?",
    ...
    def delete_piece(self, piece_id: int):
        ...
        try:
            run_exec("DELETE FROM LeaseDocumentPieces WHERE LeaseDocumentPieceID = ?", (int(piece_id),), db=self.db)
            if stored_path and os.path.isfile(stored_path):
                try:
                    os.remove(stored_path)
                except OSError:
                    pass
            if self.editing_piece_id == int(piece_id):
                self.reset_piece_form()
            self._load_pieces()
            self.form_success = "Piece deleted."
        except Exception as ex:
            self.form_error = f"Could not delete piece: {ex}"
    ...
    def toggle_piece_reusable(self, piece_id: int):
        ...
        try:
            run_exec(
                "UPDATE LeaseDocumentPieces SET IsReusable = CASE WHEN IsReusable = 1 THEN 0 ELSE 1 END WHERE LeaseDocumentPieceID = ?",
                (int(piece_id),), db=self.db,
            )
            self._load_pieces()
        except Exception as ex:
    ...
    def create_piece(self):
        ...
        try:
            ...
            return
        if not self._validate_piece_range(start, end, self.editing_piece_id):
            return
        ...
        if self.p_piece_type == "Exhibit" and code:
            dup = run_query(
                ...
                (self.selected_source_document_id, code, int(self.editing_piece_id or 0)), db=self.db,
            )
            if dup:
        ...
        try:
            ...
            if self.editing_piece_id:
                old_rows = run_query(
                    "SELECT StoredFilePath FROM LeaseDocumentPieces WHERE LeaseDocumentPieceID = ?",
                    (int(self.editing_piece_id),), db=self.db,
                )
    ...
    return rx.table.row(
        ...
        rx.table.cell(
            rx.button(
                ...
                on_click=LeaseDocumentState.toggle_piece_reusable(row.piece_id),
            )
        ),
        rx.table.cell(
            rx.button(
                ...
                on_click=LeaseDocumentState.toggle_piece_active(row.piece_id),
            )
        ),
        rx.table.cell(
            rx.hstack(
                rx.button("Edit", size="1", variant="soft", color_scheme="blue", on_click=LeaseDocumentState.edit_piece(row.piece_id)),
                rx.button("Delete", size="1", variant="soft", color_scheme="red", on_click=LeaseDocumentState.delete_piece(row.piece_id)),
                spacing="2",
            )
        ),
        style=rx.cond(
