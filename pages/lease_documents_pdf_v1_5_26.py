...
def _markup_or_xml(text: str, preserve_line_breaks: bool = False) -> str:
    ...
    if _looks_like_reportlab_markup(raw):
        ...
        if "<para" in normalized.lower():
            return normalized
        # Inline markup often comes from Word/PDF soft wraps. Collapse physical
        # line breaks to spaces so justified legal paragraphs still use the full
        # available line width. Authors can still force breaks with <br/>.
        return re.sub(r"[ \t]*\n[ \t]*", " ", normalized)
