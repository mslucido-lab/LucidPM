...
- v1.5.43: centers grouped 3+ year payment schedules and keeps Special Conditions parent with first child.
...
def _schedule_table_flowable(lines: list[str]):
    ...
    grouped_schedule = not has_two_column_rows
    ...
    if grouped_schedule:
        # For grouped 3+ year schedules, the table is narrow enough that the
        # old clause-body wrapper visually pulls it left. Return the table
        # directly so ReportLab centers it in the frame.
...
def _is_special_conditions_parent(block: str) -> bool:
    """True for parent Special Conditions headers that should stay with first child."""
...
def _story_for_text_sections(section_texts: Sequence[str]) -> list:
    ...
    while section_index < len(texts):
        blocks = _split_blocks(texts[section_index])

        # Keep "22. SPECIAL CONDITIONS" with the first lettered child condition
        # even when they are stored as separate reusable sections. This avoids
        # the parent header sitting alone above the signature footer.
        if (
            len(blocks) == 1
            ...
            if next_blocks and _is_special_conditions_child(next_blocks[0]):
                grouped_flowables = []
                grouped_flowables.extend(_paragraph_for_block(blocks[0]))
                grouped_flowables.extend(_paragraph_for_block(next_blocks[0]))
