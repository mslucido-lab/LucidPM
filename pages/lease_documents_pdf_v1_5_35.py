...
def _story_for_text_sections(section_texts: Sequence[str]) -> list:
    ...
    while idx < len(flat_blocks):
        ...
        # Keep 22. SPECIAL CONDITIONS with the first child condition. This is
        # stronger than CondPageBreak alone because the child clause lives in a
        # separate reusable section and must still travel with the header.
        if _is_special_conditions_parent(block) and idx + 1 < len(flat_blocks):
            next_block = flat_blocks[idx + 1]
            if _is_special_conditions_child(next_block):
                grouped_flowables = []
                grouped_flowables.extend(_paragraph_for_block(block))
                grouped_flowables.extend(_paragraph_for_block(next_block))
                if grouped_flowables:
                    # Force Special Conditions to begin on a fresh page when the
                    # first child condition follows. Conditional page breaks were
                    # not reliable here because the child can be large enough that
                    # ReportLab abandons KeepTogether and leaves the header behind.
                    # A hard page break is safer and matches the lease layout rule:
                    # never leave 22. SPECIAL CONDITIONS orphaned at page bottom.
                    if story:
                        story.append(PageBreak())
                    story.extend(grouped_flowables)
                idx += 2
                continue
