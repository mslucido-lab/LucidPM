...
def _markup_or_xml(text: str, preserve_line_breaks: bool = False) -> str:
    ...
    if _looks_like_reportlab_markup(raw):
        ...
        # Do not inject <br/> around <para> root blocks. ReportLab expects
        # <para> to stand alone, not be followed by sibling markup in the same Paragraph.
        if "<para" in normalized.lower():
            return normalized
...
def _markup_flowables(text: str, style) -> list:
    ...
    one Paragraph. Split those into separate Paragraphs.
    """
    ...
    after = raw[pos:].strip()
    if after:
        parts.extend([p.strip() for p in after.split("\n") if p.strip()])
    ...
    for part in parts:
        if not part:
            continue
        if "<para" in part.lower():
            flowables.append(Paragraph(part, style))
        else:
            flowables.append(Paragraph(_markup_or_xml(part), style))
    return flowables
...
def _paragraph_for_block(block: str):
    ...
    if re.match(r"^[a-z]\)\s+", clean) or re.match(r"^[ivxlcdm]+[\.)]\s+", clean, flags=re.IGNORECASE):
        return _markup_flowables(clean, LEASE_STYLES["INDENTED_BODY"])
