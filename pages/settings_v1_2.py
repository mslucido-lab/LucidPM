...
class SettingsState(AppState):
    ...
    def save_email_password(self):
        if not self.email_password:
            return
        try:
            encrypted = encrypt_value(self.email_password)
            run_exec(
    ...
    def save_ai_api_key(self):
        if not self.ai_api_key:
            return
        try:
            encrypted = encrypt_value(self.ai_api_key)
            run_exec(
    ...
    def test_email_connection(self):
        ...
        try:
            pw = decrypt_value(pw_rows[0]["PasswordEncrypted"])
        except Exception as e:
    ...
    def test_ai_connection(self):
        ...
        try:
            ...
            if not key_rows or not key_rows[0].get("APIKeyEncrypted"):
                ...
                return
            key = decrypt_value(key_rows[0]["APIKeyEncrypted"])
